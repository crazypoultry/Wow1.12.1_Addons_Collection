...
local oldPickup, PickupBag, PickupSlot
...
function XPerl_Check_PlayerOnClick(button)
	...
	if (button == "LeftButton") then
		if (not isWoW2) then
			TargetUnit(XPerl_PlayerList[index].unit)		-- TBC - remove probably
		end
	end
...
local function XPerl_GetChannelList()
	...
	channelList = {	{display = CHAT_MSG_RAID,	channel = "RAID"	, colour = GetChatColour("RAID")	},
			{display = CHAT_MSG_OFFICER,	channel = "OFFICER"	, colour = GetChatColour("OFFICER")	},
			{display = CHAT_MSG_GUILD,	channel = "GUILD"	, colour = GetChatColour("GUILD")	},
			{display = CHAT_MSG_PARTY,	channel = "PARTY"	, colour = GetChatColour("PARTY")	},
			{display = CHAT_MSG_SAY,	channel = "SAY"		, colour = GetChatColour("SAY")		}}

	for i = 1,10 do
		local c, name = GetChannelName(i)
		if (name and c ~= 0) then
			tinsert(channelList, {display = name, channel = "CHANNEL", index = c, colour = GetChatColour("CHANNEL"..c)})
		end
	end
end
...
function XPerl_Check_Channels_OnLoad()
	...
	UIDropDownMenu_SetSelectedID(this, outputChannelSelection, 1)
	UIDropDownMenu_SetWidth(100, XPerl_CheckButtonChannel)
end

-- XPerl_Channel_OnClick
local function XPerl_Channel_OnClick()

	if (this:GetID()) then
		local entry = channelList[this:GetID()]
		if (entry) then
			outputChannel = entry.channel
			outputChannelIndex = entry.index
			outputChannelSelection = this:GetID()
