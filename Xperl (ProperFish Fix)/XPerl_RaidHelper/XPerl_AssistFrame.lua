...
local playerFadeStart, petFadeStart = 0, 0
...
function XPerl_Assists_OnUpdate()
	...
	if (XPerlConfigHelper.AggroWarning == 1) then
		XPerl_RefreshAggro()
	end
end
...
local function XPerl_AddEnemy(anyEnemy, FoundEnemy, name)
	...
	elseif (not petAggro and XPerlConfigHelper.AggroWarning == 1 and UnitIsUnit("pet", name.."targettarget")) then
		petAggro = true
...
function XPerl_UpdateAssists()
	...
	if (total < 1) then
		...
		playerAggro, petAggro = false, false
		playerFadeStart, petFadeStart = 0, 0
	end
...
-- XPerl_RefreshAggro
function XPerl_RefreshAggro()
	if (playerAggro) then
		XPerl_AggroPlayer:SetVertexColor(1, 0, 0, 1)
	else
		local elapsed = GetTime() - playerFadeStart
		if (elapsed < 1) then
	...
	if (petAggro) then
		...
		local elapsed = GetTime() - petFadeStart
		if (elapsed < 1) then
