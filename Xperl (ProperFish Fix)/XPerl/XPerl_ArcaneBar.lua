...
local events = {"SPELLCAST_START", "SPELLCAST_STOP", "SPELLCAST_FAILED", "SPELLCAST_INTERRUPTED",
		"SPELLCAST_DELAYED", "SPELLCAST_CHANNEL_START", "SPELLCAST_CHANNEL_UPDATE", "SPELLCAST_CHANNEL_STOP"}
...
function XPerl_ArcaneBar_OnEvent()		-- TBC - function XPerl_ArcaneBar_OnEvent(self, event)
	if ( event == "SPELLCAST_START" ) then
		...
		XPerl_ArcaneBar_Spell:SetText(arg1)
		XPerl_Player_NameFrame_NameBarText:Hide()
	...
	elseif ( event == "SPELLCAST_CHANNEL_START" ) then
		...
		XPerl_ArcaneBarSpark:Show()
		XPerl_Player_NameFrame_NameBarText:Hide()
		XPerl_ArcaneBar_Spell:SetText(arg2)
		this.maxValue = 1
		this.startTime = GetTime()
...
function XPerl_ArcaneBar_OnUpdate()
	if (not XPerl_ArcaneBar:IsShown()) then
		XPerl_ArcaneBar_CastTime:Hide()
		XPerl_Player_NameFrame_NameBarText:Show()
	end
	...
	if ( this.casting ) then
		...
		local sparkPosition = ((status - this.startTime) / (this.maxValue - this.startTime)) * XPerl_ArcaneBarFrame:GetWidth() / XPerl_ArcaneBarFrame:GetEffectiveScale()
		if ( sparkPosition < 0 ) then
			sparkPosition = 0
		end
		XPerl_ArcaneBarSpark:SetPoint("CENTER", "XPerl_ArcaneBar", "LEFT", sparkPosition, 1)
	elseif ( this.channeling ) then
		...
		XPerl_ArcaneBarFlash:Hide()
		local sparkPosition = ((barValue - this.startTime) / (this.endTime - this.startTime)) * XPerl_ArcaneBarFrame:GetWidth() / XPerl_ArcaneBarFrame:GetEffectiveScale()
	...
	elseif ( this.fadeOut ) then
		...
		if ( alpha > 0 ) then
			this:SetAlpha(alpha)
			XPerl_Player_NameFrame_NameBarText:SetAlpha((1 - alpha) * XPerlConfig.TextTransparency)
			XPerl_Player_NameFrame_NameBarText:Show()
		else
			this.fadeOut = nil
			this:Hide()
...
function XPerl_ArcaneBar_OnLoad()
	...
	XPerl_RegisterBar(XPerl_ArcaneBar)
end
