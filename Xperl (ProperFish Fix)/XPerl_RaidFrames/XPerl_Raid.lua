...
function XPerl_Raid_OnLoad()
	...
	if (not IsAddOnLoaded("EasyRaid")) then
...
function XPerl_Raid_UpdateManaType(thisFrame, skipFlags)
	...
	if (not flags) then
		if (XPerlConfig.RaidMana == 1) then
			...
			XPerl_SetManaBarType(thisFrame.partyid, frame)
		end
	end
end
...
local function XPerl_Raid_UpdateHealth(thisFrame)
	...
	if (name and UnitIsConnected(partyid)) then
		if (myRoster and myRoster.fd) then
			if (not XPerl_IsFeignDeath(partyid)) then
				myRoster.fd = nil
			end
		end
		...
		elseif (UnitIsGhost(partyid)) then
			...
			if (XPerlConfig.HealerModeRaid == 1) then
...
-- CheckRosterBuffs
-- Check buffs exist in roster that we've received, if not then strip them out
local function CheckRosterBuffs(unitName)
	local id = XPerl_GetRaidPosition(unitName)
	if (id) then
		id = "raid"..id
		if (UnitIsConnected(id)) then
			local buffs = XPerl_Roster[unitName].Buffs
			if (buffs) then
				for buffName,buffTimeEnd in pairs(buffs) do
					if (buffTimeEnd < GetTime()) then
						buffs[buffName] = nil
					end
				end
			end

			buffs = XPerl_Roster[unitName].Debuffs
			if (buffs) then
				for buffName,buffTimeEnd in pairs(buffs) do
					if (buffTimeEnd < GetTime()) then
						buffs[buffName] = nil
...
local function UpdateBuffs(thisFrame)
	local partyid = thisFrame.partyid
	local cursed, color = XPerl_CheckDebuffs(partyid, thisFrame.FlashFrames)
...
local duplicateTextures = {
	...
	["Interface\\Icons\\Spell_Shadow_AntiShadow"] = true,
	["Interface\\Icons\\Spell_Holy_PrayerOfHealing02"] = true,
};

local debuffTimers = {	Spell_Nature_SpiritArmor = 15 * 60,	-- Mark of Nature
			Spell_Frost_ChainsOfIce = 15 * 60,	-- Mark of Frost
			INV_Misc_Bandage_08 = 60,		-- Recently Bandaged
			Spell_Shadow_DeathScream = 10 * 60,	-- Resurrection Sickness
}

local function UpdateRosterBuffs(thisFrame)

	local unit = thisFrame.partyid
	local myRoster = XPerl_Roster[UnitName(unit)]

	if (myRoster) then
		if (not myRoster.Buffs) then
			myRoster.Buffs = {}
		end
		if (not myRoster.Debuffs) then
			myRoster.Debuffs = {}
		end
		...
		for k,v in pairs(myRoster.Debuffs) do
			oldDebuffs[k] = 1
		end

		local num, buff = 1, UnitBuff(unit, 1);
		while (buff) do
			if (not duplicateTextures[buff]) then
				...
				XPerlBuffStatusTooltip:SetOwner(WorldFrame, "ANCHOR_NONE")
				XPerlBuffStatusTooltipTextLeft1:SetText("")
				XPerlBuffStatusTooltip:SetUnitBuff(unit, num)
				local tooltipName = XPerlBuffStatusTooltipTextLeft1:GetText()
			...
			if (buffName) then
				...
				if (not myRoster.Buffs[buffName]) then
					...
					local ctBuff = XPerl_CTBuffTextures[buffName]
					if (ctBuff) then
						local duration = ctBuff[2]
		...
		num, buff = 1, UnitDebuff(unit, 1);
		while (buff) do
			buff = strsub(buff, 17)
			local duration = debuffTimers[buff]

			if (duration) then
				oldDebuffs[buff] = nil
				if (not myRoster.Debuffs[buff]) then
					myRoster.Debuffs[buff] = GetTime() + duration
				end
			end

			num = num + 1
			buff = UnitDebuff(unit, num)
		end

		for k, v in pairs(oldAuras) do
			myRoster.Buffs[k] = nil
		end
		for k,v in pairs(oldDebuffs) do
			myRoster.Debuffs[k] = nil
		end

		local any
		for k,v in pairs(myRoster.Debuffs) do any = true break end
		if (not any) then myRoster.Debuffs = nil end

		any = nil
		for k,v in pairs(myRoster.Buffs) do any = true break end
		if (not any) then myRoster.Buffs = nil end
	end
end
			...
			if (secsLeft and buffIndex and buffSub) then
				...
				if (buff) then
					...
					if (buffName) then
						...
						if (delete) then
							-- Clear a buff's partner buff (can only have 1 of PWF group vs single for example)
							myRoster.Buffs[delete] = nil
						end
		...
		if (oldCell) then
			if (oldCell.lastPos) then
				if (oldCell.lastPos.group ~= grp or oldCell.lastPos.row ~= RaidGroupCounts[grp] + 1) then
					if (ManualMovement == name) then
						ManualMovement = nil
					else
						-- Cells have been re-arranged so we have to move the cell so the motion comes out right
						local parentTemp = getglobal(XPERL_RAIDGRP_PREFIX..oldCell.lastPos.group)
						local t
						if (XPerlConfig.RaidUpward == 1) then
							t = -2 + ((oldCell.lastPos.row - 1) * Spacing())
						else
							t = 2 - ((oldCell.lastPos.row - 1) * Spacing())
						end
						...
						cell:ClearAllPoints()
						if (XPerlConfig.RaidUpward == 1) then
							cell:SetPoint("BOTTOMLEFT", parentTemp, "TOPLEFT", 0, t)
						else
							cell:SetPoint("TOPLEFT", parentTemp, "BOTTOMLEFT", 0, t)
						end
					end
...
local function XPerl_Raid_Classes()
	...
	for num=1,MAX_RAID_MEMBERS do
		...
		if (class) then
			...
			if (subgroup) then
				if (subgroup == 9) then		-- TBC - Burning Crusade - REMOVE THIS
					subgroup = 8
				end
	...
	for name,index in pairs(groups) do
		...
		if (frame) then
			if (index == 8) then	-- TBC - Burning Crusade - REMOVE THIS
...
function XPerl_Raid_Position()
	...
	Last_Roster = nil
	for i,unit in pairs(XPerl_Roster) do
		unit.lastPos = nil
	end
...
function XPerl_Raid_SetBuffTooltip ()
	...
	elseif (show == "d") then
		...
		XPerl_Raid_AddDebuffDuration(partyid, GameTooltip, this:GetID(), cureCast)
	end
end
...
function XPerl_RaidTipExtra(unitid)
	...
	if (UnitInRaid(unitid)) then
		...
		if (stats) then
			...
			if (UnitIsDeadOrGhost(unitid) and not XPerl_IsFeignDeath(unitid)) then
...
-- XPerl_Raid_AddDebuffDuration
function XPerl_Raid_AddDebuffDuration(partyid, x, id, cureCast)
	local XTooltip
	if (x) then
		XTooltip = x
	else
	...
	if (UnitInRaid("player") and XTooltip:IsOwned(this)) then
		if (partyid ~= "player") then
			...
			if (stats and stats.Debuffs) then
				local buffName = XPerl_UnitDebuff(partyid, id, cureCast)

				if (buffName) then
					buffName = strsub(buffName, 17)
					local buff = stats.Debuffs[buffName]
					if (buff) then
						if (buff > GetTime()) then
							XTooltip:AddLine(SecondsToTime(buff - GetTime())..XPERL_RAID_TOOLTIP_REMAINING, 1, 0.82, 0)
						end
...
RaidPulloutDropDown_Initialize = function()
	if ( not UIDROPDOWNMENU_OPEN_MENU or not getglobal(UIDROPDOWNMENU_OPEN_MENU) or not getglobal(UIDROPDOWNMENU_OPEN_MENU).raidPulloutDropDown ) then
		return;
	end
	...
	if (currentPullout.pets) then
		info.text = XPERL_RAID_DROPDOWN_SHOWOWNER
	else
		info.text = XPERL_RAID_DROPDOWN_SHOWPET
	end
	info.func = function()
