...
function AHFav:ToggleAutoQuery()
	...
	if (self.GetOpt("autoQuery")) then
		ace:print(AH_FAV.AUTOQUERY_ENABLE_MSG)
	else
		ace:print(AH_FAV.AUTOQUERY_DISABLE_MSG)
	end
end
...
function AHFav:ToggleFSL()
	...
	if (self.GetOpt("FSL")) then
		ace:print(AH_FAV.FSL_ENABLE_MSG)
	else
		ace:print(AH_FAV.FSL_DISABLE_MSG)
	end
end
