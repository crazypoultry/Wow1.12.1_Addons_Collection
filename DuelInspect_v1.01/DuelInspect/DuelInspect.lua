...
function DuelInspect_StartDuelUnit()
	...
	if ( CheckInteractDistance("target", 3) ) then 
		...
		UIErrorsFrame:AddMessage(ERR_OUT_OF_RANGE, 1.0, 0.0, 0.0, 1.0, UIERRORS_HOLD_TIME);				  
	end
...
function DuelInspect_StaticPopup_OnHide()
	...
	if ( doHide ) then
		...
		getglobal("DuelInspect_ModelFrameBackdrop"):Hide();
		getglobal("DuelInspect_Model"):Hide();
...
function DuelInspect_UpdateStaticPopupSize(index)
	...
	for _,child in ipairs(kids) do
		if ( ( strfind( child:GetName(), "DuelInspect") ) and ( child:GetBottom() ) and ( child:IsVisible() ) ) then
			if ( child:GetName() ~= "DuelInspect_ModelFrame" ) then
...
function DuelInspect_Move_OnMouseDown(arg1)
...
function DuelInspect_Move_OnUpdate()

	if ( this.ismoving ) then

		offset = this:GetParent():GetCenter() - DuelInspect_ModelFrameBackdrop:GetCenter();	
			
		local move = this:GetCenter() + offset;
		local parent = this:GetParent():GetCenter();
		DuelInspect_Model.rotation = (move - parent) * 0.02;
		DuelInspect_Model:SetRotation(DuelInspect_Model.rotation);
	end
...
function DuelInspect_Main_OnMouseUp(arg1)
	if (arg1 == "LeftButton") then
		this:StopMovingOrSizing();
		this.ismoving = false;
		
		this.offset = DuelInspect_Model.rotation * 50;
		this:ClearAllPoints();
		
		for index = 1, STATICPOPUP_NUMDIALOGS, 1 do
			local frame = getglobal("StaticPopup"..index);
			if ( frame:IsVisible() and (frame.which == "DUEL_REQUESTED" or frame.which == "DUEL_CHALLENGE") ) then
				this:SetPoint("LEFT", "StaticPopup"..index, "LEFT" , ( 28 - getglobal("StaticPopup"..index):GetHeight() ), 0);
			end
		end
...
function DuelInspect_PrepareFrames(index,initOptions)
	...
	local point, relativeTo, relativePoint, xofs, yofs = getglobal("StaticPopup"..index):GetPoint();		
	getglobal("StaticPopup"..index):SetPoint(point, relativeTo, relativePoint , xofs, -128);
	...
	if ( DI_SAVE.ShowUsableItems and DI_ITEMUSES ~= nil) then	
	...
	newSize = getglobal("StaticPopup"..index):GetHeight();
	
	getglobal("DuelInspect_ModelFrame"):SetHeight( newSize + 25 );
	getglobal("DuelInspect_ModelFrame"):SetWidth( newSize );
	getglobal("DuelInspect_ModelFrame"):SetParent("StaticPopup"..index);
	getglobal("DuelInspect_ModelFrame"):SetPoint("LEFT", "StaticPopup"..index, "LEFT" , ( 28 - newSize ), 0);
	getglobal("DuelInspect_ModelFrame"):SetFrameStrata("HIGH");
	
	getglobal("DuelInspect_ModelFrameBackdrop"):SetHeight( newSize + 25 );
	getglobal("DuelInspect_ModelFrameBackdrop"):SetWidth( newSize );
	getglobal("DuelInspect_ModelFrameBackdrop"):SetPoint("LEFT", "StaticPopup"..index, "LEFT" , ( 28 - newSize ), 0);

	getglobal("DuelInspect_Model"):SetHeight( newSize );
	getglobal("DuelInspect_Model"):SetWidth( newSize );
	getglobal("DuelInspect_Model"):SetPoint("LEFT", "StaticPopup"..index, "LEFT" , ( 23 - newSize ), -10);
	
	if ( DI_SAVE.ShowModel ) then
		getglobal("DuelInspect_ModelFrame"):Show();
		getglobal("DuelInspect_ModelFrameBackdrop"):Show();
		getglobal("DuelInspect_Model"):Show();
	end
...
function DuelInspect_UpdateStaticPopup()
	...
	for index = 1, STATICPOPUP_NUMDIALOGS, 1 do
		...
		if ( frame:IsVisible() and (frame.which == "DUEL_REQUESTED" or frame.which == "DUEL_CHALLENGE") ) then
			...
			DuelInspect_TargetOpponent();
			DuelInspect_Model:SetUnit("target");
			DuelInspect_Model:SetRotation(270.2);
	...
	if ( CheckInteractDistance("target", 3) ) then
	  ...
	  for key,value in quality_count do
		...
		for key,val in DI_ITEMUSES do
			...
			getglobal("DuelInspect_UseItem"..cnt.."Frame"):SetWidth(getglobal("DuelInspect_UseItem"..cnt.."Text"):GetStringWidth());
