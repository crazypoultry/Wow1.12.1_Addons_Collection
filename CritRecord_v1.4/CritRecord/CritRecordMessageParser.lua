...
local CritRecord_GlobalStrings = {};
local ParseFunction;
local parseBuffer;
local parseOrder;
local parseCount;
...
function CritRecordGlobalParser_Format(formatString)
	parseBuffer = nil;
	parseOrder = {};
	parseCount = 0;
	
	--see if in "cache" table
	if (CritRecord_GlobalStrings[formatString] == nil) then
		local parseString = string.gsub(formatString, ".", ParseFunction);
		CritRecord_GlobalStrings[formatString] = { parseString, parseOrder };
	end
	
	return CritRecord_GlobalStrings[formatString];
end

function CritRecordGlobalParser_ParseMessage(arg1, formatString)
	local parseResult;
	local findResult;
	
	parseResult = CritRecordGlobalParser_Format(formatString);	
