...
function FuBar_FriendsFu:OnTooltipUpdate()
	...
	if ((AuldLangSyne_loaded and self.db.profile.tooltip.note_showauldlangsyne)
	...
	for i = 1, table.getn(self.players) do
		...
		if self.db.profile.tooltip.name_status and self.players[i][5] ~= "" then
			line['text'] = self.players[i][5].." "..self.players[i][1]
		else
		...
		if self.db.profile.tooltip.class_show then
			colcount = colcount + 1
			line['text'..colcount] = self.players[i][3]
		...
		if ((AuldLangSyne_loaded and self.db.profile.tooltip.note_showauldlangsyne)
			...
			if CT_PlayerNotes_loaded and self.db.profile.tooltip.note_showctplayernotes then
				temptext = temptext ..((CT_GuildNotes[self.players[i][1]] and (" {"..CT_PlayerNotes[self.players[i][1]].."} ")) or "")
			end
