...
local MINOR_VERSION = "$Revision: 16836 $"
...
function Jostle:Refresh(f1, f2, f3, f4, f5, f6, f7, f8)
	...
	for _,frame in ipairs(frames) do
		...
		if frame and lua51 and InCombatLockdown() and frame:IsProtected() then
			queue[frame] = true
		elseif ((frame and frame.IsUserPlaced and not frame:IsUserPlaced()) or ((frame == DEFAULT_CHAT_FRAME or frame == ChatFrame2) and SIMPLE_CHAT == "1") or frame == FramerateLabel) and (frame ~= ChatFrame2 or SIMPLE_CHAT == "1") then
