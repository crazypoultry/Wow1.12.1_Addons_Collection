...
function SelfCast_Enable(arg)
	...
	elseif( arg == 0 ) then
		...
		if( DEFAULT_CHAT_FRAME ) then
			DEFAULT_CHAT_FRAME:AddMessage("SelfCast_Enable(arg) returned an invalid argument.");
		end
	end
end

function SelfCast_RegisterUltimateUI()
	UltimateUI_RegisterConfiguration(
		"UUI_SELFCAST",
		"SECTION",
		"SelfCast",
		"Options to enable easier selfcasting."
	);
	UltimateUI_RegisterConfiguration(
		"UUI_SELFCAST_SEPARATOR",
		"SEPARATOR",
		"SelfCast",
		"Options to enable easier selfcasting."
	);
	UltimateUI_RegisterConfiguration(
		...
		"CHECKBOX",
		"Enable SelfCast",
		"Check this box to enable SelfCast.",
		SelfCast_Enable,
		1
	);
end
