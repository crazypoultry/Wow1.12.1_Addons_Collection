...
function CT_RA_ParseEvent(event)
	...
	local unitStats = CT_RA_Stats[arg2];
	...
	if ( strsub(event, 1, 13) == "CHAT_MSG_RAID" and type(sMsg) == "string" ) then
		if ( frame ) then
			...
			if ( chan and raidid ) then
				if ( rank >= 1 ) then
					if ( ctraChannel and chan ~= ctraChannel ) then
						LeaveChannelByName(ctraChannel);
					end
					chan = string.gsub(chan, "[^%w]", "");
	...
	elseif ( event == "CHAT_MSG_CHANNEL" and ctraChannel and arg9 and strlower(arg9) == strlower(ctraChannel) and sMsg and type(sMsg) == "string" ) then
		if ( frame ) then
			-- Unit is in raid
...
function CT_RA_OnEvent(event)
	if ( event == "RAID_ROSTER_UPDATE" or event == "PLAYER_ENTERING_WORLD" ) then
		...
		CT_RAOptions_Update();
		if ( CT_RA_NumRaidMembers == 0 and numRaidMembers > 0 and CT_RA_Level >= 2 ) then
	...
	elseif ( event == "UNIT_HEALTH" or event == "UNIT_MAXHEALTH" ) then
		...
		if ( id ) then
			...
			local hpp = ( UnitHealth(arg1) or 1 ) / ( UnitHealthMax(arg1) or 1 );
...
function CT_RA_UpdatePT(raidid, frame, height, key, val)
	...
	if ( raidid and UnitExists(raidid) and strlen(UnitName(raidid) or "") > 0 ) then
		...
		elseif ( UnitIsDead(raidid) or UnitIsGhost(raidid) ) then
			...
			frame.MPBG:Hide();
			frame.Status:Show();
			if ( CT_RA_CheckFD(UnitName(raidid), raidid) ) then
				frame.Status:SetText("Feign Death");
			else
				frame.Status:SetText("DEAD");
			end
