...
v2.3
- Added Khoas/UltimateUI Registration if present
...
v2.97
- Fixed default value error for UltimateUI Registration for Empty
...
- Updated file loading method to be in toc ratehr than xml

$Id: ImprovedErrorFrame.lua 2121 2005-07-12 14:20:21Z Sinaloit $
$Rev: 2121 $
$LastChangedBy: Sinaloit $
$Date: 2005-07-12 07:20:21 -0700 (Tue, 12 Jul 2005) $
]]--
...
ImprovedErrorFrame = {
	...
	IEFSetOptions = function(msg)
		...
		if (change) then
			ImprovedErrorFrame.syncUltimateUIToVars();
			return;
		end
...
IEF_TEST_FLAG2 = false;
		...
		if (event == "VARIABLES_LOADED") then
			if (Khaos) then
				...
				if (UltimateUI_RegisterConfiguration) then
					ImprovedErrorFrame.UltimateUIRegister();
				end
				if (Sky) then
	...
	-- Register everything with UltimateUI
	UltimateUIRegister = function()
		...
		ImprovedErrorFrame.syncVarsToUltimateUI();
	end;

	-- UltimateUI uses 0 & 1 not false/true so we need to convert
	...
	-- Sync changed variables to UltimateUI Vars
	syncUltimateUIToVars = function()
		if (UltimateUI_RegisterConfiguration) then
			UltimateUI_UpdateValue("UUI_IEF_NOTIFY_OPT", CSM_CHECKONOFF, ImprovedErrorFrame.convertVar(not ImprovedErrorSettings.displayOnError));
			UltimateUI_SetCVar("UUI_IEF_NOTIFY_OPT_X", ImprovedErrorFrame.convertVar(not ImprovedErrorSettings.displayOnError));
			UltimateUI_UpdateValue("UUI_IEF_BLINK_OPT", CSM_CHECKONOFF, ImprovedErrorFrame.convertVar(ImprovedErrorSettings.blinkNotification));
			UltimateUI_SetCVar("UUI_IEF_BLINK_OPT_X", ImprovedErrorFrame.convertVar(ImprovedErrorSettings.blinkNotification));
			UltimateUI_UpdateValue("UUI_IEF_COUNT_OPT", CSM_CHECKONOFF, ImprovedErrorFrame.convertVar(ImprovedErrorSettings.displayCount));
			UltimateUI_SetCVar("UUI_IEF_COUNT_OPT_X", ImprovedErrorFrame.convertVar(ImprovedErrorSettings.displayCount));
			UltimateUI_UpdateValue("UUI_IEF_ALWAYS_OPT", CSM_CHECKONOFF, ImprovedErrorFrame.convertVar(ImprovedErrorSettings.alwaysShow));
			UltimateUI_SetCVar("UUI_IEF_ALWAYS_OPT_X", ImprovedErrorFrame.convertVar(ImprovedErrorSettings.alwaysShow));
			UltimateUI_UpdateValue("UUI_IEF_SOUND_OPT", CSM_CHECKONOFF, ImprovedErrorFrame.convertVar(not ImprovedErrorSettings.gagNoise));
			UltimateUI_SetCVar("UUI_IEF_SOUND_OPT_X", ImprovedErrorFrame.convertVar(not ImprovedErrorSettings.gagNoise));
			UltimateUI_UpdateValue("UUI_IEF_EMPTY_OPT", CSM_CHECKONOFF, ImprovedErrorFrame.convertVar(ImprovedErrorSettings.emptyButton));
			UltimateUI_SetCVar("UUI_IEF_EMPTY_OPT_X", ImprovedErrorFrame.convertVar(ImprovedErrorSettings.emptyButton));
			UltimateUI_UpdateValue("UUI_IEF_DEBUG_OPT", CSM_CHECKONOFF, ImprovedErrorSettings.XMLDebug);
			UltimateUI_SetCVar("UUI_IEF_DEBUG_OPT_X", ImprovedErrorSettings.XMLDebug);
	...
	-- Sync variables to UltimateUI Vars
	syncVarsToUltimateUI = function()
		if (not UltimateUI_RegisterConfiguration) then
			return;
		end
		local cosVar = getglobal("UUI_IEF_NOTIFY_OPT_X");
		if (cosVar) then
			ImprovedErrorSettings.displayOnError = (cosVar < 1);
		end
		cosVar = getglobal("UUI_IEF_BLINK_OPT_X");
		if (cosVar) then
			ImprovedErrorSettings.blinkNotification = (cosVar > 0);
		end
		cosVar = getglobal("UUI_IEF_COUNT_OPT_X");
		if (cosVar) then
			ImprovedErrorSettings.displayCount = (cosVar > 0);
		end
		cosVar = getglobal("UUI_IEF_ALWAYS_OPT_X");
		if (cosVar) then
			ImprovedErrorSettings.alwaysShow = (cosVar > 0);
		end
		cosVar = getglobal("UUI_IEF_SOUND_OPT_X");
		if (cosVar) then
			ImprovedErrorSettings.gagNoise = (cosVar < 1);
		end
		cosVar = getglobal("UUI_IEF_EMPTY_OPT_X");
		if (cosVar) then
			ImprovedErrorSettings.emptyButton = (cosVar > 0);
		end
		cosVar = getglobal("UUI_IEF_DEBUG_OPT_X");
		if (cosVar) then
