...
function CT_RABoss_Defender_OnLoad()
	...
	CT_RABoss_AddDropDownButton("Anubisath Defenders", { CT_RABOSS_DEFENDER_PLAGUETELL, CT_RABOSS_DEFENDER_PLAGUETELL_INFOM }, "CT_RABoss_ModInfo", "sendPlagueTells", "CT_RABoss_SetInfo");
	...
	CT_RABoss_SetVar("Anubisath Defenders", "sendPlagueTells", 1);
end
...
function CT_RABoss_Ossirian_EventHandler(event)
	...
	if ( event == "CHAT_MSG_SPELL_PERIODIC_CREATURE_BUFFS" ) then
		if ( arg1 == CT_RABOSS_OSSIRIAN_SUPREMEBUFF ) then
			...
			CT_RABoss_PlaySound(1);
		elseif ( CT_RABoss_Mods["Ossirian the Unscarred"]["displayWeakness"] ) then
...
function CT_RABoss_Guardian_OnLoad()
	...
	CT_RABoss_SetVar("Anubisath Guardians", "sendPlagueTells", 1);
end
...
function CT_RABoss_Guardian_EventHandler(event)
	...
	if ( event == "CHAT_MSG_SPELL_PERIODIC_CREATURE_BUFFS" ) then
		if ( arg1 == CT_RABOSS_GUARDIAN_EXPLODESTRING ) then
			CT_RABoss_Announce(CT_RABOSS_GUARDIAN_EXPLODING, CT_RABoss_Mods["Anubisath Guardians"]["announce"]);
		elseif ( arg1 == CT_RABOSS_GUARDIAN_ENRAGESTRING ) then
			CT_RABoss_Announce(CT_RABOSS_GUARDIAN_ENRAGING, CT_RABoss_Mods["Anubisath Guardians"]["announce"]);
		end
	elseif ( event == "CHAT_MSG_SPELL_CREATURE_VS_CREATURE_DAMAGE" or event == "CHAT_MSG_SPELL_CREATURE_VS_PARTY_DAMAGE" or event == "CHAT_MSG_SPELL_CREATURE_VS_SELF_DAMAGE") then
