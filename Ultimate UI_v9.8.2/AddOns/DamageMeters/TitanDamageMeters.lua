...
-- NOTE: This needs to be updated each time that that quantities are added, removed, or changed around.
TITAN_DAMAGEMETERS_QUANTABBREVIATIONS = {
	"D", "H", "DT", "HT", "Cu", "DPS", "HPS", "DTPS", "HTPS", "CuPS", "IT", "NetD", "NetH", "D+H" };
		...
		if (TitanGetVar(TITAN_DAMAGEMETERS_ID, "ShowLabelText")) then
			...
			for quant = 1, DamageMeters_Quantity_MAX do
				if (shownValues[quant]) then
					...
					retstr = string.format("%s "..quantColorCode.."%s=|cFFFFFFFF%s"..FONT_COLOR_CODE_CLOSE, retstr, TITAN_DAMAGEMETERS_QUANTABBREVIATIONS[quant], TitanPanelDamageMeters_GetValue(quant));
				end
			end

			-- Ranks.
			local playerRanks = DamageMeters_rankTable[UnitName("Player")];
			if (nil ~= playerRanks and table.getn(playerRanks) > 0) then
				...
				for quant = 1, DamageMeters_Quantity_MAX do
					if (shownRanks[quant]) then
						...
						--local quantColorCode = DamageMeters_quantityColorCodeDefault[quant];
						local quantColorCode = "|cFFFFD040";
						retstr = string.format("%s "..quantColorCode.."%s#=|cFFFFFFFF%s"..FONT_COLOR_CODE_CLOSE, retstr, TITAN_DAMAGEMETERS_QUANTABBREVIATIONS[quant], rank);
					end
				end
			end
			...
			for quant = 1, DamageMeters_Quantity_MAX do
				if (shownLeaders[quant]) then
					--local quantColorCode = DamageMeters_quantityColorCodeDefault[quant];
					local quantColorCode = "|cFFFFD040";
					retstr = string.format("%s "..quantColorCode.."%sL=|cFFFFFFFF%s"..FONT_COLOR_CODE_CLOSE, retstr, TITAN_DAMAGEMETERS_QUANTABBREVIATIONS[quant], TitanPanelDamageMeters_GetLeaderName(quant));
				end
			end
...
function TitanPanelDamageMeters_GetLeaderName(quant)
	if (TITAN_AND_DAMAGEMETERS_LOADED == 2) then
		for player, struct in DamageMeters_rankTable do
