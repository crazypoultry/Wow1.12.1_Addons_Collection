...
function DiscordLib_Initialize()
	...
	DiscordLib_Tooltip:SetOwner(UIParent, "ANCHOR_NONE");
...
function DL_Get_ActionName(id, ranktoggle)
	...
	DiscordLib_Tooltip:SetOwner(UIParent, "ANCHOR_NONE", 100, 100);
...
function DL_Update_Auras(unit)
	...
	for i=1,16 do
		if (UnitBuff(unit, i)) then
			DiscordLib_Tooltip:SetUnitBuff(unit, i);
			if (DiscordLib_TooltipTextLeft1:IsShown()) then
				DL_BUFFS[unit][i] = DiscordLib_TooltipTextLeft1:GetText();
			else
				DL_BUFFS[unit][i] = "";
			end
		else
	...
	for i=1,16 do
		if (UnitDebuff(unit, i)) then
			DiscordLib_Tooltip:SetUnitDebuff(unit, i);
			if (DiscordLib_TooltipTextLeft1:IsShown()) then
				DL_DEBUFFS[unit][i] = DiscordLib_TooltipTextLeft1:GetText();
			else
				DL_DEBUFFS[unit][i] = "";
			end
			if (DiscordLib_TooltipTextRight1:IsShown()) then
				DL_STATUS[unit][i] = DiscordLib_TooltipTextRight1:GetText();
			else
				DL_STATUS[unit][i] = "";
			end
		else
