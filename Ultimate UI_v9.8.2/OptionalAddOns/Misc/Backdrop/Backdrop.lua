...
function Backdrop_ToggleLock(toggle)
	if (toggle) then
		...
		if (LockAllBackdrops == 1) then
			LockAllBackdrops = 0;
			--ChatFrame1:AddMessage("All Backdrops unlocked.",info.r, info.g, info.b);
		elseif (LockAllBackdrops == 0) then
			LockAllBackdrops = 1;
			--ChatFrame1:AddMessage("All Backdrops locked.",info.r, info.g, info.b);
		else
			LockAllBackdrops = 1;
...
function ToggleEnableBackdrops(toggle)
	if (toggle) then
		...
		if (EnableBackdrops == 0) then
			EnableBackdrops = 1;
			--ChatFrame1:AddMessage("All Backdrops enabled",info.r, info.g, info.b);
			ShowAllBackdrops();
		elseif (EnableBackdrops == 1) then
			EnableBackdrops = 0;
			--ChatFrame1:AddMessage("All Backdrops disabled",info.r, info.g, info.b);
			HideAllBackdrops();
		end
	end
end
...
function UUIBackdropChangeNumber(checked,value)
	if (value) then
		...
		elseif (value == 7) then
			...
			ChatFrame1:AddMessage("This shouldn't happen.",info.r,info.g, info.b);		
		end
	end
end

function UUIMainBackdrop_Onload()
	-- spare the pleasantries
	-- if (DEFAULT_CHAT_FRAME) then
	--	DEFAULT_CHAT_FRAME:AddMessage("Backdrop loaded");
	-- end
	...
	if (UltimateUI_RegisterConfiguration) then
		...
		-- ========================
		UltimateUI_RegisterConfiguration(
			"UUI_CLOCK_RESET_POSITION",		-- CVar
			"BUTTON",				-- Type
			CLOCK_OPTION_RESET_POSITION,		-- Short description
			CLOCK_OPTION_RESET_POSITION_INFO,	-- Long description
			ResetClockPosition,			-- Callback
			0,					-- Useless
			0,					-- Useless
			0,					-- Useless
			0,					-- Useless
			CLOCK_OPTION_RESET_POSITION_NAME	-- Button text
		);
		]]
	end
end
