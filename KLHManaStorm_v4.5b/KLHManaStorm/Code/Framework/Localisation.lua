...
-- module setup
local me = { name = "string" } -- we wanted to call this module "local", but the compiler complains cause he confuses it with the local keyword, lol!
local mod = thismod
mod[me.name] = me
...
Localisation.lua
	...
	The structure for localisation is probably a bit different to most mods. There is no long list of constants called MODNAME_COMBAT_RAZORGORE_PHASE2 or similar, because it looks silly, and is hard to debug, not to mention type.
...
me.onload = function()
	...
	setglobal(string.format("BINDING_HEADER_%s", mod.namespace), mod.global.name)
	
	-- to add a binding named <command>, run this line:
	-- setglobal(string.format("BINDING_NAME_%s_%s", mod.namespace, command), me.get("binding", command))
	
end

--[[
--------------------------------------------------------
				Localisation Reverse Lookup
--------------------------------------------------------
]]
	...
	for key, value in reversetree do
		...
		if localtable == nil then
			if mod.out.checktrace("warning", me, "lookup") then
				mod.out.printtrace(string.format("Can't complete the localisation reverse lookup because the strings for %s haven't been localised!", parentkey))
			end
...
mod.string.unlocalise(key1, key2, [key3, key4, key5])
	...
	The first few keys are entries in the me.reverselookup table, which specify which part of the localisation tree
the lookup will work on, i.e. are you looking for a spell, or a boss' name, etc. The last key is the value you have parsed from a combat log.
	...
	for x = 1, 5 do
		if subtable[me.stringkeys[x]] == nil then
			...
			if (x == 5) or (me.stringkeys[x + 1] == nil) then
				...
				-- error occur!
				if mod.out.checktrace("error", me, "unlocalise") then
...
mod.string.get(key1, key2, [key3, key4, key5])
	...
	Refer to enUS.lua for a list of all keys.
	...
	if stringvalue == nil then
		...
		if stringvalue == nil then
			...
			if mod.trace.check("error", me, "getstring") then
				mod.trace.print(string.format("The localisation identifier |cffffff00%s|r does not exist.", me.keysettostring()))
			end
			...
			if mod.out.checktrace("info", me, "get") then
...
me.testlocalisation = function(locale)
	...
	if me.data[locale] == nil then
		me.print(string.format("Sorry, there's no localisation at all for the |cffffff00%s|r locale.", locale))
		return
	end
...
me.testlocalisationbranch = function(english, mine, linkstring)
	...
	for key, value in english do
		...
		if mine[key] == nil then
			if type(value) == "table" then
				me.print(string.format("Missing the set of keys |cffffff00%s%s|r.", linkstring, key))
				errors = errors + 1
			else
				me.print(string.format("Missing the key |cffffff00%s%s|r. The english value is |cff3333ff%s|r", linkstring, key, value))
				errors = errors + 1
			end
	...
	for key, value in mine do
		...
		if english[key] == nil then
			if type(value) == "table" then
				me.print(string.format("The set of keys |cffffff00%s%s|r does not exist in the english version, so is probably no longer used.", linkstring, key))
				errors = errors + 1
			else
				me.print(string.format("The key |cffffff00%s%s|r does not exist in the english version, so is probably no longer used. The current value is |cff3333ff%s|r", linkstring, key, value))
				errors = errors + 1
			end
		end
	end
	...
	if linkstring == "" then
		me.print(string.format("Found |cffffff00%d|r errors in total.", errors))
	end
