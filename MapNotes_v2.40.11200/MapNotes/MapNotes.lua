...
function MapNotes_Hooker()
	...
	function MapNotes_ToggleWorldMapHook(...)
		...
		MapNotes_ToggleWorldMap(unpack(arg));
	end
	orig_ToggleWorldMap = ToggleWorldMap;
	ToggleWorldMap = MapNotes_ToggleWorldMapHook;

	-- CloseDropDownMenus
	function MapNotes_CloseDropDownMenusHook(...)
		orig_CloseDropDownMenus(unpack(arg));
		MapNotes_CloseDropDownMenus(unpack(arg));
	end
	orig_CloseDropDownMenus = CloseDropDownMenus;
	CloseDropDownMenus = MapNotes_CloseDropDownMenusHook;

	-- ToggleDropDownMenu
	function MapNotes_ToggleDropDownMenuHook(...)
		MapNotes_CloseDropDownMenus(unpack(arg));
		orig_ToggleDropDownMenu(unpack(arg));
	end
...
function MapNotes_CheckNearNotes(key, xPos, yPos)
	...
	for j, value in pairs(MapNotes_Data_Notes[key]) do
...
function MapNotes_ChatFrame_OnEvent(...)
			...
			if ( ( MapNotes_Keys[key] ) and ( MapNotes_Keys[key].miniData ) ) then
				...
				if ( ( currentConst ) and ( currentConst.scale ~= 0 ) ) then
					...
					if GetCurrentMapZone() > 0 then
						xscale = MapNotes_Const[GetCurrentMapContinent()][currentZoom].xscale;
						yscale = MapNotes_Const[GetCurrentMapContinent()][currentZoom].yscale;
					else
...
function MapNotes_GetZoneTableSize(zoneTable)
	...
	for index, records in pairs(zoneTable) do
		i = i + 1;
	end
...
function MapNotes_WorldMapButton_OnClick(...)
	...
	if ( ( mouseButton == "LeftButton" ) or ( ( mouseButton == "RightButton" ) and ( not IsControlKeyDown() ) and ( not IsShiftKeyDown() ) ) ) then --Telic_7
		orig_MapNotes_WorldMapButton_OnClick(unpack(arg));
		return;
	end
