...
local Plugin = Clique:NewModule("sageuf")
Plugin.fullname = "Sage Unit Frames"
Plugin.url = "http://www.wowinterface.com/downloads/fileinfo.php?id=4862"
...
function Plugin:OnEnable()
    self:Hook("SFrame_OnClick", "OnClick")
end

function Plugin:OnClick(button)
    local unit = this.id
    if not Clique:OnClick(button, unit) then
        return self.hooks.SFrame_OnClick.orig(button)
    end
end