...
local Plugin = core:NewModule("potionbuffsta")
Plugin.fullname = "Stamina Buff Items"

local PT = PeriodicTableEmbed:GetInstance("1")
-- register our custom item set w/ PT
PT:RegisterCustomSet({"scrollsta", "potionbuffsta"},"XADbuffsta")
...
function Plugin:ShowItem()
	local bag, slot = PT:GetBest("XADbuffsta")
...
function Plugin:ItemInList(itemlink)
	...
	return PT:ItemInSet(itemlink, "XADbuffsta")
end
