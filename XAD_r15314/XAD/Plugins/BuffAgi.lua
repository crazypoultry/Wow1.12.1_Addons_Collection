...
local Plugin = core:NewModule("buffagi")
Plugin.fullname = "Agility Buff Items"

local PT = PeriodicTableEmbed:GetInstance("1")

-- register our custom item set w/ PT
PT:RegisterCustomSet({"scrollagi", "potionbuffagi"},"XADbuffagi")
...
function Plugin:ShowItem()
	local bag, slot = PT:GetBest("XADbuffagi")
...
--	local itemlink = GetContainerItemLink(bag,slot)
	return PT:ItemInSet(itemlink, "XADbuffagi")
end
