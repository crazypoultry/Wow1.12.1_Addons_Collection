...
local Plugin = core:NewModule("buffstr")
Plugin.fullname = "Strength Buff Items"

local PT = PeriodicTableEmbed:GetInstance("1")
-- register our custom item set w/ PT
PT:RegisterCustomSet({"scrollstr", "potionbuffstr"},"XADbuffstr")
...
function Plugin:ShowItem()
	local bag, slot = PT:GetBest("XADbuffstr")
...
function Plugin:ItemInList(itemlink)
	...
	return PT:ItemInSet(itemlink, "XADbuffstr")
end
