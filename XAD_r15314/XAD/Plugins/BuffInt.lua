...
local Plugin = core:NewModule("buffint")
Plugin.fullname = "Int Buff Items"

local PT = PeriodicTableEmbed:GetInstance("1")
-- register our custom item set w/ PT
PT:RegisterCustomSet({"scrollint", "potionbuffint"},"XADbuffint")
...
function Plugin:ShowItem()
	local bag, slot = PT:GetBest("XADbuffint")
...
function Plugin:ItemInList(itemlink)
	...
	return PT:ItemInSet(itemlink, "XADbuffint")
end
