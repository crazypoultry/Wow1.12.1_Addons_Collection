...
local Plugin = core:NewModule("buffarmor")
Plugin.fullname = "Armor Buff Items"

local PT = PeriodicTableEmbed:GetInstance("1")
-- register our custom item set w/ PT
PT:RegisterCustomSet({"scrollprot", "potionbuffarmor"},"XADbuffarmor")
...
function Plugin:ShowItem()
	local bag, slot = PT:GetBest("XADbuffarmor")
...
function Plugin:ItemInList(itemlink)
	...
	return PT:ItemInSet(itemlink, "XADbuffarmor")
end
