...
local Plugin = core:NewModule("buffspr")
Plugin.fullname = "Spirit Buff Items"

local PT = PeriodicTableEmbed:GetInstance("1")
-- register our custom item set w/ PT
PT:RegisterCustomSet({"scrollspi", "potionbuffspr"},"XADbuffspr")
...
function Plugin:ShowItem()
	local bag, slot = PT:GetBest("XADbuffspr")
...
function Plugin:ItemInList(itemlink)
	...
	return PT:ItemInSet(itemlink, "XADbuffspr")
end
