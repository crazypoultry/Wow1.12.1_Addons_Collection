...
local Plugin = core:NewModule("poisoncrippling", tempweaponbuffs)
Plugin.fullname = "Poison Crippling"

local PT = PeriodicTableEmbed:GetInstance("1")
-- register our custom item set w/ PT
PT:RegisterCustomSet("3775:50 3776:70","XADpoisoncrippling")	
...
function Plugin:ShowItem()
	local bag, slot = PT:GetBest("XADpoisoncrippling")
...
function Plugin:ItemInList(itemlink, spellid)
	...
	return PT:ItemInSet(itemlink,"XADpoisoncrippling")
end
