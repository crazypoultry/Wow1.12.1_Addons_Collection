...
-- Function to be called for an active battleground
...
function BGBuddy_DisplayLines()
    ...
    baseHeight = baseHeight + ((current_line + 1) * 14) + (qcount * 14);
      
    if ( honorshown == true ) then
		baseHeight = baseHeight + 8;
        lc = lc - 6;
    else
	...
	if ( (BGBuddy_RankName:GetWidth()+63) > maxWidth and BGBuddy_SavedVars["config"]["displayRank"] == 1 ) then
		maxWidth = BGBuddy_RankName:GetWidth() + 63;
    else
        maxWidth = 180;
	end
    ...
    if qcount == 0 then
        ...
        if bactive == 0 then
            baseHeight = baseHeight - (qcount * 18);
            maxWidth = 180;
            elseif qcount == 0 then
                   baseHeight = baseHeight + (lc * 6);
                   maxWidth = 180;
            elseif qcount == 1 then
                   baseHeight = baseHeight + 36 + ((lc - 1) * 6);
                   maxWidth = 180;
            elseif qcount == 2 then
                   baseHeight = baseHeight + 36;
                   --baseHeight = baseHeight + 36 + ((lc - 26) * 6);
                   maxWidth = 180;
        end
    end
    
    BGBuddy_StandardFrame:SetWidth(maxWidth + 14);
    BGBuddy_StandardFrame:SetHeight(baseHeight);
    BGBuddy_Line1:SetText(display_text);
    --BGBuddy_Line1:SetText(lc);
end
...
--function BGBuddy_ManualJoin(index)
--	if( join == 1 ) then
--       join = 0;
--        BGBuddy_OverlayFrame_EnterButton:Hide();
--		AcceptBattlefieldPort(index, true);
