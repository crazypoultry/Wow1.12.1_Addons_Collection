...
local MINOR_VERSION = "$Revision: 7560 $"
...
function FuBarPlugin:GetTitle()
	...
	return (string.gsub(title, "|c%x%x%x%x%x%x%x%x(.-)|r", "%1"))
end
...
function FuBarPlugin:IsTextShown()
	return not self.hasNoText and (not self.db or not self.db.profile or self.db.profile.showText ~= 0)
end
...
function FuBarPlugin:IsIconShown()
	...
	elseif not self.db then
		return true
	elseif not self.db.profile.showIcon then
		return true
	else
...
function FuBarPlugin:ToggleIconShown()
	...
	self.db.profile.showIcon = value and 1 or 0
	if value then
...
function FuBarPlugin:IsTextShown()
	...
	elseif not self.db then
		return true
	elseif not self.db.profile.showText then
		return true
	else
...
function FuBarPlugin:ToggleTextShown()
	...
	self.db.profile.showText = value and 1 or 0
	if value then
		self.textFrame:Show()
		self:UpdateText()
	else
...
--	self:ScheduleEvent(FuBarPlugin.RegisterForLoad, 0, target)
end
...
function FuBarPlugin:OpenMenu(frame)
	...
	if not Dewdrop:IsRegistered(self:GetFrame()) then
		if type(self.OnMenuRequest) == "table" and (not self.OnMenuRequest.handler or self.OnMenuRequest.handler == self) and self.OnMenuRequest.type == "group" then
			...
			if self.OnMenuRequest.args and CheckFuBar() then
...
function FuBarPlugin:AddImpliedMenuOptions(level)
	...
	if not impliedMenuOptions[self] then
		...
		if impliedMenuOptions[self].args and CheckFuBar() then
...
function FuBarPlugin.OnEmbedEnable(FuBarPlugin, self)
	...
	if not self.hideWithoutStandby or not self.db.profile.hidden then
		FuBarPlugin:ScheduleEvent(self.Show, 0, self)
	end
		...
		if not recheckPlugins then
			...
			function recheckPlugins()
				for k,v in pairs(AceConsole.registry) do
					if type(v) == "table" and v.args and AceOO.inherits(v.handler, FuBarPlugin) then
						v.args.profile = nil
					end
				end
			end
...
function FuBarPlugin.GetAceOptionsDataTable(FuBarPlugin, self)
	return {
		icon = {
			...
			get = "IsIconShown",
			hidden = function()
				return not self.hasIcon or self.hasNoText or self:IsMinimapAttached()
			end,
			order = -13.7,
			handler = self,
		},
		text = {
			...
			get = "IsTextShown",
			hidden = function()
				return self.cannotHideText or not self.hasIcon or self.hasNoText or self:IsMinimapAttached()
			end,
			order = -13.6,
			handler = self,
		},
		colorText = {
			...
			get = "IsTextColored",
			hidden = function()
				return self.userDefinedFrame or self.hasNoText or self.hasNoColor or self:IsMinimapAttached()
			end,
			order = -13.5,
			handler = self,
		},
		...
		minimapAttach = {
			...
			hidden = function()
				return (self.cannotAttachToMinimap and not self:IsMinimapAttached()) or not CheckFuBar()
			end,
			order = -13.1,
			handler = self,
		},
		hide = {
			...
			func = "Hide",
			hidden = function()
				return not self.hideWithoutStandby
			end,
			order = -13,
			handler = self,
		},
...
function MinimapContainer:ReadjustLocation(plugin)
	...
	if not plugin.db.profile.minimapPositionWild then
		local position = plugin.db.profile.minimapPosition or plugin.defaultMinimapPosition or math.random(1, 360)
...
function MinimapContainer.OnUpdate()
	if not IsAltKeyDown() then
		...
		local lastPosition = this.plugin.db.profile.minimapPosition
