...
function aftt_buttonToggle(option)
	...
	elseif (option == "target") then
		if (AF_ToolTip[aftt_localUser]["Target"] == 1) then
			AF_ToolTip[aftt_localUser]["Target"] = 0;
		else
			AF_ToolTip[aftt_localUser]["Target"] = 1;
		end
...
function aftt_onRefresh()
	--Set our options frame refresh
		if (aftt_optionFrame:IsVisible()) then
			...
			aftt_TextLeftTarget:SetText("Target:");
			if (AF_ToolTip[aftt_localUser]["Target"] == 1) then
				aftt_TextLeftTarget:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRightTarget:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRightTarget:SetText("Shown");
			else
				aftt_TextLeftTarget:SetVertexColor(0.4, 0.4, 0.0);
				aftt_TextRightTarget:SetVertexColor(0.4, 0.4, 0.0);
				aftt_TextRightTarget:SetText("Hidden");
			end
...
function aftt_format(unit)
	...
	--Append Easter Eggs---------------------------------------
	-----------------------------------------------------------
		if (aftt_updateFlag["EasterEgg"] == 0 and UnitName(unit) == "Chickaaa") then
	...
	--Show/Hide Guild------------------------------------------
	-----------------------------------------------------------
		local hackname, hackrealm = UnitName(unit);
		local outname;
		...
		if (aftt_updateFlag["Guild"] == 0 and GetGuildInfo(unit)) then
			...
			elseif (AF_ToolTip[aftt_localUser]["Guild"] == 1) then
				...
				if (hackrealm) then
					outname = "<" .. GetGuildInfo(unit) .. "> - " .. hackrealm;
				else
					outname = "<" .. GetGuildInfo(unit) .. ">";
				end
				GameTooltipTextLeft2:SetText("|cff" .. aftt_color_guild .. outname .."|r");
			elseif (AF_ToolTip[aftt_localUser]["Guild"] == 2) then
				--Show at Bottom
				GameTooltip:AddLine("|cff" .. aftt_color_guild .. GetGuildInfo(unit) .. "|r");
			end
			aftt_updateFlag["Guild"] = 1;
		elseif ((aftt_updateFlag["Guild"] == 0) and not GetGuildInfo(unit) and (AF_ToolTip[aftt_localUser]["Guild"] == 1) and hackrealm)  then 
			aftt_old_line2 = GameTooltipTextLeft2:GetText();
			aftt_old_line3 = GameTooltipTextLeft3:GetText();
			if (GameTooltipTextLeft3:GetText() ~= nil) then
				GameTooltipTextLeft3:SetText(aftt_old_line2);
			...
			GameTooltipTextLeft2:SetText("|cff" .. aftt_color_description .. hackrealm .."|r");
			aftt_updateFlag["Guild"] = 1;
		end
	...
	--Show/Hide Target-----------------------------------------
	-----------------------------------------------------------
		-- DEFAULT_CHAT_FRAME:AddMessage("|cffffff00Target status = " .. aftt_updateFlag["Target"]);
		if ((aftt_updateFlag["Target"] == 0) and (AF_ToolTip[aftt_localUser]["Target"] == 1)) then
			local ptarget = UnitName("mouseovertarget");
			if(ptarget) then
				GameTooltip:AddLine("|cffffffffTarget: " .. ptarget .. "|r");
			end
			aftt_updateFlag["Target"] = 1;
		end
