...
-- Whine at the user to get the author to fix their externals
error("Please update this addon's externals, RosterLib-2.0 has moved to http://svn.wowace.com/root/trunk/RosterLib/RosterLib-2.0/")
...
3.	UpdateRoster() will create/update the roster. To see if everything worked fine,
	it will call CheckRoster() when it's done.
...
function RosterLib:CheckRoster()
...
--	print("RosterLib:UpdateRoster")
	...
	if Compost then
		for name in pairs(oldRoster) do
			Compost:Reclaim(oldRoster[name])
			oldRoster[name] = nil
		end
	else
		oldRoster = {}
...
function RosterLib:IterateRoster(pets)
	if not rosterAvailable then return nil end
	local key
	return function()
