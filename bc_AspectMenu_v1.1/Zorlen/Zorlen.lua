...
local Druid_FBN = 678
local Hunter_FBN = 681
local Mage_FBN = 681
local Paladin_FBN = 681
local Priest_FBN = 678
local Rogue_FBN = 680
local Shaman_FBN = 681
local Warlock_FBN = 681
local Warrior_FBN = 681

local Other_FBN = 678
local Pets_FBN = 678

local ImmuneMobList_FBN = 648
local XML_FBN = 681

local Localization_FBN = 681
...
function Zorlen_OnUpdate(arg1)
	...
	if TimerRunDown then
		if Zorlen_IsTimer("Casting_timer", nil, "InternalZorlenMiscTimer") then
			if Zorlen_GetTimer("Casting_timer", nil, "InternalZorlenMiscTimer") <= 0 then
				Zorlen_ClearTimer("Casting_timer", nil, "InternalZorlenMiscTimer")
				if Zorlen_Casting then
					Zorlen_debug("Casting Stop: from time out")
					Zorlen_SetTimer(1, Zorlen_CastingSpellName, nil, "InternalZorlenSpellCastDelay", 2)
					if Zorlen_CastingSpellTargetName then
						Zorlen_SpellTimerSet()
						Zorlen_SetTimer(1, Zorlen_CastingSpellName, Zorlen_CastingSpellTargetName, "InternalZorlenSpellCastDelay", 2)
					end
					Zorlen_Casting = nil
					Zorlen_CastingSpellRank = 0
		...
		if Zorlen_IsTimer("Channeling_timer", nil, "InternalZorlenMiscTimer") then
			if Zorlen_GetTimer("Channeling_timer", nil, "InternalZorlenMiscTimer") <= 0 then
				Zorlen_ClearTimer("Channeling_timer", nil, "InternalZorlenMiscTimer")
		...
		if Zorlen_IsTimer("InventoryScan_timer", nil, "InternalZorlenMiscTimer") then
			...
			if Zorlen_GetTimer("InventoryScan_timer", nil, "InternalZorlenMiscTimer") <= 0 then
				Zorlen_ClearTimer("InventoryScan_timer", nil, "InternalZorlenMiscTimer")
				if Zorlen_isCurrentClassWarlock then
		...
		if Zorlen_IsTimer("BagScan_timer", nil, "InternalZorlenMiscTimer") then
			...
			elseif Zorlen_GetTimer("BagScan_timer", nil, "InternalZorlenMiscTimer") <= 0 then
				Zorlen_ClearTimer("BagScan_timer", nil, "InternalZorlenMiscTimer")
				Zorlen_UpdateEatItemInfo()
				Zorlen_UpdateDrinkItemInfo()
			end
		end
		if Zorlen_IsTimer("ACTIONBAR_HIDEGRID_timer", nil, "InternalZorlenMiscTimer") then
			if Zorlen_GetTimer("ACTIONBAR_HIDEGRID_timer", nil, "InternalZorlenMiscTimer") <= 0 then
				Zorlen_ClearTimer("ACTIONBAR_HIDEGRID_timer", nil, "InternalZorlenMiscTimer")
		...
		if Zorlen_IsTimer("EnteringWorld", nil, "InternalZorlenMiscTimer") then
			if Zorlen_GetTimer("EnteringWorld", nil, "InternalZorlenMiscTimer") <= 0 then
				Zorlen_ClearTimer("EnteringWorld", nil, "InternalZorlenMiscTimer")
				if not ZorlenConfig[ZORLEN_ZPN][ZORLEN_FOODOFF] then
					Zorlen_UpdateEatItemInfo()
					Zorlen_UpdateDrinkItemInfo()
				end
	...
	if Zorlen_Warrior_OnUpdate(arg1, TimerRunDown) then
	elseif Zorlen_Hunter_OnUpdate(arg1, TimerRunDown) then
	end
	Zorlen_MovingUpdate()
end
...
function Zorlen_OnUpdate_TimerRunDown(arg1)
	local counter = 1
	local found = nil
	while Zorlen_TimerFullName[counter] do
		if Zorlen_TimerSeconds[Zorlen_TimerFullName[counter]] then
			found = 1
			Zorlen_TimerSeconds[Zorlen_TimerFullName[counter]] = Zorlen_TimerSeconds[Zorlen_TimerFullName[counter]] - arg1
			if not Zorlen_KeepTimer[counter] and Zorlen_TimerSeconds[Zorlen_TimerFullName[counter]] <= 0 then
				if Zorlen_TimerTimer[Zorlen_TimerFullName[counter]] then
					if Zorlen_DebugFlag then
						if Zorlen_TimerTag[counter] ~= "" and not string.find(Zorlen_TimerTag[counter], "InternalZorlen") then
							if Zorlen_TimerPre[counter] ~= "" then
								Zorlen_debug("Pre Timer Expired:  '"..Zorlen_TimerTag[counter].."'  '"..Zorlen_TimerPre[counter].."'  '"..Zorlen_TimerName[counter].."'", Zorlen_ShowTimer[counter])
							else
								Zorlen_debug("Pre Timer Expired:  '"..Zorlen_TimerTag[counter].."'  '"..Zorlen_TimerName[counter].."'", Zorlen_ShowTimer[counter])
							end
						else
							if Zorlen_TimerPre[counter] ~= "" then
								Zorlen_debug("Pre Timer Expired:  '"..Zorlen_TimerPre[counter].."'  '"..Zorlen_TimerName[counter].."'", Zorlen_ShowTimer[counter])
							else
								Zorlen_debug("Pre Timer Expired:  '"..Zorlen_TimerName[counter].."'", Zorlen_ShowTimer[counter])
							end
						end
					end
					local a = (Zorlen_TimerTimer[Zorlen_TimerFullName[counter]][1] - Zorlen_TimerTimer[Zorlen_TimerFullName[counter]][7])
					local b = Zorlen_TimerTimer[Zorlen_TimerFullName[counter]][2]
					local c = Zorlen_TimerTimer[Zorlen_TimerFullName[counter]][3]
					local d = Zorlen_TimerTimer[Zorlen_TimerFullName[counter]][4]
					local e = Zorlen_TimerTimer[Zorlen_TimerFullName[counter]][5]
					local f = Zorlen_TimerTimer[Zorlen_TimerFullName[counter]][6]
					Zorlen_TimerSeconds[Zorlen_TimerFullName[counter]] = nil
					if Zorlen_AllTimersAreExpired(counter) then
						Zorlen_WipeAllTimers(counter)
					else
						Zorlen_TimerTimer[Zorlen_TimerFullName[counter]] = nil
						Zorlen_TimerSlot[Zorlen_TimerFullName[counter]] = nil
					end
					...
					if Zorlen_DebugFlag then
						if Zorlen_TimerTag[counter] ~= "" and not string.find(Zorlen_TimerTag[counter], "InternalZorlen") then
							if Zorlen_TimerPre[counter] ~= "" then
								Zorlen_debug("Timer Expired:  '"..Zorlen_TimerTag[counter].."'  '"..Zorlen_TimerPre[counter].."'  '"..Zorlen_TimerName[counter].."'", Zorlen_ShowTimer[counter])
							else
								Zorlen_debug("Timer Expired:  '"..Zorlen_TimerTag[counter].."'  '"..Zorlen_TimerName[counter].."'", Zorlen_ShowTimer[counter])
							end
						else
							if Zorlen_TimerPre[counter] ~= "" then
								Zorlen_debug("Timer Expired:  '"..Zorlen_TimerPre[counter].."'  '"..Zorlen_TimerName[counter].."'", Zorlen_ShowTimer[counter])
							else
								Zorlen_debug("Timer Expired:  '"..Zorlen_TimerName[counter].."'", Zorlen_ShowTimer[counter])
							end
						end
					end
					Zorlen_TimerSeconds[Zorlen_TimerFullName[counter]] = nil
...
function Zorlen_AllTimersAreExpired(counter)
	...
	while Zorlen_TimerFullName[c] do
		if Zorlen_TimerSeconds[Zorlen_TimerFullName[c]] then
			return false, c
		end
		c = c + 1
	end
...
function Zorlen_WipeAllTimers(counter)
	local c = counter or 1
	while Zorlen_TimerFullName[c] do
		Zorlen_TimerSeconds[Zorlen_TimerFullName[c]] = nil
		Zorlen_TimerSlot[Zorlen_TimerFullName[c]] = nil
		Zorlen_TimerTimer[Zorlen_TimerFullName[c]] = nil
		Zorlen_TimerPre[c] = nil
		Zorlen_TimerName[c] = nil
		Zorlen_TimerTag[c] = nil
		Zorlen_KeepTimer[c] = nil
		Zorlen_ShowTimer[c] = nil
		Zorlen_TimerFullName[c] = nil
		c = c + 1
	end
end
...
function Zorlen_OnEvent(event, arg1, arg2, arg3)
	...
	elseif (event == "BAG_UPDATE") then
			...
			if not ZorlenConfig[ZORLEN_ZPN][ZORLEN_FOODOFF] then
				...
				Zorlen_SetTimer(1, "BagScan_timer", nil, "InternalZorlenMiscTimer", 2, 1)
			end
	...
	elseif (event == ("PLAYER_ENTERING_WORLD")) then
			...
			Zorlen_CheckVariables()
			Zorlen_SetTimer(2, "EnteringWorld", nil, "InternalZorlenMiscTimer", 2, 1)
	...
	elseif (event == ("UNIT_INVENTORY_CHANGED")) then
			...
			if not ZorlenConfig[ZORLEN_ZPN][ZORLEN_ITEMSCANOFF] and UnitIsUnit("player", arg1) then
				Zorlen_SetTimer(1, "InventoryScan_timer", nil, "InternalZorlenMiscTimer", 2, 1)
			end
	...
	elseif (event == ("ACTIONBAR_HIDEGRID")) then
			...
			Zorlen_SetTimer(1, "ACTIONBAR_HIDEGRID_timer", nil, "InternalZorlenMiscTimer", 2, 1)
	...
	elseif (event == "CHAT_MSG_SPELL_SELF_DAMAGE") then
			---------------------------
			local failed = nil
			local immune, _, S, T = string.find(arg1, LOCALIZATION_ZORLEN_immune)
			if immune then
				Zorlen_LastCastingSpellName = S
				Zorlen_LastCastingSpellTargetName = T
				if Zorlen_LastCastingSpellTargetIsMob and Zorlen_LastCastingSpellName and Zorlen_LastCastingSpellTargetName then
					...
					if not ZorlenConfig[ZORLEN_ZPN]["Immune"][Zorlen_LastCastingSpellName][Zorlen_LastCastingSpellTargetName] then
			...
			if not immune then
				local counter = 1
				while LOCALIZATION_ZORLEN_HitsOrCritsArray[counter] do
					if string.find(arg1, LOCALIZATION_ZORLEN_HitsOrCritsArray[counter]) then
						hit = counter
					end
					counter = counter + 1
				end
			end
			if immune or (Zorlen_LastCastingSpellName and not hit) then
				Zorlen_debug(arg1)
	...
	elseif (event == ("SPELLCAST_START")) then
			...
			if Zorlen_CastingSpellName and Zorlen_CastingSpellRank > 0 then
				Zorlen_debug("Casting Start:   "..Zorlen_CastingSpellName.."("..LOCALIZATION_ZORLEN_Rank.." "..Zorlen_CastingSpellRank..")")
			elseif Zorlen_CastingSpellName then
				Zorlen_debug("Casting Start:   "..Zorlen_CastingSpellName)
			else
				Zorlen_debug("Casting Start")
			end
			Zorlen_SetTimer(Zorlen_CastingSpellCastTime, "Casting_timer", nil, "InternalZorlenMiscTimer", 2, 1)
	...
	elseif (event == ("SPELLCAST_CHANNEL_START")) then
			...
			if Zorlen_ChannelingSpellName and Zorlen_ChannelingSpellRank then
				Zorlen_debug("Channeling Start:   "..Zorlen_ChannelingSpellName.."("..LOCALIZATION_ZORLEN_Rank.." "..Zorlen_ChannelingSpellRank..")")
			...
			Zorlen_SetTimer(ChannelingTime, "Channeling_timer", nil, "InternalZorlenMiscTimer", 2, 1)
	...
	elseif (event == ("SPELLCAST_DELAYED")) then
			...
			Zorlen_SetTimer(CastingTime, "Casting_timer", nil, "InternalZorlenMiscTimer", 2, 1)
	...
	elseif (event == ("SPELLCAST_STOP")) then
			...
			if Zorlen_Casting then
				...
				Zorlen_Casting = nil
			elseif not Zorlen_Channeling then
				if not Zorlen_OtherCastingStopDone then
					Zorlen_CastingSpellName = Zorlen_Temp_CastingSpellName
					if
						Zorlen_CastingSpellName == LOCALIZATION_ZORLEN_Attack
						or
						Zorlen_CastingSpellName == LOCALIZATION_ZORLEN_AutoShot
						or
						Zorlen_CastingSpellName == LOCALIZATION_ZORLEN_Shoot
						or
						Zorlen_CastingSpellName == LOCALIZATION_ZORLEN_Throw
						or
						Zorlen_CastingSpellName == LOCALIZATION_ZORLEN_ShootBow
						or
						Zorlen_CastingSpellName == LOCALIZATION_ZORLEN_ShootGun
						or
						Zorlen_CastingSpellName == LOCALIZATION_ZORLEN_ShootCrossbow
					then
						Zorlen_CastingSpellName = nil
					end
					if Zorlen_CastingSpellName then
						...
						if Zorlen_isCurrentClassHunter then
							Zorlen_debug(LOCALIZATION_ZORLEN_AutoShot)
...
function Zorlen_gsub(firststring, findstring1, replacestring1, findstring2, replacestring2)
	local finalstring = firststring or ""
	if findstring1 and replacestring1 then
		finalstring = string.gsub(finalstring, findstring1, replacestring1)
	end
	if findstring2 and replacestring2 then
		finalstring = string.gsub(finalstring, findstring2, replacestring2)
	end
	return finalstring
end

function Zorlen_gsub_immune(spellreplacestring, targetreplacestring)
	if spellreplacestring or targetreplacestring then
		if spellreplacestring and targetreplacestring then
			return Zorlen_gsub(LOCALIZATION_ZORLEN_immune, "%(%.%+%)", spellreplacestring, "%(%.%*%)", targetreplacestring)
		elseif spellreplacestring then
			return Zorlen_gsub(LOCALIZATION_ZORLEN_immune, "%(%.%+%)", spellreplacestring)
		end
		return Zorlen_gsub(LOCALIZATION_ZORLEN_immune, "%(%.%*%)", targetreplacestring)
	end
	return LOCALIZATION_ZORLEN_immune
end
...
function Zorlen_DebuffTimer_OnEvent_PLAYER_TARGET_CHANGED()
	local counter = 1
	while Zorlen_TimerFullName[counter] do
		if (Zorlen_TimerTag[counter] == "InternalZorlenSpellTimers" or Zorlen_TimerTag[counter] == "InternalZorlenSpellCastDelay") and Zorlen_TimerPre[counter] == "" then
			Zorlen_ClearTimer(Zorlen_TimerFullName[counter])
		end
		counter = counter + 1
	end
end
...
function Zorlen_DebuffTimer_OnEvent_CHAT_MSG_COMBAT_HOSTILE_DEATH(arg1, arg2, arg3)
	local counter = 1
	while Zorlen_TimerFullName[counter] do
		if Zorlen_TimerTag[counter] == "InternalZorlenSpellTimers" then
			if Zorlen_TimerPre[counter] ~= "" and string.find(arg1, Zorlen_gsub(LOCALIZATION_ZORLEN_EnemyDies, "%(%.%*%)", Zorlen_TimerPre[counter])) then
				if UnitName("target") ~= Zorlen_TimerPre[counter] or UnitHealth("target") == 0 then
					Zorlen_ClearTimer(Zorlen_TimerFullName[counter])
				end
			end
		end
...
function Zorlen_AllDebuffSlotsUsed(unit)
	local u = unit or "target"
	if UnitDebuff(u, Zorlen_MaxDebuffSlots) then
		return true
	end
	return false
end
...
function Zorlen_isBreakOnDamageCC(unit, dispelable)
	if
	Zorlen_checkDebuff("Spell_Nature_Polymorph", unit, dispelable, LOCALIZATION_ZORLEN_Polymorph) --Polymorph
	or
	Zorlen_checkDebuff("Spell_Nature_Sleep", unit, dispelable) --Sleep: Hibernate
	or
	Zorlen_checkDebuff("Spell_Nature_Slow", unit, dispelable, LOCALIZATION_ZORLEN_ShackleUndead) --Shackle Undead
	or
	Zorlen_checkDebuff("Ability_Sap", unit, dispelable, LOCALIZATION_ZORLEN_Sap) --Sap
	or
	Zorlen_checkDebuff("Spell_Shadow_MindSteal", unit, dispelable, LOCALIZATION_ZORLEN_Seduction) --Succubus Seduction
	or
	Zorlen_checkDebuff("Ability_GolemStormBolt", unit, dispelable, LOCALIZATION_ZORLEN_ScatterShot) --Scatter Shot
	or
	Zorlen_checkDebuff("Spell_Frost_ChainsOfIce", unit, dispelable, LOCALIZATION_ZORLEN_FreezingTrap) --Freezing Trap
	or
	Zorlen_checkDebuff("INV_Spear_02", unit, dispelable, LOCALIZATION_ZORLEN_WyvernSting, LOCALIZATION_ZORLEN_Asleep) --Sleep: Wyvern Sting
	then
		return true
	end
	return false
...
function Zorlen_isNoDamageCC(unit, dispelable)
	...
	or
	Zorlen_checkDebuff("Spell_Shadow_Cripple", unit, dispelable, LOCALIZATION_ZORLEN_Banish) --Banish
	--[[
	or
...
function Zorlen_isCrowedControlled(unit, dispelable)
	...
	or
	Zorlen_checkDebuff("Spell_Shadow_Possession", unit, dispelable, LOCALIZATION_ZORLEN_Fear) --Fear
	or
	Zorlen_checkDebuff("Spell_Shadow_DeathScream", unit, dispelable, LOCALIZATION_ZORLEN_HowlOfTerror) --Fear: Howl of Terror
	or
	Zorlen_checkDebuff("Spell_Shadow_PsychicScream", unit, dispelable, LOCALIZATION_ZORLEN_PsychicScream) --Fear: Psychic Scream
	or
...
function Zorlen_GiveGroupUnit(mode, showdebug, notunit, notunitarray, buff, notlowesthealth, dispelabledebuff, findbuff1, findbuff2)
	...
	while counter <= NumMembers do
		...
		if UnitExists(u) then
			h = UnitHealth(u)
			if (not notunit or not UnitIsUnit(notunit, u)) and (notlowesthealth or h / UnitHealthMax(u) < 0.85) and h > 0 and UnitIsVisible(u) and not UnitIsDeadOrGhost(u) and (not buff or not Zorlen_checkBuff(buff, u)) and (not dispelabledebuff or Zorlen_checkDebuff(nil, u, "dispelable")) and ((not findbuff1 and not findbuff2) or (findbuff1 and Zorlen_checkBuff(findbuff1, u)) or (findbuff2 and Zorlen_checkBuff(findbuff2, u))) then
				if PLAYER == "player" or UnitCreatureFamily(u) ~= LOCALIZATION_ZORLEN_Imp or not Zorlen_checkBuff("Spell_Shadow_ImpPhaseShift", u) then
...
function Zorlen_isMainHandFistWeapon()
	...
	if link then
		if Zorlen_GetItemSubType(link) == Zorlen_GetItemSubType(15905) then
...
function Zorlen_isFishingPoleEquipped()
	...
	local link = GetInventoryItemLink("player", slot)
	if link then
		if Zorlen_GetItemSubType(link) == Zorlen_GetItemSubType(6367) then
			if GetInventoryItemBroken("player", slot) then
...
function Zorlen_isStaveEquipped()
	...
	local link = GetInventoryItemLink("player", slot)
	if link then
		if Zorlen_GetItemSubType(link) == Zorlen_GetItemSubType(7713) then
			if GetInventoryItemBroken("player", slot) then
...
function Zorlen_isPolearmEquipped()
	...
	local link = GetInventoryItemLink("player", slot)
	if link then
		if Zorlen_GetItemSubType(link) == Zorlen_GetItemSubType(18725) then
			if GetInventoryItemBroken("player", slot) then
...
function Zorlen_isMainHandMace()
	...
	if link then
		...
		if itemType == Zorlen_GetItemSubType(9359) or itemType == Zorlen_GetItemSubType(6976) then
			if GetInventoryItemBroken("player", slot) then
...
function Zorlen_isMainHandAxe()
	...
	if link then
		local itemType = Zorlen_GetItemSubType(link)
		if itemType == Zorlen_GetItemSubType(18737) or itemType == Zorlen_GetItemSubType(6975) then
			if GetInventoryItemBroken("player", slot) then
...
function Zorlen_isMainHandSword()
	...
	if link then
		local itemType = Zorlen_GetItemSubType(link)
		if itemType == Zorlen_GetItemSubType(12940) or itemType == Zorlen_GetItemSubType(6977) then
			if GetInventoryItemBroken("player", slot) then
...
function Zorlen_isMainHandDagger()
	...
	local link = GetInventoryItemLink("player", slot)
	if link then
		if Zorlen_GetItemSubType(link) == Zorlen_GetItemSubType(13964) then
			if GetInventoryItemBroken("player", slot) then
...
function Zorlen_isShieldEquipped()
	...
	local link = GetInventoryItemLink("player", slot)
	if link then
		if Zorlen_GetItemSubType(link) == Zorlen_GetItemSubType(1168) then
			if GetInventoryItemBroken("player", slot) then
...
-- This is a test function and is no longer needed
						...
						if Zorlen_IsSpellKnown(SpellNameArray[j], SpellRankArray[j]) then
							...
							if not Zorlen_isCasting(SpellNameArray[j]) or ((SpellRankArray[j] - CurrentCastingSpellRank) > 1) or ((CurrentCastingSpellRank - SpellRankArray[j]) > 1) then
								if Zorlen_isCasting() then
									SpellStopCasting()
									Zorlen_debug("Stopping current cast to cast: "..SpellNameArray[j].."("..LOCALIZATION_ZORLEN_Rank.." "..SpellRankArray[j]..") on "..SpellTarget.." ("..UnitName(SpellTarget)..")")
								else
									if dotarget then
										TargetUnit(unit)
										retarget = 1
									...
									CastSpellByName(SpellNameArray[j].."("..LOCALIZATION_ZORLEN_Rank.." "..SpellRankArray[j]..")")
									Zorlen_debug("Casting: "..SpellNameArray[j].."("..LOCALIZATION_ZORLEN_Rank.." "..SpellRankArray[j]..") on "..SpellTarget.." ("..SpellTargetName..")")
								end
							end
							if (retarget) then
								TargetLastTarget()
					...
					elseif not SetName or not string.find(lefttext, "^%(%d+%) ") then
						while true do
							...
							if Healing then
									if bonusName then
										counter = 1
										while LOCALIZATION_ZORLEN_HealingBonusWordsArray[counter] do
											if LOCALIZATION_ZORLEN_HealingBonusWordsArray[counter] == bonusName then
										...
										counter = 1
										while LOCALIZATION_ZORLEN_HealingBonusPhraseArray[counter] do
											found, _, varplus = string.find(lefttext, LOCALIZATION_ZORLEN_HealingBonusPhraseArray[counter])
											if found then
							...
							if SpellDamage or Healing or ShadowDamage then
									if bonusName then
										counter = 1
										while LOCALIZATION_ZORLEN_SpellDamageAndHealingBonusWordsArray[counter] do
											if LOCALIZATION_ZORLEN_SpellDamageAndHealingBonusWordsArray[counter] == bonusName then
												minplus = tonumber(minplus)
												if maxplus then
													maxplus = tonumber(maxplus)
										...
										counter = 1
										while LOCALIZATION_ZORLEN_SpellDamageAndHealingBonusPhraseArray[counter] do
											found, _, varplus = string.find(lefttext, LOCALIZATION_ZORLEN_SpellDamageAndHealingBonusPhraseArray[counter])
											if found then
												varplus = tonumber(varplus)
							...
							if SpellDamage or ShadowDamage then
									if bonusName then
										counter = 1
										while LOCALIZATION_ZORLEN_SpellDamageBonusWordsArray[counter] do
											if LOCALIZATION_ZORLEN_SpellDamageBonusWordsArray[counter] == bonusName then
												minplus = tonumber(minplus)
												if maxplus then
													maxplus = tonumber(maxplus)
										...
										counter = 1
										while LOCALIZATION_ZORLEN_SpellDamageBonusPhraseArray[counter] do
											found, _, varplus = string.find(lefttext, LOCALIZATION_ZORLEN_SpellDamageBonusPhraseArray[counter])
											if found then
												varplus = tonumber(varplus)
										...
										if string.find(lefttext, "^"..LOCALIZATION_ZORLEN_BrilliantWizardOil) then
											...
											break
										elseif string.find(lefttext, "^"..LOCALIZATION_ZORLEN_WizardOil) then
											...
											break
										elseif string.find(lefttext, "^"..LOCALIZATION_ZORLEN_LesserWizardOil) then
											...
											break
										elseif string.find(lefttext, "^"..LOCALIZATION_ZORLEN_MinorWizardOil) then
							...
							if ShadowDamage then
									if bonusName then
											counter = 1
											while LOCALIZATION_ZORLEN_ShadowDamageBonusWordsArray[counter] do
												if LOCALIZATION_ZORLEN_HealingBonusWordsArray[counter] == bonusName then
													minplus = tonumber(minplus)
													if maxplus then
														maxplus = tonumber(maxplus)
										...
										counter = 1
										while LOCALIZATION_ZORLEN_ShadowDamageBonusPhraseArray[counter] do
											found, _, varplus = string.find(lefttext, LOCALIZATION_ZORLEN_ShadowDamageBonusPhraseArray[counter])
											if found then
												varplus = tonumber(varplus)
							...
							if PlayerLevel ~= Zorlen_MaxPlayerLevel then
								...
								for line=2,Lines do
									...
									if lefttext then
										...
										if string.find(lefttext, LOCALIZATION_ZORLEN_RequiresLevel) then
...
function Zorlen_UpdateEatItemInfo(MaxItems)
	...
	if not MaxItems and Zorlen_isCurrentClassMage and Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_ConjureFood) then
		Max = 7
	end
...
function Zorlen_UpdateDrinkItemInfo(MaxItems)
	...
	then
		local Max = MaxItems or 15
		if not MaxItems and Zorlen_isCurrentClassMage and Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_ConjureWater) then
			Max = 7
		end
...
function Zorlen_GetEatSlotNumber(MaxItems)
	local Max = MaxItems or 15
	local Start = 1
	if not Zorlen_GiveContainerItemSlotNumberByName(LOCALIZATION_ZORLEN_Conjured, 1, LOCALIZATION_ZORLEN_Water) then
		if (Max < 8) then
			Start = nil
		else
			Start = 8
	...
	local b = nil
	local s = nil
	local notname = LOCALIZATION_ZORLEN_well_fed
	local level = nil
	local fullcount = nil
	if Start then
...
function Zorlen_GetDrinkSlotNumber(MaxItems)
	local Max = MaxItems or 15
	local Start = 1
	if not Zorlen_GiveContainerItemSlotNumberByName(LOCALIZATION_ZORLEN_AllConjuredWater, 1) then
		if (Max <= 7) then
			Start = nil
		else
			Start = 8
...
function Zorlen_Eat()
	if Zorlen_notInCombat() and UnitHealth("player") / UnitHealthMax("player") <= 0.9 and not isEatingActive() then
		...
		if Zorlen_EatBagID then
			...
			local MaxItems = 15
			if Zorlen_isCurrentClassMage and Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_ConjureFood) then
				MageSpellKnown = 1
				MaxItems = 7
			end
			...
			elseif MageSpellKnown and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ConjureFood) then
				CastSpellByName(LOCALIZATION_ZORLEN_ConjureFood)
				return true
			end
		end
	end
...
function Zorlen_Drink()
	if Zorlen_notInCombat() and UnitPowerType("player") == 0 and UnitMana("player") / UnitManaMax("player") <= 0.9 and not isDrinkingActive() then
		...
		if Zorlen_DrinkBagID then
			local MageSpellKnown = nil
			local MaxItems = 15
			if Zorlen_isCurrentClassMage and Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_ConjureWater) then
				MageSpellKnown = 1
				MaxItems = 7
			end
			...
			elseif MageSpellKnown and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ConjureWater) then
				CastSpellByName(LOCALIZATION_ZORLEN_ConjureWater)
				return true
			end
		end
	end
...
function Zorlen_MakeEatMacro(show)
	return Zorlen_MakeMacro(LOCALIZATION_ZORLEN_EatMacroName, "/zorlen eat", 0, "Spell_Misc_Food", nil, 1, show)
end

function Zorlen_MakeDrinkMacro(show)
	return Zorlen_MakeMacro(LOCALIZATION_ZORLEN_DrinkMacroName, "/zorlen drink", 0, "Spell_Misc_Drink", nil, 1, show)
end
...
function Zorlen_CastCommonRegisteredSpell(SpellRank, SpellName, DebuffName, DebuffImmune, ManaNeeded, SelfHealthGreaterThanPercent, EnemyTargetNotNeeded, BuffName, TargetThatUsesManaNeeded, DebuffCheckIncluded, DebuffCheck, BuffCheckIncluded, BuffCheck, SpellCheckNotNeeded, BuffUnit, DoDebuffIncluded, DoDebuff, DoBuffIncluded, DoBuff, StopCasting, NoRangeCheck, test, DebuffTimer)
	if not SpellName then
		return false
	end
	local SB = Zorlen_Button[SpellName]
	if SpellRank and SpellName then
		SB = Zorlen_Button[SpellName.."."..SpellRank]
	end
	local SpellID = Zorlen_CheckIfCommonRegisteredSpellCastable(SpellName, SB, DebuffName, DebuffImmune, ManaNeeded, SelfHealthGreaterThanPercent, EnemyTargetNotNeeded, BuffName, TargetThatUsesManaNeeded, DebuffCheckIncluded, DebuffCheck, BuffCheckIncluded, BuffCheck, SpellCheckNotNeeded, BuffUnit, DoDebuffIncluded, DoDebuff, DoBuffIncluded, DoBuff, StopCasting, NoRangeCheck, DebuffTimer)
	if SpellID then
		if not test then
			if StopCasting and Zorlen_isCasting() then
				SpellStopCasting()
			else
				if SB then
					UseAction(SB)
				elseif SpellRank and SpellRank <= Zorlen_GetSpellRank(SpellName) then
					CastSpellByName(SpellName.."("..LOCALIZATION_ZORLEN_Rank.." "..SpellRank..")")
				else
					CastSpell(SpellID, 0)
				end
			end
...
function Zorlen_CheckIfCommonRegisteredSpellCastable(SpellName, SpellButton, DebuffName, DebuffImmune, ManaNeeded, SelfHealthGreaterThanPercent, EnemyTargetNotNeeded, BuffName, TargetThatUsesManaNeeded, DebuffCheckIncluded, DebuffCheck, BuffCheckIncluded, BuffCheck, SpellCheckNotNeeded, BuffUnit, DoDebuffIncluded, DoDebuff, DoBuffIncluded, DoBuff, StopCasting, NoRangeCheck, DebuffTimer)
	local _ = nil
	local isUsable = nil
	...
	if DebuffImmune or Zorlen_isImmune(SpellName, unitname) then
	...
	elseif SpellButton then
		...
		hasRange = ActionHasRange(SpellButton)
		inRange = IsActionInRange(SpellButton)
		isCurrent = IsCurrentAction(SpellButton)
	elseif SpellCheckNotNeeded or Zorlen_IsSpellKnown(SpellName) then
		if SpellName then
	...
	if not Zorlen_isCasting() or StopCasting then
		if not Zorlen_isChanneling(SpellName) and (SpellButton or not SpellIsTargeting() or StopCasting) then
			if not SpellButton or ( isUsable == 1 and not notEnoughMana and duration == 0 and (NoRangeCheck or not hasRange or inRange == 1) and (isCurrent ~= 1 or StopCasting) ) then
				if SpellButton or Zorlen_checkCooldown(SpellID) then
					if SpellButton or (not ManaNeeded or UnitMana("player") >= ManaNeeded) then
						if not TargetThatUsesManaNeeded or UnitPowerType("target") == 0 then
							if not SelfHealthGreaterThanPercent or Zorlen_HealthPercent("player") > SelfHealthGreaterThanPercent then
								if EnemyTargetNotNeeded or Zorlen_isEnemy() then
									if (not DoDebuffIncluded and not DebuffName and not DebuffCheckIncluded and not DebuffTimer) or (DoDebuffIncluded and DoDebuff) or (not Zorlen_AllDebuffSlotsUsed() and not Zorlen_IsTimer(SpellName, nil, "InternalZorlenSpellCastDelay") and ((DebuffName and not Zorlen_checkDebuffByName(DebuffName)) or (DebuffCheckIncluded and not DebuffCheck) or (DebuffTimer and not Zorlen_IsTimer(SpellName, unitname, "InternalZorlenSpellTimers")))) then
										if (not DoBuffIncluded and not BuffName and not BuffCheckIncluded) or (DoBuffIncluded and DoBuff) or (not Zorlen_IsTimer(SpellName, unitname, "InternalZorlenSpellCastDelay") and ((BuffName and not Zorlen_checkBuffByName(BuffName, BuffUnit)) or (BuffCheckIncluded and not BuffCheck))) then
											if SpellButton then
												return SpellButton
											else
												return SpellID
	...
	elseif Zorlen_isCasting(SpellName) then
		if (not DoDebuffIncluded and not DebuffName and not DebuffCheckIncluded and not DebuffTimer) or (DoDebuffIncluded and DoDebuff) or (not Zorlen_AllDebuffSlotsUsed() and ((not DebuffTimer and DebuffName and not Zorlen_checkDebuffByName(DebuffName)) or (not DebuffTimer and DebuffCheckIncluded and not DebuffCheck) or (DebuffTimer and not Zorlen_IsTimer(SpellName, unitname, "InternalZorlenSpellTimers")))) then
			if (not DoBuffIncluded and not BuffName and not BuffCheckIncluded) or (DoBuffIncluded and DoBuff) or ((BuffName and not Zorlen_checkBuffByName(BuffName, BuffUnit)) or (BuffCheckIncluded and not BuffCheck)) then
			elseif DoBuffIncluded or BuffName or BuffCheckIncluded then
				SpellStopCasting()
			end
		elseif DoDebuffIncluded or DebuffName or DebuffCheckIncluded or DebuffTimer then
			SpellStopCasting()
		end
	end
	return nil
...
function castSpellName(SpellRank, test)
	...
	local DebuffImmune = 
	local ManaNeeded = 
	local SelfHealthGreaterThanPercent = 
	local EnemyTargetNotNeeded = 
	local BuffName = 
	local TargetThatUsesManaNeeded = 
	local DebuffCheckIncluded = 
	local DebuffCheck = 
	local BuffCheckIncluded = 
	local BuffCheck = 
	local SpellCheckNotNeeded = 
	local BuffUnit = 
	local DoDebuffIncluded = 
	local DoDebuff = 
	local DoBuffIncluded = 
	local DoBuff = 
	local StopCasting = 
	local NoRangeCheck = 
	local DebuffTimer = 
...
function Zorlen_SetTimer(Seconds, Name, Pre, Tag, show, keep, TimerTimer)
	local Sec = TimerTimer or Seconds
	local keep = keep
	if Name and Name ~= "" and Sec and Sec > 0 then
		local tt = ""
		local TT = nil
		if TimerTimer then
			if TimerTimer > 0 and TimerTimer < Seconds then
				TT = {Seconds, Name, Pre, Tag, show, keep, TimerTimer}
				tt = "TT"
				keep = nil
			else
				return
		...
		local counter = Zorlen_TimerSlot[fn] or 1
		if not Zorlen_TimerSlot[fn] then
			while Zorlen_TimerFullName[counter] and Zorlen_TimerSeconds[Zorlen_TimerFullName[counter]] do
				counter = counter + 1
			end
			if Zorlen_TimerFullName[counter] then
				Zorlen_TimerSeconds[Zorlen_TimerFullName[counter]] = nil
				Zorlen_TimerSlot[Zorlen_TimerFullName[counter]] = nil
				Zorlen_TimerTimer[Zorlen_TimerFullName[counter]] = nil
			end
		end
		Zorlen_TimerSeconds[fn] = Sec
		Zorlen_TimerSlot[fn] = counter
		Zorlen_TimerTimer[fn] = TT
		Zorlen_TimerName[counter] = Name
		Zorlen_TimerPre[counter] = p
		Zorlen_TimerTag[counter] = t
		Zorlen_TimerFullName[counter] = fn
		Zorlen_KeepTimer[counter] = keep
		Zorlen_ShowTimer[counter] = show
		if Zorlen_DebugFlag then
			if TimerTimer then
				if t ~= "" and not string.find(t, "InternalZorlen") then
					if p ~= "" then
						Zorlen_debug("Pre Timer Set in Slot#"..counter..":  '"..t.."'  '"..p.."'  '"..Name.."'  "..Seconds.." Second(s).", Zorlen_ShowTimer[counter])
					else
						Zorlen_debug("Pre Timer Set in Slot#"..counter..":  '"..t.."'  '"..Name.."'  "..Seconds.." Second(s).", Zorlen_ShowTimer[counter])
					end
				else
					if p ~= "" then
						Zorlen_debug("Pre Timer Set in Slot#"..counter..":  '"..p.."'  '"..Name.."'  "..Seconds.." Second(s).", Zorlen_ShowTimer[counter])
					else
						Zorlen_debug("Pre Timer Set in Slot#"..counter..":  '"..Name.."'  "..Seconds.." Second(s).", Zorlen_ShowTimer[counter])
					end
				end
			else
				if t ~= "" and not string.find(t, "InternalZorlen") then
					if p ~= "" then
						Zorlen_debug("Timer Set in Slot#"..counter..":  '"..t.."'  '"..p.."'  '"..Name.."'  "..Seconds.." Second(s).", Zorlen_ShowTimer[counter])
					else
						Zorlen_debug("Timer Set in Slot#"..counter..":  '"..t.."'  '"..Name.."'  "..Seconds.." Second(s).", Zorlen_ShowTimer[counter])
					end
				else
					if p ~= "" then
						Zorlen_debug("Timer Set in Slot#"..counter..":  '"..p.."'  '"..Name.."'  "..Seconds.." Second(s).", Zorlen_ShowTimer[counter])
					else
						Zorlen_debug("Timer Set in Slot#"..counter..":  '"..Name.."'  "..Seconds.." Second(s).", Zorlen_ShowTimer[counter])
					end
				end
			end
		end
...
function Zorlen_IsTimer(Name, Pre, Tag)
	if Name and Name ~= "" then
		if Zorlen_TimerSeconds[Name] then
			return true
		end
		local p = Pre or ""
		local t = Tag or ""
		local fn = p.."___Pre_DEVIDER_123__"..Name.."___Tag_DEVIDER_456__"..t.."___Timer_Timer_DEVIDER_789__"
		if Zorlen_TimerSeconds[fn] or Zorlen_TimerSeconds[fn.."TT"] then
			return true
		end
	end
	return false
...
function Zorlen_GetTimer(Name, Pre, Tag)
	if Name and Name ~= "" then
		if Zorlen_TimerSeconds[Name] and Zorlen_TimerSeconds[Name] > 0 then
			return Zorlen_TimerSeconds[Name]
		end
		local p = Pre or ""
		local t = Tag or ""
		local fn = p.."___Pre_DEVIDER_123__"..Name.."___Tag_DEVIDER_456__"..t.."___Timer_Timer_DEVIDER_789__"
		if Zorlen_TimerSeconds[fn.."TT"] then
			if Zorlen_TimerTimer[fn.."TT"] then
				local n = (Zorlen_TimerTimer[fn.."TT"][1] - Zorlen_TimerTimer[fn.."TT"][7]) + Zorlen_TimerSeconds[fn.."TT"]
				return n
			end
		elseif Zorlen_TimerSeconds[fn] and Zorlen_TimerSeconds[fn] > 0 then
			return Zorlen_TimerSeconds[fn]
		end
	end
	return 0
end

function Zorlen_ClearTimer(Name, Pre, Tag, TimerTimer)
	if Name and Name ~= "" then
		local p = Pre or ""
		local t = Tag or ""
		local n = Name
		...
		local s = Zorlen_TimerSlot[fn]
		if Zorlen_TimerSeconds[n] then
			s = Zorlen_TimerSlot[n]
			p = Zorlen_TimerPre[s]
			t = Zorlen_TimerTag[s]
			n = Zorlen_TimerName[s]
			fn = Zorlen_TimerFullName[s]
		end
		if Zorlen_TimerSeconds[fn] then
			Zorlen_TimerSeconds[fn] = nil
			if Zorlen_DebugFlag then
				if Zorlen_TimerTimer[fn] then
					if t ~= "" and not string.find(t, "InternalZorlen") then
						if p ~= "" then
							Zorlen_debug("Pre Timer Cleared:  '"..t.."'  '"..p.."'  '"..n.."'", Zorlen_ShowTimer[s])
						else
							Zorlen_debug("Pre Timer Cleared:  '"..t.."'  '"..n.."'", Zorlen_ShowTimer[s])
						end
					else
						if p ~= "" then
							Zorlen_debug("Pre Timer Cleared:  '"..p.."'  '"..n.."'", Zorlen_ShowTimer[s])
						else
							Zorlen_debug("Pre Timer Cleared:  '"..n.."'", Zorlen_ShowTimer[s])
						end
					end
				else
					if t ~= "" and not string.find(t, "InternalZorlen") then
						if p ~= "" then
							Zorlen_debug("Timer Cleared:  '"..t.."'  '"..p.."'  '"..n.."'", Zorlen_ShowTimer[s])
						else
							Zorlen_debug("Timer Cleared:  '"..t.."'  '"..n.."'", Zorlen_ShowTimer[s])
						end
					else
						if p ~= "" then
							Zorlen_debug("Timer Cleared:  '"..p.."'  '"..n.."'", Zorlen_ShowTimer[s])
						else
							Zorlen_debug("Timer Cleared:  '"..n.."'", Zorlen_ShowTimer[s])
						end
					end
				end
			end
...
function Zorlen_Hook_CastSpellByName(Spell, onSelf)
	if Spell then
		...
		_, _, Zorlen_Temp_CastingSpellName = string.find(s, "^%s*(.*)$")
		_, _, i, r = string.find(Zorlen_Temp_CastingSpellName, "(.*)%(.*(%d)%)$")
		if (i and r) then
			Zorlen_Temp_CastingSpellName = i
			Zorlen_Temp_CastingSpellRank = tonumber(r)
		else
			Zorlen_Temp_CastingSpellRank = Zorlen_GetSpellRank(Zorlen_Temp_CastingSpellName)
		end
		if onSelf or not Zorlen_isEnemy() then
			Zorlen_Temp_CastingSpellTargetName = nil
			Zorlen_Temp_CastingSpellTargetIsMob = nil
		else
...
function Zorlen_Hook_UseAction(slot, checkCursor, onSelf)
	...
	if SpellName then
		...
		if onSelf == 1 or not Zorlen_isEnemy() then
			Zorlen_Temp_CastingSpellTargetName = nil
			Zorlen_Temp_CastingSpellTargetIsMob = nil
		else
			Zorlen_Temp_CastingSpellTargetName = UnitName("target")
...
function Zorlen_inBG()
	...
	if t == LOCALIZATION_ZORLEN_WarsongGulch or t == LOCALIZATION_ZORLEN_AlteracValley or t == LOCALIZATION_ZORLEN_ArathiBasin then
		return true
	end
	return false
end
