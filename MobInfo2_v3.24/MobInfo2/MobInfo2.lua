...
function MI2_DeleteAllMobData()
	MobInfoDB = { ["DatabaseVersion:0"] = { ver = MI2_DB_VERSION, loc=GetLocale(), sv=MI2_DB_SV } }
...
-----------------------------------------------------------------------------
 function MI2_InitOptions()
	...
	if  not MobInfoConfig.ShowItemPrice	then  MobInfoConfig.ShowItemPrice = 0	end
...
function MI2_GetLootItemString( itemID )
	...
	if quality then color = MI2_QualityColor[quality] end
...
local function MI2_AddResistToTooltip( resistData )
	...
	if immunities ~= "" then
		GameTooltip:AddLine( mifontGold..MI_TXT_IMMUNE..mifontWhite..immunities )
	end
...
local function MI2_AddItemPriceToTooltip()
	...
	if itemID and amount then
	    ...
	    if price > 0 then
			GameTooltip:AddDoubleLine( mifontLightBlue..MI_TXT_PRICE..mifontWhite..amount, mifontWhite..copper2text(price) )
			priceFound = true
	    end
	end

if MI2_DebugItems > 0 then
GameTooltip:AddLine( mifontSubWhite.."[frame="..frame:GetName()..",id="..frame:GetID().."]" )
GameTooltip:AddLine( mifontSubWhite.."[parent="..parent:GetName()..",id="..parent:GetID().."]" )
GameTooltip:AddLine( mifontSubWhite.."[bagId="..bagId..",bagSlot="..bagSlot.."]" )
GameTooltip:AddLine( mifontSubWhite.."link="..link )
GameTooltip:AddLine( mifontSubWhite.."[itemID="..itemID..",price="..price..",amount="..amount.."]" )
end
