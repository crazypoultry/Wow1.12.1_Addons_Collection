...
function GEM_IsPlayerInGroup(pl_name)
  ...
  then
    ...
    local party_count = GetNumPartyMembers();
    for i = 1,party_count
    do
...
StaticPopupDialogs["GEM_CONFIRM_IGNORE"] = {
  ...
  OnAccept = function()
    ...
    then
      ...
      GEM_EVT_ClearEvent(event.id,"Ignored",false);
...
function GEMList_EventOnClick(button)
  if(button == "RightButton" and selectEvent)
  then
    ToggleDropDownMenu(1,nil,GEMListMenu,"cursor");
  else
...
function GEMList_GetEvent()
  ...
  then 
    UIEvents = {};
    local MustFilter = GEM_Events.realms[GEM_Realm].my_names[GEM_PlayerName].FilterChannel;
    ...
    do
      if((GEM_Events.realms[GEM_Realm].ignore[event.id] == nil) and -- Not in ignore list
         ...
         (not MustFilter or GEM_IsChannelInList(event.channel))-- Check for filter
      )
      then
...
function GEMListMenu_OnLoad()
  HideDropDownMenu(1);
  GEMListMenu.initialize = GEMListMenu_Initialize;
  GEMListMenu.displayMode = "MENU";
  GEMListMenu.name = "Titre";
end

function GEMListMenu_Initialize()
  local event = GEM_Events.realms[GEM_Realm].events[selectEvent];

  info = { };
  info.text =  "|c004040FF"..event.leader.."|r : "..event.ev_place;
  info.isTitle = true;
  info.notCheckable = 1;
  UIDropDownMenu_AddButton(info,1);
  ...
  info.value = event.leader;
  info.func = GEMListAdminMenu_GroupInvite_OnClick;
  if(GEM_IsMyReroll(event.leader) or GEM_IsPlayerInGroup(event.leader))
  then
    info.disabled = 1;
  end
  UIDropDownMenu_AddButton(info,1);
  ...
  info = { };
  info.text =  GEM_TEXT_BAN_PLAYER;
  info.notCheckable = 1;
  info.value = event.leader;
  info.func = GEMList_BanOnClick;
  UIDropDownMenu_AddButton(info,1);
...
StaticPopupDialogs["GEM_CONFIRM_IGNORE_PLAYER"] = {
  text = TEXT(GEM_TEXT_IGNORE_PLAYER_CONFIRM),
  button1 = TEXT(OKAY),
  button2 = TEXT(CANCEL),
  OnAccept = function()
    local pl_name = this:GetParent().data.pl_name;
    GEM_Events.realms[GEM_Realm].ignore_players[pl_name] = true;
    for ev_id,event in GEM_Events.realms[GEM_Realm].events
    do
      if(event.leader == pl_name)
      then
        GEM_ChatPrint("Ignoring EventId '"..ev_id.."' from '"..event.leader.."' at '"..event.ev_place.."' because of ban on player "..pl_name);
        GEM_EVT_ClearEvent(ev_id,"Ignored",true);
...
function GEMList_BanOnClick()
  local event = GEM_Events.realms[GEM_Realm].events[selectEvent];
  if(event)
  then
    local dialogFrame = StaticPopup_Show("GEM_CONFIRM_IGNORE_PLAYER");
    if(dialogFrame)
    then
      dialogFrame.data = { pl_name=event.leader };
    end
  end
end
