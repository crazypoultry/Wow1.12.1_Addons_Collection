...
GEM_COM_ScheduledTaskInterval = 1.500;

GEM_ADDON_PREFIX = "GEM2";
...
local function _GEM_COM_SendChatMessage(Msg,ChanId)
  if(ChanId == -1) -- Guild message
  then
    SendAddonMessage(GEM_ADDON_PREFIX,Msg,"GUILD");
  else
    _GEM_COM_newSendChatMessage(Msg,"CHANNEL",nil,ChanId);
  end
end

local function _GEM_COM_ScheduledPlayerInfos()
  if(not GEM_Paused and not GEM_AFK_Mode and (_GEM_COM_LastPlayerInfosSend + _GEM_COM_PlayersInfosInterval < time()))
  then
...
local function _GEM_COM_ScheduledSendRoutine()
  ...
  do
    ...
    do
      ...
      then
        -- Reschedule routine
        GEMSystem_Schedule(GEM_COM_ScheduledTaskInterval,_GEM_COM_ScheduledSendRoutine);
        return;
      end
      -- Re-read params
      local channel = _GEM_COM_QueuedMessages[1].channel;
    ...
    -- DEBUG
    if(GEM_Events.debug and GEM_Events.debug ~= 0)
    then
      if(GEM_GetChannelNameOrGuild(channel) ~= GEM_COM_Channels[channel].id)
      then
        if(GEM_GetChannelNameOrGuild(channel) ~= 0)
        then
          GEM_ChatWarning("_GEM_COM_ScheduledSendRoutine : CurrentChanID differs from saved one : "..GEM_GetChannelNameOrGuild(channel).." - "..GEM_COM_Channels[channel].id);
        else
    ...
    do
      ...
      _GEM_COM_SendChatMessage(submsg,GEM_COM_Channels[channel].id);
      msg_sent = msg_sent + 1;
    ...
    _GEM_MoreCounter = _GEM_MoreCounter + 1;
      _GEM_COM_SendChatMessage(msg,GEM_COM_Channels[channel].id);
    msg_sent = msg_sent + 1;
...
function GEM_COM_ParseChannelMessage(channel,from,message)
  ...
  then
    if(params[1] and GEM_Events.realms[GEM_Realm].ignore_players[params[1]])
    then
      GEM_ChatDebug(GEM_DEBUG_PROTOCOL,"Player "..params[1].." is in ignore list, discarding message");
      return;
    end
...
local function _GEM_COM_Process_EventUpdate(channel,from,stamp,ev_id,params)
  ...
  if(params[2] and GEM_Events.realms[GEM_Realm].ignore_players[params[2]])
  then
    GEM_ChatDebug(GEM_DEBUG_PROTOCOL,"Player "..params[2].." is in ignore list, discarding EventID "..ev_id);
    return;
  end
...
local function _GEM_COM_Process_Global_PlayerInfos(channel,from,stamp,ev_id,params)
  ...
  if(pl_level == nil)
  then
    GEM_ChatWarning("_GEM_COM_Process_Global_PlayerInfos : pl_level sent by '"..tostring(from).."' is nil !");
    return;
  end
