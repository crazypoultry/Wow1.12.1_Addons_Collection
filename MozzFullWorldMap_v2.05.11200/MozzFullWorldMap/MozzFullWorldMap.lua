...
Update for patch 1.10 by Shub 20060329

NUM_WORLDMAP_OVERLAYS and overlay textures now created dynamically in Blizzard's
WorldMapFrame_Update(), updated worldMapFrame_UpdateOverlays() to reflect changes. Hack of
GetNumMapOverlays() and WorldMapFrame_Update() updated as well.

Version 1.02 by Shub 20060330

Minor bugfix.

Version 1.10 by Shub 20060402
...
SEE ReadMe.txt for latest Patch Notes
    ...
    for i,oinfo in next,zoneTable do
        ...
        if MozzFullWorldMap.Enabled or discovered[textureName] then
            ...
            if ( neededTextures > NUM_WORLDMAP_OVERLAYS ) then
				...
				if(MOZZ_DEBUG) then echo("Creating texture for: "..textureName) end
            ...
            for j=1, numTexturesTall do
                ...
                for k=1, numTexturesWide do
                    ...
                    if(MOZZ_DEBUG) then echo("Calling GGWMO with textureCount "..textureCount+1) end
                    ...
                    if discovered[textureName] then
                        ...
                        if(MozzFullWorldMap.transparency >= 0.0 and MozzFullWorldMap.transparency <= 1.0) then
...
function MozzFullWorldMap_Command(msg)
    ...
    if(msg == "blue") then
		MozzFullWorldMap.colorStyle = 0;
		Print("MozzFullWorldMap: unexplored areas will now be blue.");
	elseif(msg == "normal") then
		MozzFullWorldMap.colorStyle = 1;
		Print("MozzFullWorldMap: unexplored areas will now be normal.");
	elseif(string.find(cmd, "^trans [01]%.[0-9]+$")) then
		...
		if(trans >= 0.0 and trans <= 1.0) then
			...
			Print("MozzFullWorldMap: transparency set to "..trans)
			else Print("Please enter between 0.0 and 1.0 for transparency.")
		end
	else
		Print("MozzFullWorldMap Commands:");
		Print("blue: shows unexplored areas in blue");
		Print("normal: shows unexplored areas in the normal map color");
		Print("trans [0.0 to 1.0]: controls transparency of unexplored areas. 0.0 is completely clear, 1.0 is completely opaque.");
	end
end
