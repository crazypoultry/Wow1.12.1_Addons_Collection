...
function TitanPanelTimersButton_GetButtonText(id)
	...
	if time == 0 then
		if table.getn(timerdata) == 0 then
			...
			if timerdata[currI].paused then
				if Timers_GetFieldValue(triggerdata[Timers_TriggerNameToNumber(timerdata[currI].trigger)],"mode") ~= TIMERS_LOC_INC then
					return Timers_TimeToText(time-timerdata[currI].paused);
				else
					return Timers_TimeToText(timerdata[currI].paused-time);
				end			
			else
				if Timers_GetFieldValue(triggerdata[Timers_TriggerNameToNumber(timerdata[currI].trigger)],"mode") ~= TIMERS_LOC_INC then
					return TitanUtils_GetRedText(Timers_TimeToText(time-Timers_GetLocalTime()));
				else
					return TitanUtils_GetRedText(Timers_TimeToText(Timers_GetLocalTime()-time));
				end
			end			
		end
	else
	...
	if table.getn(timerdata) == 0 then
		...
		for i=1 , table.getn(clonedTable) do
			...
			if clonedTable[i].time-Timers_GetLocalTime() < 0 then
				if Timers_GetFieldValue(triggerdata[Timers_TriggerNameToNumber(clonedTable[i].trigger)],"mode") ~= TIMERS_LOC_INC then
					ToolTip = ToolTip..TitanUtils_GetRedText(Timers_TimeToText(clonedTable[i].time-Timers_GetLocalTime()).."\n");
				else
					if clonedTable[i].paused then
						ToolTip = ToolTip..TitanUtils_GetHighlightText(Timers_TimeToText(clonedTable[i].paused-clonedTable[i].time).."\n");					
					else
						ToolTip = ToolTip..TitanUtils_GetHighlightText(Timers_TimeToText(Timers_GetLocalTime()-clonedTable[i].time).."\n");
					end
				end
			else
