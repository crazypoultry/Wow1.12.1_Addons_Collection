...
function TimersMenu_CreateTimerRowChannelDropDown(level)
	if level == 1 then
		if not (string.find(this:GetName(), "AddFrame") or triggerdata[trigger_edit].name == "default") then
		...
		UIDropDownMenu_AddButton(info, 1);
		info = {["text"] = TIMERS_LOC_BOTH, ["func"] = TimersEdit_SaveChannel, ["arg1"] = this ,["arg2"] = TIMERS_LOC_BOTH};
		UIDropDownMenu_AddButton(info, 1);		
		if IsAddOnLoaded("sct") then
			info = {["text"] = TIMERS_LOC_SCT, ["func"] = TimersEdit_SaveChannel, ["arg1"] = this ,["arg2"] = TIMERS_LOC_SCT};
			UIDropDownMenu_AddButton(info, 1);
		end
		info = {["text"] = TIMERS_LOC_ONSCREEN, ["func"] = TimersEdit_SaveChannel, ["arg1"] = this ,["arg2"] = TIMERS_LOC_ONSCREEN};
		UIDropDownMenu_AddButton(info, 1);
		info = {["text"] = TIMERS_LOC_GUILD, ["func"] = TimersEdit_SaveChannel, ["arg1"] = this ,["arg2"] = TIMERS_LOC_GUILD};
		UIDropDownMenu_AddButton(info, 1);
		info = {["text"] = TIMERS_LOC_OFFICER, ["func"] = TimersEdit_SaveChannel, ["arg1"] = this ,["arg2"] = TIMERS_LOC_OFFICER};
		UIDropDownMenu_AddButton(info, 1);			
		info = {["text"] = TIMERS_LOC_SCRIPT, ["func"] = TimersEdit_SaveChannel, ["arg1"] = this ,["arg2"] = TIMERS_LOC_SCRIPT};
		UIDropDownMenu_AddButton(info, 1);		
	end
end
...
function TimersMenu_CreateTimerRowTypeDropDown(level)
	if level == 1 then
		...
		UIDropDownMenu_AddButton(info, 1);
		info = {["text"] = TIMERS_LOC_RESET, ["func"] = TimersEdit_SaveType, ["arg1"] = this ,["arg2"] = TIMERS_LOC_RESET};
		UIDropDownMenu_AddButton(info, 1);		
...
function TimersMenu_CreateMainMenuDropDown(level)
	if configdata.activetab == 1 or string.find(this:GetName(),"Ancor") then
		if level == 1 then
			info = {["text"] = TIMERS_LOC_ADD_TIMER, ["func"] = ShowUIPanel, ["arg1"] = TimersTimerAddFrame};
			UIDropDownMenu_AddButton(info, 1);		
			...
			UIDropDownMenu_AddButton(info, 1);
		elseif level == 2 and string.find(this:GetName(), "Button6") then
		...
		elseif level == 2 and string.find(this:GetName(), "Button10") then
			if table.getn(timerdata) == 0 then
				info = {["text"] = TIMERS_LOC_NO_TIMERS,["notClickable"] = 1};
				UIDropDownMenu_AddButton(info, 2);
			else
		...
		elseif level == 2 and string.find(this:GetName(), "Button13") then
			...
			UIDropDownMenu_AddButton(info, 2);
			info = {["text"] = TIMERS_LOC_GUILD, ["func"]=TimersSend_AllTimer,  ["arg1"]=TIMERS_LOC_GUILD};
			UIDropDownMenu_AddButton(info, 2);			
...
function TimersMenu_CreateTimerRowDropDown(level)
	if (configdata.activetab == 1 or configdata.activetab == 2) then
		...
		elseif level == 2 then
			...
			UIDropDownMenu_AddButton(info, 2);
			info = {["text"] = TIMERS_LOC_BOTH, ["func"] = TimersSend_Timer, ["arg1"] = timers_dropdown_row, ["arg2"] = TIMERS_LOC_BOTH};
			UIDropDownMenu_AddButton(info, 2);			
			info = {["text"] = TIMERS_LOC_GUILD, ["func"] = TimersSend_Timer, ["arg1"] = timers_dropdown_row, ["arg2"] = TIMERS_LOC_GUILD};
			UIDropDownMenu_AddButton(info, 2);			
...
function Timers_ScaleControl()
	...
	TimersTriggersConfigFrame:SetScale(configdata.framescale/100);
	TimersTimerAddFrame:SetScale(configdata.framescale/100);
	TimersMainAncor:SetScale(configdata.framescale/100);
end
...
function TimersConfig_ToggleBars()
	if(this:GetChecked() ~= nil) then
		TimersMainAncor:Show();
		configdata.showbars = true;
	else 
		TimersMainAncor:Hide();
		configdata.showbars = false;
	end
end
...
function Timers_ShowAdvancedConfig()
	...
	TimersTriggersConfigFrameWarnTimeInput:SetText(Timers_TimeToText(Timers_GetFieldValue(triggerdata[trigger_edit],"prewarn"),true));	
...
function TimersSlash_Reset()
	...
	Timers_TabContentControl();	
	TimersMainFrame:ClearAllPoints(); 
	TimersMainAncor:ClearAllPoints(); 
	TimersMinimizedFrame:ClearAllPoints(); 
	TimersTimerAddFrame:ClearAllPoints(); 
	TimersTriggersConfigFrame:ClearAllPoints(); 
	TimersNameBar:ClearAllPoints(); 
	TimersMainFrame:SetPoint("CENTER","UIParent","CENTER");
	TimersMainAncor:SetPoint("CENTER","UIParent","CENTER");
	TimersMinimizedFrame:SetPoint("CENTER","UIParent","CENTER");
	TimersTimerAddFrame:SetPoint("CENTER","UIParent","CENTER");
	TimersTriggersConfigFrame:SetPoint("CENTER","UIParent","CENTER");
	TimersNameBar:SetPoint("CENTER","UIParent","CENTER");
	Timers_PrintText(TIMERS_LOC_RESETTED);
end

function Timers_Invoke(name)
	local NotSucc = true;
...
function Timers_MsgSendTo(msg, channel)
	if channel == TIMERS_LOC_RAID or channel == TIMERS_LOC_CTRAID or channel == TIMERS_LOC_BOTH then
		if GetNumRaidMembers() == 0 then
			msg = TIMERS_LOC_NOT_IN_RAID..": "..msg;
			channel = TIMERS_LOC_SELF;
		elseif channel == TIMERS_LOC_RAID or channel == TIMERS_LOC_BOTH then
			SendChatMessage(msg,"RAID");
			if channel == TIMERS_LOC_BOTH then
	...
	if channel == TIMERS_LOC_SELF or (channel == TIMERS_LOC_SCT and not (IsAddOnLoaded("sct"))) then
		Timers_PrintText(msg);
	elseif channel == TIMERS_LOC_REPLY then
		SendChatMessage(msg,"WHISPER",nil,ChatEdit_GetLastTellTarget(ChatFrameEditBox));
	elseif channel == TIMERS_LOC_OFFICER then
		SendChatMessage(msg,"OFFICER");
	elseif channel == TIMERS_LOC_GUILD then
		SendChatMessage(msg,"GUILD");
	elseif channel == TIMERS_LOC_SCT then
		SCT_Display_Toggle("SHOWMISS","[Timers: "..msg.."]");
	elseif channel == TIMERS_LOC_ONSCREEN then
		Timers_OnScreenFrame:AddMessage("[Timers: "..msg.."]", 1, 1, 1, 1, 1)
	elseif channel == TIMERS_LOC_SCRIPT then
		RunScript(msg);
	end		
end
...
function Timers_OnEvent(event)
	if event == "VARIABLES_LOADED" then
		...
		Timers_WindowControl();
		if configdata.showbars then
			TimersMainAncor:Show();
			Timers_SetTimerDataBars();
		end 
	...
	elseif event == "TRAINER_CLOSED" or event == "SPELLS_CHANGED" then
		...
		local text;
		if (event == "SPELLCAST_START" or event == "SPELLCAST_STOP" or event == "SPELLCAST_FAILED" or event == "SPELLCAST_INTERRUPTED" or event == "CHAT_MSG_COMBAT_SELF_HITS") then
...
function Timers_SpellEvent(event,arg1,arg2)
	if event == "SPELLCAST_FAILED" or event == "SPELLCAST_INTERRUPTED" or event == "CHAT_MSG_COMBAT_SELF_MISSES" then
		currSpellCast = nil;
	elseif event == "SPELLCAST_STOP" or event == "CHAT_MSG_COMBAT_SELF_HITS" then
...
function Timers_SpellHookByName(name,onSelf)
	Timers_ParseSpellName(name);
	Timers_OldSpellHookByName(name,onSelf);
end
		...
		if string.find(msg,triggerdata[i].text) then
			if (not (triggerdata[i].lastinvoked) or triggerdata[i].lastinvoked+tonumber(Timers_GetFieldValue(triggerdata[i],"threshold"))<Timers_GetLocalTime()) and not (triggerdata[i].inactive or groupdata[Timers_GroupNameToNumber(triggerdata[i].group)].inactive) then
				...
				elseif Timers_GetFieldValue(triggerdata[i],"type") == TIMERS_LOC_ONCEGROUP and Timers_CheckForOnceGroup(triggerdata[i].group) then
					Timers_StartTimer(triggerdata[i],msg);
				elseif Timers_GetFieldValue(triggerdata[i],"type") == TIMERS_LOC_RESET then
					if (not (Timers_CheckForOnce(triggerdata[i].name))) then
						for j = 1, table.getn(timerdata) do
							if timerdata[j].trigger == triggerdata[i].name then
								TimersEdit_ResetTimerByNr(j);
							end
						end
					else
						Timers_StartTimer(triggerdata[i],msg);
...
function Timers_OnUpdate(arg1)
	...
	if (timeSinceUpdate > 0.25) then
		...
		if configdata.activetab == 1 or configdata.activetab == 2 then 
			...
			if configdata.showbars and timerUpdateFlag then
				table.sort(timerdata,Timers_CompareTimers);		
				Timers_SetTimerDataBars();		
				timerUpdateFlag = false;
			end
...
function Timers_TextToTime(text,iswarn)
	local i = 0;
	local mul = 1;
	local time = 0;
	local minus = false;
...
function Timers_SlashCommandHandler(msg)
	...
	if (command == "addi" and args[2]) then
		Timers_addi(args[2],args[3]);
	elseif (command == "add" and args[2] and args[3]) then
		Timers_add(args[2], args[3], args[4], args[5], args[6], args[7], args[8]);
	elseif (command == "reset") then
		TimersSlash_Reset();
	elseif (command == "invoke" and args[2] ) then
		Timers_Invoke(args[2]);		
	elseif (command == "show") then
		ShowUIPanel(TimersMainFrame);
	elseif (command == "debug") then
		Timers_ToggleDebug()
	elseif (command == "help" or commanrd == "?") then
		Timers_PrintText("Advanced Timers v"..configdata.version.." by TyrionWarmage");
		Timers_PrintText("see timers.txt or inGame Menu for Help");
	else
		Timers_PrintText("no valid command - see timers.txt or ingame help for commands");
	end		
end
...
function Timers_TimeToText(time,iswarn)
	if iswarn then
		time=time*60;
	end
...
function Timers_ResetAllTriggerMsg(nondefault)
	...
	if nondefault then
		for i = 1, table.getn(triggerdata) do
			if triggerdata[i].isRC then
				table.insert(RCClone,Timers_cloneTable(triggerdata[i]));
			end
		end	
		triggerdata = {};
	end
	...
	for i = 1, table.getn(RCClone) do
		table.insert(triggerdata,Timers_cloneTable(RCClone[i]));
	end
	table.sort(triggerdata,Timers_CompareTriggers);
	Timers_SetTriggerData();
end	
...
function Timers_SetTriggerData()
	if configdata.activetab == 3 then
		...
		for i = 1, table.getn(triggerdata) do
			...
			if(not(groupdata[group].fold) and not((groupdata[group].inactive or triggerdata[i].inactive) and configdata.hideinactive) and not (triggerdata[i].isRC)) then
...
function Timers_SetEditBox(trigger,field,row,nr)
	...
	if trigger[field] == nil then
		for i = 1, table.getn(triggerdata) do
			if triggerdata[i].name == "default" then
				if field == "time" or field == "prewarn" then
					getglobal("TimersMainFrameTimerRow"..row.."EditBox"..nr):SetText(Timers_TimeToText(tonumber(triggerdata[i][field]),true));
				else		
	...
	elseif field == "prewarn" then
		getglobal("TimersMainFrameTimerRow"..row.."EditBox"..nr):SetText(Timers_TimeToText(tonumber(trigger[field]),true));	
	else		
		getglobal("TimersMainFrameTimerRow"..row.."EditBox"..nr):SetText(trigger[field]);
	end
end

function Timers_InitAddFrame()
	getglobal(this:GetName().."NameInput"):SetText(TIMERS_LOC_NEW_TIMER);
	getglobal(this:GetName().."TimeInput"):SetText(Timers_TimeToText(tonumber(Timers_GetDefaultValue("time"))));
	getglobal(this:GetName().."ModeBox"):SetText(TIMERS_LOC_DEFAULT);
	getglobal(this:GetName().."WarnChanBox"):SetText(TIMERS_LOC_DEFAULT);
	getglobal(this:GetName().."WarnTimeInput"):SetText(Timers_TimeToText(tonumber(Timers_GetDefaultValue("prewarn")),true));
	getglobal(this:GetName().."WarnMessInput"):SetText(Timers_GetDefaultValue("warnmessage"));
	getglobal(this:GetName().."ExpChanBox"):SetText(TIMERS_LOC_DEFAULT);
	getglobal(this:GetName().."ExpMessInput"):SetText(Timers_GetDefaultValue("message"));
end
	
function Timers_AddNewGUITimer()
	local name = TimersTimerAddFrameNameInput:GetText();
	local time = Timers_TextToTime(TimersTimerAddFrameTimeInput:GetText());
	local mode = TimersTimerAddFrameModeBox:GetText();
	local warnchan = TimersTimerAddFrameWarnChanBox:GetText();
	local warntime = Timers_TextToTime(TimersTimerAddFrameWarnTimeInput:GetText(),true);
	local warnmess = TimersTimerAddFrameWarnMessInput:GetText();
	local expchan = TimersTimerAddFrameExpChanBox:GetText();
	local expmess = TimersTimerAddFrameExpMessInput:GetText();
	if time ~= tonumber(Timers_GetDefaultValue("time")) or mode ~= TIMERS_LOC_DEFAULT or warnchan ~= TIMERS_LOC_DEFAULT or warntime ~= Timers_GetDefaultValue("prewarn") or warnmess ~= Timers_GetDefaultValue("warnmessage") or expchan ~= TIMERS_LOC_DEFAULT or expmess ~= Timers_GetDefaultValue("message") then
		TimersRCCounter = TimersRCCounter + 1;
		table.insert(triggerdata, {["name"] = "TmpRC "..TimersRCCounter, ["time"] = time, ["isRC"] = true, ["text"] = "< only a RuntimeCreation >", ["channel"] = expchan, ["warnchannel"] = warnchan, ["message"] = expmess, ["warnmessage"] = warnmess, ["prewarn"] = warntime, ["mode"] = mode});		
		if mode ~= TIMERS_LOC_INC then
			table.insert(timerdata, {name=name, time=time+Timers_GetLocalTime(), trigger="TmpRC "..TimersRCCounter});
		else
			table.insert(timerdata, {name=name, time=Timers_GetLocalTime()-time, trigger="TmpRC "..TimersRCCounter});					
		end
	elseif Timers_GetDefaultValue("mode") ~= TIMER_LOC_INC then
		table.insert(timerdata, {name=name, time=time+Timers_GetLocalTime(), trigger="default"});
	else
		table.insert(timerdata, {name=name, time=Timers_GetLocalTime()-time, trigger="default"});					
	end		
	Timers_PrintText(TIMERS_LOC_TIMERADDED);
	HideUIPanel(TimersTimerAddFrame);
	timerUpdateFlag = true;
end
	
function Timers_add(name,time,channel,msg,warnchannel,warnmsg,warntime)
	if tonumber(time) then
		time = time*60;
	else
		time = Timers_TextToTime(time);
	end
	if not (tonumber(warntime)) then
		if warntime and Timers_TextToTime(warntime) ~= 0 then
			warntime = Timers_TextToTime(warntime,iswarn);
		else
			warntime = nil;
		end
	end
	TimersRCCounter = TimersRCCounter + 1;
	table.insert(triggerdata, {["name"] = "TmpRC "..TimersRCCounter, ["time"] = time, ["isRC"] = true, ["text"] = "< only a RuntimeCreation >", ["channel"] = channel, ["warnchannel"] = warnchannel, ["message"] = msg, ["warnmessage"] = warnmsg, ["prewarn"] = warntime});	
	table.insert(timerdata, {name=name, time=time+Timers_GetLocalTime(), trigger="TmpRC "..TimersRCCounter});
	timerUpdateFlag = true;
	Timers_PrintText(TIMERS_LOC_TIMERADDED);
end

function Timers_addi(name,time)
	if not (time) then
		time = 0;
	...
	TimersRCCounter = TimersRCCounter + 1;
	table.insert(triggerdata, {["name"] = "TmpRC "..TimersRCCounter, ["time"] = time, ["isRC"] = true, ["text"] = "< only a RuntimeCreation >", ["mode"] = TIMERS_LOC_INC});	
	table.insert(timerdata, {name=name, time=Timers_GetLocalTime()-time, trigger="TmpRC "..TimersRCCounter});
	timerUpdateFlag = true;
	Timers_PrintText(TIMERS_LOC_TIMERADDED);
end
...
function Timers_SetTimerDataBars()	
	local Offset = FauxScrollFrame_GetOffset(TimersMainAncorScrollFrame);
	local Max = table.getn(timerdata);
	for i = 1 , 10 do
		if Max < i+Offset then 
			getglobal("TimersMainAncorTimerRow"..i):Hide();
		else
			getglobal("TimersMainAncorTimerRow"..i.."Text1"):SetText(timerdata[i+Offset].name);
			getglobal("TimersMainAncorTimerRow"..i.."Text1"):SetTextColor(1,1,1,1);	
			local max = Timers_GetFieldValue(triggerdata[Timers_TriggerNameToNumber(timerdata[i+Offset].trigger)],"time")
			getglobal("TimersMainAncorTimerRow"..i.."StatusBar"):SetMinMaxValues(0,max);			
			getglobal("TimersMainAncorTimerRow"..i):Show();
		end
	end
	FauxScrollFrame_Update(TimersMainAncorScrollFrame, table.getn(timerdata), 10, 20);
end	
		...
		if Max >= i+Offset then 
			...
			elseif timerdata[i+Offset].paused and not timerdata[i+Offset].editing then
				if Timers_GetFieldValue(triggerdata[Timers_TriggerNameToNumber(timerdata[i+Offset].trigger)],"mode") == TIMERS_LOC_INC then
					...
					getglobal("TimersMainFrameTimerRow"..i.."EditBox2"):SetText(Timers_TimeToText((-1)*(timerdata[i+Offset].paused-timerdata[i+Offset].time)));											
				end
				getglobal("TimersMainFrameTimerRow"..i.."EditBox2"):SetTextColor(1,1,0,1);	
			end					
		end
...
function Timers_UpdateTimesBars()					
	local Offset = FauxScrollFrame_GetOffset(TimersMainAncorScrollFrame);
	local Max = table.getn(timerdata);
	for i = 1 , 10 do
		if Max >= i+Offset then 
			if not timerdata[i+Offset].paused and not timerdata[i+Offset].editing then	
				if Timers_GetFieldValue(triggerdata[Timers_TriggerNameToNumber(timerdata[i+Offset].trigger)],"mode") == TIMERS_LOC_INC then		
					getglobal("TimersMainAncorTimerRow"..i.."Text2"):SetText(Timers_TimeToText(Timers_GetLocalTime()-timerdata[i+Offset].time));							
					getglobal("TimersMainAncorTimerRow"..i.."StatusBar"):SetValue(0);
					getglobal("TimersMainAncorTimerRow"..i.."Text2"):SetTextColor(1,1,1,1);	
				else
					getglobal("TimersMainAncorTimerRow"..i.."Text2"):SetText(Timers_TimeToText(timerdata[i+Offset].time-Timers_GetLocalTime()));			
					if timerdata[i+Offset].time-Timers_GetLocalTime() > 0 then
						getglobal("TimersMainAncorTimerRow"..i.."Text2"):SetTextColor(1,1,1,1);				
						getglobal("TimersMainAncorTimerRow"..i.."StatusBar"):SetValue((timerdata[i+Offset].time-Timers_GetLocalTime()));				
					else
						getglobal("TimersMainAncorTimerRow"..i.."Text2"):SetTextColor(1,0,0,1);
				  	getglobal("TimersMainAncorTimerRow"..i.."StatusBar"):SetValue(0);				
					end			
					if timerdata[i+Offset].paused then
						getglobal("TimersMainAncorTimerRow"..i.."Text2"):SetTextColor(1,1,0,1);
					end	
				end
			elseif timerdata[i+Offset].paused and not timerdata[i+Offset].editing then
				if Timers_GetFieldValue(triggerdata[Timers_TriggerNameToNumber(timerdata[i+Offset].trigger)],"mode") == TIMERS_LOC_INC then
					getglobal("TimersMainAncorTimerRow"..i.."Text2"):SetText(Timers_TimeToText(timerdata[i+Offset].paused-timerdata[i+Offset].time));						
				else
					getglobal("TimersMainAncorTimerRow"..i.."Text2"):SetText(Timers_TimeToText((-1)*(timerdata[i+Offset].paused-timerdata[i+Offset].time)));											
				end
				getglobal("TimersMainAncorTimerRow"..i.."Text2"):SetTextColor(1,1,0,1);	
			end					
		end
...
-- to 3.2 (reload default trigger for warnmessage/channel
...
-- to 3.31 setting Buffcast trigger to Reset
	if (not (configdata.version)) or configdata.version < 3.31 then
		for i = 1, table.getn(triggerdata) do
			if (not (triggerdata[i].isnotdefault)) and (triggerdata[i].group == TIMERS_LOC_WARLOCK or triggerdata[i].group == TIMERS_LOC_PRIEST or triggerdata[i].group == TIMERS_LOC_DRUID or triggerdata[i].group == TIMERS_LOC_MAGE) then
				triggerdata[i].type = TIMERS_LOC_RESET;
			end
		end
	end
-- to 3.33 french warn message fix
	if GetLocale()=="frFR" and ((not (configdata.version)) or configdata.version < 3.33) then
		for i = 1, table.getn(triggerdata) do
			if triggerdata[i].name == "default" then
				triggerdata[i].warnmessage = TIMERS_LOC_DEFAULT_MESSAGE;
			end
		end
	end
-- general: Update Version Tag
	configdata.version = configdata_DEFAULT.version;
end
...
function TimersEdit_DeleteTimer(arg)
	local i = Timers_GetRow(arg) + FauxScrollFrame_GetOffset(TimersMainFrameScrollFrame);
	if triggerdata[Timers_TriggerNameToNumber(timerdata[i].trigger)].isRC then
		Timers_DeleteRC(timerdata[i].trigger);
	end
	table.remove(timerdata, i);
	timerUpdateFlag = true;
end

function Timers_DeleteRC(name)
	local i = table.getn(triggerdata);
	while i > 0 do
		if triggerdata[i].name == name then
			table.remove(triggerdata,i);
...
function TimersEdit_ResetTimerByNr(i)
	...
	if triggerdata[Timers_TriggerNameToNumber(timerdata[i].trigger)].mode == TIMERS_LOC_INC then
		timerdata[i].time = Timers_GetLocalTime()-triggerdata[Timers_TriggerNameToNumber(timerdata[i].trigger)].time;		
	else
...
function TimersEdit_SaveChannel(caller,text)
	...
	if not (TimersTimerAddFrame:IsVisible()) then
...
function TimersEdit_SaveCount()
	...
	triggerdata[i].count = this:GetText();
	Timers_SetTriggerData();
end	
...
function TimersEdit_SavePrewarn()
	...
	triggerdata[i].prewarn = Timers_TextToTime(this:GetText(),true);
	Timers_SetTriggerData();
end	
...
function Timers_MoveAncor(frame)
	MoveTmp[0] = frame:GetLeft();
	MoveTmp[1] = frame:GetTop();
end

function Timers_ClickAncor(frame)
	if MoveTmp[0] == frame:GetLeft() and MoveTmp[1] == frame:GetTop() then
		ToggleDropDownMenu(1, nil, TimersMainFrameDropDownMenu, TimersMainAncorMenuButton, 0, 0)
	end
end
...
function Timers_CheckWarnAllTimers()
	...
	local i = table.getn(timerdata);
	while i > 0 do
		if not (Timers_GetFieldValue(triggerdata[Timers_TriggerNameToNumber(timerdata[i].trigger)],"mode") == TIMERS_LOC_INC or timerdata[i].paused) then
