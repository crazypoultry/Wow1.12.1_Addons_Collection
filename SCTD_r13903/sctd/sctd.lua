--[[
  ****************************************************************
	Scrolling Combat Text - Damage 2.02
	...
	if (info.type == "hit") then
		if (info.source == ParserLib_SELF) then
			if (self:IsMeleeRanged(info.skill)) then
				...
				if (info.isCrushing and SCT.db.profile["SHOWGLANCE"]) then text = SCT.LOCALS.Crushchar..text..SCT.LOCALS.Crushchar end;
				if (info.isGlancing and SCT.db.profile["SHOWGLANCE"]) then text = SCT.LOCALS.Glancechar..text..SCT.LOCALS.Glancechar end;
				self:DisplayText("SCTD_SHOWMELEE", text, info.isCrit, nil, nil, info.victim);
			else
				if (info.isDOT) then
...
----------------------
-- Parses all combat events using paserlib for pets
function SCTD:ParsePetCombat(info)
	--self doesn't work here because of the call from ParserLib
	local self = SCTD;
	...
	if (info.type == "hit") then
		if (info.skill == ParserLib_MELEE) then
			...
			if (info.isCrushing and SCT.db.profile["SHOWGLANCE"]) then text = SCT.LOCALS.Crushchar..text..SCT.LOCALS.Crushchar end;
			if (info.isGlancing and SCT.db.profile["SHOWGLANCE"]) then text = SCT.LOCALS.Glancechar..text..SCT.LOCALS.Glancechar end;
			self:DisplayText("SCTD_SHOWPET", text, info.isCrit, info.element, info.amountResist, info.victim, PET);
		else
	...
	elseif (info.type == "miss") then
		...
		local miss = getglobal(strupper(info.missType));
		self:DisplayText("SCTD_SHOWPET", miss, nil, nil, nil, info.victim, info.skill);
	end
end
...
function SCTD:RegisterSelfEvents()
	...
	parser:RegisterEvent("sctd", "CHAT_MSG_COMBAT_PET_HITS", SCTD.ParsePetCombat) ;
	parser:RegisterEvent("sctd", "CHAT_MSG_COMBAT_PET_MISSES", SCTD.ParsePetCombat) ;
	parser:RegisterEvent("sctd", "CHAT_MSG_SPELL_PET_DAMAGE", SCTD.ParsePetCombat) ;
end
