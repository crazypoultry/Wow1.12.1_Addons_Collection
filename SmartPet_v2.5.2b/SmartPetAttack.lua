...
function SmartPet_Attack()
	...
	if (SmartPet_Config.DebufCheck ) then
		if (SmartPet_CheckDebuff()) then
			UIErrorsFrame:AddMessage(gsub(SMARTPET_PET_DEBUFF, "%%n", UnitName("pet")), 0, 0, 1, 1.0, UIERRORS_HOLD_TIME);
			return;
		end
	end
	...
	if  (not (pet == nil) and not ( UnitIsDead("pet") ) )then
		...
		if (UnitExists("target") and UnitCanAttack("player", "target") ) then
			...
			if not ( UnitIsDead("target") ) then
				...
				SmartPet_AddDebugMessage("Last Attack Time: "..SmartPet_Vars.LastAttack, "spew");
				SmartPet_AddDebugMessage("Alert Timeout : "..SmartPet_Config.AlertTimeout, "spew");
...
function SmartPet_CheckDebuff()
	unit = "target";
	--SmartPet_AddDebugMessage("Checking Debuff", "spew");
	SmartPetTip:ClearLines();
		...
		local msg = "";	
			...
			--SmartPet_AddDebugMessage(UnitDebuff(unit,j), "spew");
			SmartPetTip:SetUnitDebuff(unit, j);
			msg = SmartPetTipTextLeft1:GetText();
			--SmartPet_AddDebugMessage(msg, "spew");
