if (not Nurfed_CombatLog) then
	...
	function Nurfed_CombatLog:UpdateCVar(cvar)
		local val = utility:GetOption("combatlog", cvar);
		SetCVar(cvar, tonumber(val));
	end
	...
	function Nurfed_CombatLog:FormatRGB(option, crit)
		local info = utility:GetOption("combatlog", option);
		if (not info) then
			return "|cffffffff";
		end
		local r, g, b;
		if (crit) then
			local overlay = utility:GetOption("combatlog", "overlay");
	...
	function Nurfed_CombatLog:FormatName(name, option, t, noformat)
		...
		local unit;
		local out = {};
		local format = utility:GetOption("combatlog", option);
		local watches = utility:GetOption("combatlog", option.."watches");
	...
	function Nurfed_CombatLog:Overheal(name, heal)
		local unit;
		if (name == YOU) then
			unit = "player";
		else
	...
	function Nurfed_CombatLog:Output(vars, t, text)
		...
		if (t == "Heal" or t == "Hit" or t == "Spell" or t == "Dot" or t == "SpellMiss") then
			...
			if (vars[5] == "Heal") then
				local heal = self:Overheal(vars[3], vars[4]);
				amount = self:FormatRGB("Heal", vars[6]).."+"..heal.."|r";
			else
			...
			if (vars[6]) then
				format = utility:GetOption("combatlog", "crit");
		...
		elseif (t == "Death" or t == "Destroyed") then
			local deathout = utility:GetOption("combatlog", "deathout");
			if (deathout == 0) then
				return;
			end
			...
			if (t == "Destroyed") then
				local destroyed = utility:GetOption("combatlog", "destroyed");
				if (destroyed ~= 1) then
					return;
				end
			end
			...
			format = utility:GetOption("combatlog", "death");
		...
		elseif (t == "Cast" or t == "Perform") then
			...
			if (casted) then
				format = utility:GetOption("combatlog", "spellalert");
