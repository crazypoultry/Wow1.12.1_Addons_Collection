...
function FriendShare_Import()
	...
	for i, name in globalRemoves do
	...
	for i, name in globalFriends do
...
function FriendShare_ProcessAlts( curFriends )
	local player = UnitName( "player" );
	for i, name in FriendShare_Alts[realmAndFaction] do
		if ( FriendShare_AutoAlts ) then
...
function FriendShare_OnEvent(event)
	...
	if ( event == "PLAYER_LEAVING_WORLD" ) then
		...
		for i, name in importedGlobalFriends do
