...
recap_temp.Filter = {
		...
		{ source=1, dest=3, damage=4, effect=2, crit=0, pattern=SPELLLOGOTHEROTHER, ambigous=true }, -- (.+)'s (.+) hits (.+) for (%d+)
		{ source=1, dest=6, damage=3, effect=2, crit=0, pattern=SPELLLOGOTHERSELF }, -- (.+)'s (.+) hits you for (%d+)

		{ source=6, dest=2, damage=3, effect=1, crit=1, pattern=SPELLLOGCRITSELFOTHER }, -- Your (.+) crits (.+) for (%d+)
		{ source=1, dest=3, damage=4, effect=2, crit=1, pattern=SPELLLOGCRITOTHEROTHER, ambigous=true }, -- (.+)'s (.+) crits (.+) for (%d+)
		{ source=1, dest=6, damage=3, effect=2, crit=1, pattern=SPELLLOGCRITOTHERSELF }, -- (.+)'s (.+) crits you for (%d+)
		...
		{ source=1, dest=6, damage=2, effect=7, crit=0, pattern=COMBATHITOTHERSELF }, -- (.+) hits you for (%d+).
		...
		{ source=1, dest=6, damage=2, effect=7, crit=1, pattern=COMBATHITCRITOTHERSELF }, -- (.+) crits you for (%d+).

		-- WoW 1.7.0 patterns

		{ source=6, dest=2, damage=3, effect=1, crit=1, pattern=SPELLLOGCRITSCHOOLSELFOTHER }, -- Your (.+) crits (.+) for (%d+) (.+) damage.
		{ source=1, dest=3, damage=4, effect=2, crit=1, pattern=SPELLLOGCRITSCHOOLOTHEROTHER, ambigous=true }, -- (.+)'s (.+) crits (.+) for (%d+) (.+) damage.
		{ source=1, dest=6, damage=3, effect=2, crit=1, pattern=SPELLLOGCRITSCHOOLOTHERSELF }, -- (.+)'s (.+) crits you for (%d+) (.+) damage.

		{ source=6, dest=2, damage=3, effect=1, crit=0, pattern=SPELLLOGSCHOOLSELFOTHER }, -- Your (.+) hits (.+) for (%d+) (.+) damage.
		{ source=1, dest=3, damage=4, effect=2, crit=0, pattern=SPELLLOGSCHOOLOTHEROTHER, ambigous=true }, -- (.+)'s (.+) hits (.+) for (%d+) (.+) damage.
		{ source=1, dest=6, damage=3, effect=2, crit=0, pattern=SPELLLOGSCHOOLOTHERSELF }, -- (.+)'s (.+) hits you for (%d+) (.+) damage.

		{ source=6, dest=1, damage=2, effect=3, crit=1, pattern=COMBATHITCRITSCHOOLSELFOTHER }, -- You crit (.+) for (%d+) (.+) damage.
		{ source=1, dest=2, damage=3, effect=4, crit=1, pattern=COMBATHITCRITSCHOOLOTHEROTHER }, -- (.+) crits (.+) for (%d+) (.+) damage.
		{ source=1, dest=6, damage=2, effect=3, crit=1, pattern=COMBATHITCRITSCHOOLOTHERSELF }, -- (.+) crits you for (%d+) (.+) damage

		{ source=6, dest=1, damage=2, effect=3, crit=0, pattern=COMBATHITSCHOOLSELFOTHER }, -- You hit (.+) for (%d+) (.+) damage.
		{ source=1, dest=2, damage=3, effect=4, crit=0, pattern=COMBATHITSCHOOLOTHEROTHER }, -- (.+) hits (.+) for (%d+) (.+) damage.
		{ source=1, dest=6, damage=2, effect=3, crit=0, pattern=COMBATHITSCHOOLOTHERSELF }, -- (.+) hits you for (%d+) (.+) damage.

		-- WoW 1.7.0 patterns end

		{ source=6, dest=2, damage=3, effect=1, pattern=SPELLSPLITDAMAGESELFOTHER }, -- Your (.+) causes (.+) (%d+) damage
		{ source=1, dest=3, damage=4, effect=2, pattern=SPELLSPLITDAMAGEOTHEROTHER, ambigous=true }, -- (.+)'s (.+) causes (.+) (%d+) damage
		{ source=1, dest=6, damage=3, effect=2, pattern=SPELLSPLITDAMAGEOTHERSELF }, -- (.+)'s (.+) causes you (%d+) damage
		...
		{ source=1, dest=6, damage=2, effect=8, pattern=DAMAGESHIELDOTHERSELF }, -- (.+) reflects (%d+) (.+) damage to you.		
...
recap_temp.FilterSize = table.getn(recap_temp.Filter); -- number of filters, change to recap_temp.Filter size
...
recap_temp.HealFilter = {
		...
		{ source=6, dest=6, heal=2, effect=1, crit=1, pattern=HEALEDCRITSELFSELF }, -- Your (.+) critically heals you for (%d+)
		{ source=6, dest=2, heal=3, effect=1, crit=1, pattern=HEALEDCRITSELFOTHER }, -- Your (.+) critically heals (.+) for (%d+)
		{ source=1, dest=3, heal=4, effect=2, crit=1, pattern=HEALEDCRITOTHEROTHER, ambigous=true }, -- (.+)'s (.+) critically heals (.+) for (%d+)
		{ source=1, dest=6, heal=4, effect=2, crit=1, pattern=HEALEDCRITOTHERSELF }, -- (.+)'s (.+) critically heals you for (%d+)

		{ source=6, dest=6, heal=2, effect=1, crit=0, pattern=HEALEDSELFSELF }, -- Your (.+) heals you for (%d+)
		{ source=6, dest=2, heal=3, effect=1, crit=0, pattern=HEALEDSELFOTHER }, -- Your (.+) heals (.+) for (%d+)
		{ source=1, dest=3, heal=4, effect=2, crit=0, pattern=HEALEDOTHEROTHER, ambigous=true }, -- (.+)'s (.+) heals (.+) for (%d+)
		{ source=1, dest=6, heal=3, effect=2, crit=0, pattern=HEALEDOTHERSELF, tag=100 } -- Your (.+) heals (.+) for (%d+)
};
recap_temp.HealFilterSize = table.getn(recap_temp.HealFilter);
...
recap_temp.DeathFilterSize = table.getn(recap_temp.DeathFilter);
...
local function Recap_Localize(filter)
	...
	for i in filter do
		...
		-- Quote magic characters ".()".
		filter[i].pattern = string.gsub(filter[i].pattern,"%.", "%%%.");
		filter[i].pattern = string.gsub(filter[i].pattern,"%(", "%%%(");
		filter[i].pattern = string.gsub(filter[i].pattern,"%)", "%%%)");
		-- Convert placeholders to regular expression patterns.
		...
		if string.find(filter[i].pattern,"%$") then
			...
			for k in filter[i] do
				if tonumber(filter[i][k]) and 0<tonumber(filter[i][k]) and tonumber(filter[i][k])<6 then
		...
		-- Speedup patternmatching by making patterns as specific as possible (See PiL 20.4)
		-- and append pattern for possible trailer.
		filter[i].pattern = "^"..filter[i].pattern..".*$";

		-- In case of an ambigous filter change "(.+)s" into "([^%s]+)s".
		if filter[i].ambigous then
			filter[i].pattern = string.gsub(filter[i].pattern,"%(%.%+%)s","([^%%s]+)s");
		end
	end
