...
Reputations_XMsg = 0;    --Display inverted Chat-Messages (Raw)
...
function Reputation_RegisterCosmos()
        if (Cosmos_RegisterConfiguration) then
                ...
                Cosmos_RegisterConfiguration(
                        ...
                        "CHECKBOX",
                        REPUTATION_XMSG,
                        REPUTATION_XMSG_INFO,
                        Reputation_XMsg_Toggle,
                        1);
        end
end
...
function ReputationMod_OnLoad()
	...
	this:RegisterEvent("PLAYER_LEAVING_WORLD");
        ...
        if( DEFAULT_CHAT_FRAME and Reputations_Load==1 ) then
                DEFAULT_CHAT_FRAME:AddMessage(format(REPUTATION_LOADED_MSG,Reputations_Version));
        end
end
...
local function ReputationMod_Update()
        ...
        local Reputations_TempRaw = Reputations_Raw;
        if (Reputations_XMsg == 1) then
                if (Reputations_Raw == 1) then
                        Reputations_Raw = 0;
                elseif (Reputations_Raw == 0) then
                        Reputations_Raw = 1;
                end
        end
        ...
        Reputations_Raw = Reputations_TempRaw;
end

Reputations_inworld = 0;
function ReputationMod_OnEvent()
if(event == "PLAYER_LEAVING_WORLD") then
Reputations_inworld = 0
end
if(event == "PLAYER_ENTERING_WORLD") then
Reputations_inworld = 1
end
if(event == "UPDATE_FACTION") and Reputations_inworld==1 then
...
function Reputation_Debug_Toggle(toggle)
        if (toggle) then
                ...
                if (toggle==1) then
                        ...
                        DEFAULT_CHAT_FRAME:AddMessage(REPUTATION_DEBUG_OFF);
                end
        else
                if (Reputations_Debug == 1) then
                        Reputations_Debug = 0;
...
function Reputation_ChatFrame_Change(checked,value)  --Checked will always be 0
        if (value) then
                if (value > 0 and value <= FCF_GetNumActiveChatFrames()) then
                        ...
                        REPUTATIONS_CHAT_FRAME = getglobal("ChatFrame"..Reputations_Frame);
                        REPUTATIONS_CHAT_FRAME:AddMessage(REPUTATION_NOTIFY_FRAME,1.0,1.0,0.0);
                else
...
function Reputation_XMsg_Toggle(toggle)
        if (toggle) then
                Reputations_XMsg = toggle;
                if (toggle==1) then
                        DEFAULT_CHAT_FRAME:AddMessage(REPUTATION_XMSG_ON);
                else
                        DEFAULT_CHAT_FRAME:AddMessage(REPUTATION_XMSG_OFF);
                end
        else
                if (Reputations_XMsg == 1) then
                        Reputations_XMsg = 0;
        ...
        if (Cosmos_UpdateValue) then
                Cosmos_UpdateValue("COS_REPUTATION_XMSG_X",CSM_CHECKONOFF,Reputations_XMsg);
        end
end
...
function Reputation_SlashHandler(msg)
        ...
        if (not msg or msg == "") then --Show Help
                ...
                if (command == REPUTATION_LIST) then
                        ...
                        for factionIndex=1, numFactions, 1 do
                                ...
                                if (not isHeader) then
                                        ...
                                        REPUTATIONS_CHAT_FRAME:AddMessage(name..": "..factionCompleteInfo);
                                end
                        end
                ...
                elseif (command == REPUTATION_PCT_RAW) then
                        Reputation_Raw_Toggle();
                elseif (command == REPUTATION_PCT_XMSG) then
                        Reputation_XMsg_Toggle();
...
function ReputationFrame_Update_New()
        ...
        for i=1, NUM_FACTIONS_DISPLAYED, 1 do
                ...
                if ( factionIndex <= numFactions ) then
                        _, description, standingID, barMin, barMax, barValue, _, _, isHeader, _ = GetFactionInfo(factionIndex);
