...
function SmartHeal:setDefault(defaultOption)
	...
	if(not defaultOption) then
		for option,val in pairs(SmartHeal.default) do
			
			if (type(SmartHeal.default[option])=="table") then
				
				local module=option
				for option2,value in pairs(SmartHeal.default[module]) do
		...
		for i,option in pairs(defaultOption) do
...
function SmartHeal:doCast(spell,rank)
	...
	if (rank and rank~="") then
		...
		if SmartHeal.spellList[spell]~=nil and SmartHeal.spellList[spell].HoT~=1 then
	...
	local SystemAutoSelfCast=GetCVar("autoSelfCast")
	SetCVar("autoSelfCast","0")
	
	CastSpellByName(spell_with_rank,SmartHeal.selfCast)
	
	SetCVar("autoSelfCast",SystemAutoSelfCast)
...
function SmartHeal:Cast(spell_fullname,ForceTarget)
	...
	if (not spell) then spell=spell_fullname end
	if (rankCap) then rankCap=tonumber(rankCap) end
	...
	if (not SmartHeal.selfCast) then
		SmartHeal.selfCast=SmartHeal:isSelfCast()
...
function SmartHeal:UseAction(slot, arg2, onself)
	...
	if ( (SmartHeal.spellList[spell]~=nil or SmartHeal.buffList[spell]~=nil)
		...
		elseif(	SmartHeal:isSelfCast()==1 and SmartHeal:getConfig("autoselfcast") and
...
function SmartHeal:SpellMaxRank(spellname,ignoreMana)
	...
	for r,spell in pairs(spells) do
		local i = 1
		while true do
			local sName , sRank = GetSpellName(i, BOOKTYPE_SPELL)
			if not sName then
