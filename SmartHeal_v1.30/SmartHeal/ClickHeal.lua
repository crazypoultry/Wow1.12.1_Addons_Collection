...
function SmartHeal:InitClickMode()
	...
	for option,value in pairs(SmartHeal.defaultClick[SmartHeal.playerClass]) do
	...
	Perl_Custom_ClickFunction = SH_Perl_Custom_ClickFunction
...
function SmartHeal:setClickMode(status)
	...
	if (status) then
		...
		Perl_Custom_ClickFunction = function(button,unit) return SmartHeal:XPerl_ClickHeal(button,unit) end
...
function SmartHeal:XPerl_ClickHeal(button,unit)
	...
	elseif KeyDownType then
		...
		return true
	elseif ( type(SH_Perl_Custom_ClickFunction)=="function") then
		return SH_Perl_Custom_ClickFunction(button,unit)
	else
		return false
	end
end
...
function SmartHeal:GetClickHealButton()
	...
	local i=1
	for key,status in pairs(keydowntype) do
...
function SmartHeal:ResumeAttack_OnUpdate(arg1)
	...
	if(SmartHeal.ResumeAttack) then
		...
		if(SmartHeal.ResumeAttackTimer>SmartHeal.ResumeAttackDelay) then
			if (UnitExists("target") and not SmartHeal.Attacking and not UnitIsDead("target") and UnitCanAttack("player","target")) then 
					AttackTarget()
			end
			SmartHeal:ResetResumeAttack()
		end
...
function SmartHeal:ResetResumeAttack()
