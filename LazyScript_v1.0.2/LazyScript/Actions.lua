...
function lazyScript.Action:New(code, texture, interrupts, triggersGlobal, problemTexture, name)
   ...
   if (problemTexture ~= nil) then
      ...
      if not lazyScript.getLocaleString("ACTION_TTS", false, true) then
         lazyScript.p("The action "..code.." is not supported for your locale. Please use 'action=<full action name>' instead.")
      end
   else
...
function lazyScript.Action:InitTexture(texture)
   ...
   elseif type(texture) == "table" then
      ...
      error("Holy flying unknown texture type, Batman!")
   end
end
...
function lazyScript.Action:GetSlot(sayNothing)
                  ...
                  if (thisTexture and self:CompareTexture(thisTexture)) then
                     if (self.problemTexture) then
                        if (lazyScript.GetActionNameFromTooltip(slot) == self.name) then
                           lazyScript.d("found "..self.name.." at slot "..slot)
                           self.slot = slot
                           break
                        end
                     else
                        lazyScript.d("found "..lazyScript.safeString(self.name).." at slot "..slot)
                        self.slot = slot
                        break
                     end
                  end
   ...
   if (not self.slot) then
      if (not sayNothing) then
         lazyScript.p("Couldn't find "..lazyScript.safeString(self.name).." on your action bar, PLEASE ADD IT.")
      end
      return nil
   end
...
function lazyScript.Action:FindSpellRanks(sayNothing)
      ...
      if (self:CompareTexture(texture)) then
         if self.problemTexture then
            if GetSpellName(spellIndex,"spell") == self.name then
               if not sayNothing then
                  lazyScript.d("SpellSearch found "..self.code.." at index "..spellIndex..".")
               end
   ...
   if not spellIndexStart then
      if not sayNothing then
         lazyScript.d("spellSearch found more than 1000 spells.")
      end
      return nil
   end
   ...
   for spellRank = 1, 99 do
      ...
      if (not texture) then
         if not sayNothing then
            lazyScript.d("SpellSearch found nil texture at index "..spellIndex..". Rank Count = "..rankCount)
         end
         break
      end
      lazyScript.d("rankCount = "..rankCount..", texture = "..texture)
      if (not self:CompareTexture(texture)) then
         if not sayNothing then
            lazyScript.d("SpellSearch stopped finding "..self.code.." at index "..spellIndex..". Rank Count = "..rankCount)
         end
         break
      else
         if self.problemTexture then
...
function lazyScript.Action:FindSpellRanksByName(sayNothing)
   ...
   for spellIndex = 1, 1000 do
      ...
      if (not spellName) then
         if not sayNothing then
            lazyScript.d("SpellSearch found nil name at index "..spellIndex..".")
         end
         return nil
      end
   ...
   for spellRank = 1, 99 do
      ...
      if (not spellName) then
         if not sayNothing then
            lazyScript.d("SpellSearch found nil name at index "..spellIndex..". Rank Count = "..rankCount)
         end
         break
      end
...
function lazyScript.Action:GetRank()
   ...
   if (not self.rank) then
      lazyScript.p("Couldn't find "..self.name.." in your spell book.")
      return 0
   end
   return self.rank
end
...
function lazyScript.CastSpellByRankAction:Use()
   lazyScript.d("Attempting to cast "..self.code)
...
function lazyScript.EquipItem:IsUsable(sayNothing)
   ...
   if (link) then
      ...
      if (id) then
         ...
         if ((self.id and id == self.id) or string.lower(name) == string.lower(self.name)) then
            if (not sayNothing) then
               lazyScript.d("Item is already equipped.")
            end
            return false
         end
      end
   ...
   if (not sayNothing) then
      lazyScript.d("Item not found: "..lazyScript.safeString(self.name or self.id))
   end
   return false
end
...
function lazyScript.Item:Use()
   ...
   if not bag then
      lazyScript.d("Using item: "..self.name.."("..lazyScript.safeString(self.id)..") at equipped slot: "..slot)
      ...
      if (not self.equippedOnly) then
         lazyScript.d("Using item: "..self.name.."("..lazyScript.safeString(self.id)..") at bag/slot slot: "..bag.."/"..slot)
...
function lazyScript.ApplyWeaponBuff:Use()
   if (not self.slot) or (not self.bag) then
      -- wtf?
      lazyScript.d("Weapon buff bag slot not found. WTF?")
      return
   end
...
function lazyScript.ApplyWeaponBuff:IsUsable(sayNothing)
   ...
   if itemType ~= "Weapon" then
      if not sayNothing then
         lazyScript.d("ApplyWeaponBuff: Item "..self.name.." in "..self.equipSlot.." item type is not a Weapon: "..lazyScript.safeString(itemType))
      end
      return false
   end
   ...
   if (link) then
      ...
      if (id) then
         ...
         if ((itemId and id == itemId) or (itemName and (string.lower(name) == string.lower(itemName)))) then
            lazyScript.d("IsItemStillHere: Found item "..(itemName or tostring(itemId)).." at Bag/Slot: "..bag.."/"..slot)
            return true
         end
      end
   end
      ...
      for slot = 1, GetContainerNumSlots(bag) do
         ...
         if (link) then
            ...
            if (id) then
               -- itemId might be nil, in which case match by name
               if ((itemId and id == itemId) or (itemName and (string.lower(name) == string.lower(itemName)))) then
                  lazyScript.d("SearchBags: Found item "..(itemName or tostring(itemId)).." at Bag/Slot: "..bag.."/"..slot)
                  return slot, bag
               end
            end
         end
...
function lazyScript.pseudoActions.assist:IsUsable(sayNothing)
   ...
   if (lazyScript.assistName == nil) then
      if (not sayNothing) then
         lazyScript.p("Assist target not set. Please set with /lazyscript assist <assist name>")
      end
      return false
   end
   ...
   if (lazyScript.assistUnitId == nil or string.lower(UnitName(lazyScript.assistUnitId) or "") ~= string.lower(lazyScript.assistName)) then
      ...
      if (lazyScript.assistUnitId == nil) then
         if (not sayNothing) then
            lazyScript.p("Cannot find "..lazyScript.assistName.." in group or raid to assist. Please update with /lazyscript assist <assist name>")
         end
         return false
      end
   end
   ...
   if (UnitIsDead(lazyScript.assistUnitId)) then
      if (not sayNothing) then
         lazyScript.p(lazyScript.assistName.." seems to be dead, cannot assist.")
      end
      return false
   end
   ...
   if (not UnitIsVisible(lazyScript.assistUnitId)) then
      if (not sayNothing) then
         lazyScript.p(lazyScript.assistName.." seems to be out of range, cannot assist.")
      end
      return false
   end
   ...
   if (UnitIsDead(assistTarget)) then
      if (not sayNothing) then
         lazyScript.d(lazyScript.assistName.."'s target is dead, skipping to next action")
      end
      return false
   end
...
function lazyScript.pseudoActions.stopAll:Use()
   ...
   if lazyScript.ClassCanAutoShot() then
      if lazyScript.IsAutoShooting() then
         lazyScript.d("Stopping Auto Shot...")
...
function lazyScript.ShapeshiftForm:FindShapeshift(sayNothing)
   ...
   if numForms < 1 then
      if (not sayNothing) then
         lazyScript.p("No shapeshift forms available.")
      end
      return false
   end
   ...
   if (not sayNothing) then
      lazyScript.p("Shapeshift form: "..self.code.." not found.")
   end
   return false
end
...
function lazyScript.PetAction:GetSlot(sayNothing)
         ...
         if (thisTexture) then
            if (self.texture) then
               if (string.find(thisTexture, self.texture)) then
                  ...
                  lazyScript.d("Found pet action "..self.name.." at slot "..slot..".")
                  self.slot = slot
                  break
               end
            else
   ...
   if (not self.slot) then
      if (not sayNothing) then
         lazyScript.p("Could not find "..self.code.." on your pet action bar, PLEASE ADD IT.")
      end
      return nil
   end
...
function lazyScript.PetAction:Use()
   lazyScript.d("Pet action: "..lazyScript.safeString(self.name))
...
function lazyScript.PetAction:IsUsable(sayNothing)
   if (not PetHasActionBar()) then
      if (not sayNothing) then
         lazyScript.d("You do not have a pet action bar.")
      end
      return false
   end
...
function lazyScript.pseudoActions.petAttack:IsUsable(sayNothing)
   if (not UnitExists("pet")) then
      if (not sayNothing) then
         lazyScript.d("You do not have a pet.")
      end
      return false
   end
...
function lazyScript.CallFormAction:GetSubstituteActions(sayNothing)
   ...
   if not actions then
      if not sayNothing then
         lazyScript.p("Could not call form "..self.form..".")
      end
      return nil
   end
...
function lazyScript.Try(actions, masks, doNothing)
   ...
   for idx, actionObj in ipairs(actions) do
      if not sayNothing then
         lazyScript.d("Try: Action #"..idx..": "..actionObj.code)
      end
