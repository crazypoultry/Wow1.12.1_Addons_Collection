...
function lazyScript.bitParsers.targetUnit(bit, actions, masks)
   ...
   if not lazyScript.validateUnitId(unitId) then
      lazyScript.p("The UnitId '"..unitId.."' is not valid.")
      return nil
   end
...
function lazyScript.bitParsers.sayIn(bit, actions, masks)
   ...
   elseif (channel == "Minion") then
      ...
      lazyScript.p("Unknown channel name: "..lazyScript.match1)
      return nil
...
function lazyScript.bitParsers.wand(bit, actions, masks)
   ...
   if class ~= "MAGE" and class ~= "PRIEST" and class ~= "WARLOCK" then
      lazyScript.p("You can't shoot wands.")
      return nil
   end
...
function lazyScript.bitParsers.autoShot(bit, actions, masks)
   ...
   if class ~= "HUNTER" then
      lazyScript.p("You do not have Auto Shot.")
      return nil
   end
...
function lazyScript.bitParsers.cancelBuff(bit, actions, masks)
   ...
   for idx, obj in pairs(lazyScript.buffTable) do
      ...
      if (idx == buffCode) then
         lazyScript.d("Found known buff: "..buffCode)
         buffObj = obj
         break
      end
   end
   ...
   if (buffObj == nil) then
      lazyScript.p("No buff entry found for "..buffCode..". Try using cancelBuffTitle instead.")
      return nil
   else
...
function lazyScript.bitParsers.cancelBuffTitle(bit, actions, masks)
   ...
   if (buffObj == nil) then
      lazyScript.d("No buff entry found for "..buffCode..".")
      buffName = buffCode
   else
      buffName = buffObj.name
      buffTexture = buffObj.texture
...
function lazyScript.bitParsers.callForm(bit, actions, masks)
   ...
   if not lazyScript.FindForm(form) then
      lazyScript.p("Could not find form "..form..".")
      return nil
   end
...
function lazyScript.bitParsers.applyWeaponBuff(bit, actions, masks)
   ...
   if (lazyScript.match1 ~= "MainHand" and lazyScript.match1 ~= "OffHand") then
      lazyScript.p("Only MainHand and OffHand are supported, not"..lazyScript.match1)
      return nil
   end
...
function lazyScript.masks.TargetInMeleeRange(sayNothing)
   if (not lazyScript.rangeCheckAction) then
      if not sayNothing then
         lazyScript.p("TargetInMeleeRange: rangeCheckAction is nil.")
      end
      return nil
   end
   ...
   if rangeCheck == nil then
      if not sayNothing then
         lazyScript.p("TargetInMeleeRange: IsActionInRange returned nil. Target is invalid.")
      end
      return nil
   end
...
function lazyScript.bitParsers.ifTargetInMeleeRange(bit, actions, masks)
   ...
   if class ~= "ROGUE" and class ~= "DRUID" and class ~= "HUNTER" and class ~= "WARRIOR" then
      lazyScript.p("ifTargetInMeleeRange is not currently supported for your class.")
      return nil
   elseif (not lazyScript.rangeCheckAction) then
      local rangeAction = lazyScript.getRangeCheckAction()
      lazyScript.p("ifTargetInMeleeRange is not currently supported for your class because you do not have "..lazyScript.safeString(rangeAction.name).." yet.")
      return nil
   end
   ...
   elseif (unitId == "target") then
      if (wantPct) then
         ...
         if (not MobHealth_GetTargetCurHP) then
            if (not sayNothing) then
               lazyScript.p("MobInfo2 (or equivalent) not installed, can't determine target's HP.")
            end
            return nil
         end
...
function lazyScript.bitParsers.UnitHPMP(bit, actions, masks)
   ...
   if not lazyScript.validateUnitId(unitId) then
      lazyScript.p(unitId.." is not a valid UnitId.")
      return nil
   end
   ...
   while true do
      ...
      if (lazyScript.masks.problemMounts[texture]) then
         ...
         if (not mountString) then
            if not sayNothing then
               lazyScript.d("Localization does not exist for problem mount: "..texture)
            end
            return false
         end
...
function lazyScript.masks.InInstance(sayNothing)
   ...
   if (not instances) then
      if (not sayNothing) then
         lazyScript.p("Sorry, ifInInstance not supported for your locale.")
      end
      return nil
   end
...
function lazyScript.masks.InBattleground(sayNothing)
   for i = 1, MAX_BATTLEFIELD_QUEUES do
      ...
      if (status == "active") then
         if (not sayNothing) then
            lazyScript.d("You're in batteground: "..mapName)
         end
         return true
      end
      return false
...
function lazyScript.masks.IsGlobalCooldown(gcdActionObj)
   return function(sayNothing)
      ...
      if not slot then
         if not sayNothing then
            lazyScript.p("To enable checking of the Global Cooldown, please place the "..lazyScript.safeString(gcdActionObj.name).." action on your action bar.")
         end
         return nil
      end
...
function lazyScript.bitParsers.ifGlobalCooldown(bit, actions, masks)
   ...
   elseif class == "PALADIN" then
      ...
      lazyScript.p("HAX! Unknown class: "..lazyScript.safeString(class))
      return nil
   end
...
function lazyScript.bitParsers.ifFlagRunner(bit, actions, masks)
   ...
   elseif unitId == "target" then
      if (not lazyScript.getLocaleString("BG_WSG_ZONE")) then
         lazyScript.p("Sorry, ifTargetFlagRunner is not supported for hostile targets in your locale.")
         return nil
      end
...
function lazyScript.masks.TargetMyLevel(gtLtEq, val)
   return function(sayNothing)
      ...
      if (gtLtEq == ">") then
         if (not sayNothing) then
            lazyScript.d("Search for target over lvl "..tmp)
         end
         return (tmp < tLevel)
      elseif (gtLtEq == "=") then
         if (not sayNothing) then
            lazyScript.d("Search for target equal to lvl "..tmp)
         end
         return (tmp == tLevel)
      else
         if (not sayNothing) then
            lazyScript.d("Search for target under lvl "..tmp)
         end
         return (tmp > tLevel)
      end
   end
...
function lazyScript.bitParsers.ifTargetMyLevel(bit, actions, masks)
   ...
   elseif lazyScript.match3 == "minus" then
      ...
      lazyScript.d("unable to determine plus/minus sign")
      return nil
   end
   ...
   if spellTypeStrings and nameRegex then
      for k, v in pairs(spellTypeStrings) do
         ...
         if string.upper(nameRegex) == string.upper(k) then
            ...
            lazyScript.d("Match found: Key: "..k..", value: "..v.." SpellTypeRegex: "..nameRegex)
            break
         end
      end
   end
...
function lazyScript.bitParsers.ifKeyDown(bit, actions, masks)
   ...
   if (key ~= lazyScript.relax("Ctrl") and key ~= lazyScript.relax("Alt") and key ~= lazyScript.relax("Shift")) then
      lazyScript.p("Only Ctrl, Alt, and Shift are supported, not: "..key)
   end
...
function lazyScript.bitParsers.everyXSeconds(bit, actions, masks)
   ...
   if (not lastAction) then
      lazyScript.p("You must put an action before -everyXXs.")
      return nil
   end
...
function lazyScript.bitParsers.ifTimer(bit, actions, masks)
   ...
   if (lazyScript.match2 == nil) or (lazyScript.match3 == nil) then
      lazyScript.p("syntax e.g. -ifTimer>5s=gouge")
      return nil
   end
   local negate = lazyScript.negate1()
   local val = tonumber(lazyScript.match2)
   if val == nil then
      lazyScript.p(lazyScript.match2.." is not a valid number. Valid examples are: 1, 1.0, 1.5, 19.25")
      return nil
   end
...
function lazyScript.bitParsers.ifLastUsed(bit, actions, masks)
   ...
   if (lazyScript.match2 == nil) or (lazyScript.match3 == nil) then
      lazyScript.p("syntax e.g. -ifLastUsed>5s=gouge")
      return nil
   end
   local negate = lazyScript.negate1()
   local val = tonumber(lazyScript.match2)
...
function lazyScript.bitParsers.ifInCooldown(bit, actions, masks)
   ...
   for action in string.gfind(actions, "[^,]+") do
      ...
      if (not actionObj) then
         lazyScript.p("Could not find action: "..action)
         return nil
      end
   ...
   return function(sayNothing)
      if (lazyScript.perPlayerConf.Immunities[action] and lazyScript.perPlayerConf.Immunities[action][UnitName("target")]) then
         if (lazyScript.perPlayerConf.showReasonForTargetCCd) then
            if (not sayNothing) then
               lazyScript.d("-ifTargetImmune:  "..UnitName("target").." has immunity to "..action)
            end
         end
         return true
      end
...
function lazyScript.bitParsers.ifTargetImmune(bit, actions, masks)
   ...
   if (lazyScript.match2 ~= "") then
      ...
      local lastAction = actions[table.getn(actions)]
      if (not lastAction) then
         lazyScript.p("-ifTargetImmune must appear after the action to be checked.")
         return nil
      end
...
function lazyScript.bitParsers.ifTracking(bit, actions, masks)
   ...
   for trackItem in string.gfind(trackItems, "[^,]+") do
      ...
      if (not found) then
         lazyScript.p("Did not recognise tracking type: "..trackItem)
         return nil
      end
   end
   table.insert(masks, lazyScript.orWrapper(subMasks, negate))
...
function lazyScript.bitParsers.ifCurrentAction(bit, actions, masks)
   ...
   if (lazyScript.match2 ~= "") then
      for action in string.gfind(lazyScript.match2, "[^,]+") do
         ...
         if (not actionObj) then
            lazyScript.p("Unrecognised action: "..action)
            return nil
         end
      ...
      if (not actionObj) then
         lazyScript.p("The action must appear before -ifIsCurrentAction.")
         return nil
      end
...
function lazyScript.masks.EventTime(time, op, val)
   if time ~= nil then
      ...
      elseif op == ">" then
         ...
         if not sayNothing then
            lazyScript.p("Invalid operator: "..op)
         end
         return false
      end
   else
...
function lazyScript.bitParsers.ifEventTime(bit, actions, masks)
   ...
   if factor and factor ~= "" then
      ...
      val = tonumber(s)
      if val == nil then
         lazyScript.p("'"..s.."' is not a valid number.")
         return nil
      end
   end
...
function lazyScript.masks.TimeToDeath(gtLtEq, val)
   return function(sayNothing)
      ...
      if (not sayNothing) then
         lazyScript.d("Death in: "..timeToDeath.." seconds.")
      end
...
function lazyScript.bitParsers.ifTimeToDeath(bit, actions, masks)
   ...
   if (not MobHealth_GetTargetCurHP) then
      lazyScript.p("TimeToDeath cannot be estimated without MobInfo-2 installed.")
      return nil
   end
...
function lazyScript.bitParsers.ifWeaponBuffed(bit, actions, masks)
   ...
   if (lazyScript.match2 ~= "MainHand" and lazyScript.match2 ~= "OffHand") then
      lazyScript.p("ifWeaponBuffed: Only MainHand or OffHand supported, not "..lazyScript.match2)
      return nil
   end
...
function lazyScript.spellSearch(actionObj)
   ...
   for spellIndex = 1, 1000 do
      ...
      if (not texture) then
         lazyScript.d("spellSearch found nil texture at index "..spellIndex..".")
         return false
      elseif (texture == actionObj.texture) then
         lazyScript.d("spellSearch found "..actionObj.code.." at index "..spellIndex..".")
         return spellIndex
      end
   end
   lazyScript.d("spellSearch found more than "..spellIndex.."spells.")
   return false
end
