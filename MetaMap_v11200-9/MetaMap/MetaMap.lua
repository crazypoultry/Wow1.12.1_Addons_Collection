...
function MetaMap_ChatFrame_OnEvent(event)
	if(event == "CHAT_MSG_WHISPER" and string.find(arg1, "<MapN>")) then
		if(arg2 ~= UnitName("player")) then
			MetaMapNotes_GetNoteFromChat(arg1, arg2);
		end
	elseif(event == "CHAT_MSG_WHISPER_INFORM" and string.find(arg1, "<MapN>")) then
		--- Discard the return info
	elseif(event == "CHAT_MSG_ADDON" and arg1 == "MetaMap:MN") then
		MetaMapNotes_GetNoteFromChat(arg2, arg4);
	else
...
function MetaMap_LoadZones()
	...
	for zoneKey in pairs(MetaMap_Data) do
	...
	MetaMap_Continents[0] = "World";
	for continentKey,continentName in ipairs{GetMapContinents()} do
		MetaMap_Continents[continentKey] = continentName;
		if(MetaMapNotes_Data[continentKey] == nil) then
			MetaMapNotes_Data[continentKey] = {};
		end
		if(MetaMapNotes_Lines[continentKey] == nil) then
			MetaMapNotes_Lines[continentKey] = {};
		end
...
function MetaMap_NameToZoneID(zoneText)
	...
	for continentKey in pairs(MetaMap_ZoneNames) do
		for zoneKey in pairs(MetaMap_ZoneNames[continentKey]) do
...
function MetaMap_GetCurrentMapInfo()
	...
	if(not mapName) then
		mapName = MetaMap_Continents[continent];
	end
...
function MetaMap_ZSMOnShow()
	if(GetLocale() == "deDE") then
			MetaMap_OptionsInfo:SetText("German client detected");
	elseif(GetLocale() == "frFR") then
			MetaMap_OptionsInfo:SetText("French client detected");
	else
		MetaMap_LoadZSMButton:Disable();
		MetaMap_OptionsInfo:SetText("English client detected - no zoneshift required");
	end
end
...
function MetaMap_LoadNBK(mode)
	...
	if(IsAddOnLoaded("MetaMapNBK")) then
		...
		MetaMap_StatusPrint("MetaMapNBK "..METAMAP_NOMODULE, true);
	end
end
...
function MetaMapFrameDropDown_Initialize()
  ...
  table.sort(DropDownList, MetaMap_SortCriteria);
	...
	for zoneKey in pairs(DropDownList) do
		local info = {
...
function MetaMap_Refresh()
	if(MetaMapOptions.MetaMapZone > 0 and MetaMapOptions.MetaMapZone < MetaMap_TableSize(MetaMap_Data)+1) then
...
function MetaMapMenu_Init()
	local x = 1; local y = 1;
	for i, menudata in pairs(METAMAPMENU_LIST) do
		local flag = false;
...
function MetaMap_ShowLocation(zoneName, noteName, noteID)
	...
	if(not noteID) then
		for index, value in pairs(currentZone), 1 do
...
function MetaMapList_BuildList()
	...
	if(not selectedZone or zone < 0) then
		...
		local index = 1;
		for i, value in pairs(selectedZone) do
...
function MetaMapNotes_AddNewNote(continent, zone, xPos, yPos, name, inf1, inf2, creator, icon, ncol, in1c, in2c, mininote)
	...
	if(continent == -1 or type(continent) == "string") then
		...
		index = MetaMap_TableSize(currentZone);
	end
...
function MetaMapNotes_CheckNearNotes(continent, zone, xPos, yPos)
	...
	for i, value in pairs(currentZone) do
...
function MetaMap_SendNote(mode)
	...
	elseif(mode == 2) then
		if(GetNumRaidMembers() > 0 ) then
			SendAddonMessage("MetaMap:MN", MetaMapNotes_GenerateSendString(1), "RAID");
			MetaMap_StatusPrint(METAMAPNOTES_RAIDSENT, true);
		elseif(GetNumPartyMembers() > 0) then
			SendAddonMessage("MetaMap:MN", MetaMapNotes_GenerateSendString(1), "PARTY");
...
function MetaMapNotes_MoveNote(continent, zone, id)
	...
	local lineCount = MetaMap_TableSize(zoneTable)
...
function MetaMapNotes_NewNoteSlot()
	...
	return MetaMap_TableSize(currentZone) + 1
end
...
function MetaMapNotes_WorldMapButton_OnClick(args)
	if(not MetaMapNotes_FramesHidden()) then return; end
	local mouseButton, button = args;
	...
	elseif mouseButton == "RightButton" or (mouseButton == "LeftButton" and not IsControlKeyDown() and not IsShiftKeyDown()) then
		...
		MetaMap_OrigWorldMapButton_OnClick(args);
		return;
	else
...
function MetaMapNotes_WorldMapButton_OnUpdate(...)
	...
	if(currentZone) then
		...
		if(currentLineZone) then
			for i,line in pairs(currentLineZone) do
...
function MetaMap_DeleteNotesListInit()
	local Temp_List = {};
	for continent, continentTable in pairs(MetaMapNotes_Data) do
		for zone, zoneTable in pairs(continentTable) do
			for index, indexTable in pairs(zoneTable) do
...
function MetaMapNotes_DeleteNote(id, continent, zone)
	...
	local lastEntry = MetaMap_TableSize(currentZone);
	...
	if(lastEntry ~= 0 and id <= lastEntry) then
		...
		for index, indexTable in pairs(newZone) do
			if(index ~= id) then
...
function MetaMap_InvertList(list)
  local newlist = {};
  local count = MetaMap_TableSize(list);
  for i=1,count
  do
...
function MetaMap_TableSize(table)
	if(type(table) == "string") then
		local continent, zone = MetaMap_NameToZoneID(table);
		table = MetaMapNotes_Data[continent][zone];
	end
	local i = 0;
