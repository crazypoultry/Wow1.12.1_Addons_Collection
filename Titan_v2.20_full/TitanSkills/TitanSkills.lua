...
Titan Panel [Skills] plug-in for Titan Panel that displays professions, secondary skills and Weapons Skills when hovered over. 
Skills are color coded so that you can see what needs to be worked on.

This mod hasn't been updated in a long time like so many others out there.  Though this mod is very simple, I found that I
really couldn't live without it and decided to update it.  Thanks to Corgi, the original author of this mod and all the other
contributors that helped him make the mod what it was.

Author: Morsultima  

v0.10 (September 14, 2006)
- added color coding based on skill in professions and weapons
- miscellaneous UI changes to bring mod up to current version of WoW and Titan
- toc# updated for 1.12 patch
...
TITAN_SKILLS_RED = RED_FONT_COLOR_CODE;
TITAN_SKILLS_ORANGE = "|cffff8020";
TITAN_SKILLS_YELLOW = "|cffffff20";
TITAN_SKILLS_GREEN = GREEN_FONT_COLOR_CODE;
TITAN_SKILLS_WHITE = HIGHLIGHT_FONT_COLOR_CODE;
	...
	for i=1, numSkills do
		...
		if ( SkillList[i].stype == TRADE_SKILLS or SkillList[i].stype == TITAN_SKILLS_SECONDARY_TEXT or SkillList[i].stype == TITAN_SKILLS_WEAPON_TEXT or SkillList[i].stype == TITAN_SKILLS_CLASS_TEXT ) then
			if ( currentHeader ~= prevHeader and SkillList[i].maxrank > 1 ) then
				tooltipRichText = tooltipRichText.."\n"..TITAN_SKILLS_WHITE..SkillList[i].stype.."\n";
				prevHeader = currentHeader;
			end
			
			if ( SkillList[i].maxrank > 1 ) then

				TITAN_SKILLS_PERCENT = floor( ( (SkillList[i].rankbonus + SkillList[i].rank) / SkillList[i].maxrank ) * 100 );
				TITAN_SKILLS_SKILL_COLOR = TITAN_SKILLS_RED;
				--TITAN_SKILLS_RANK_COLOR = TITAN_SKILLS_RANK_RED;

				if ( TITAN_SKILLS_PERCENT > 49) then
					TITAN_SKILLS_SKILL_COLOR = TITAN_SKILLS_ORANGE;
					--TITAN_SKILLS_RANK_COLOR = TITAN_SKILLS_RANK_ORANGE;
				end
				if ( TITAN_SKILLS_PERCENT > 74 ) then
					TITAN_SKILLS_SKILL_COLOR = TITAN_SKILLS_YELLOW;
					--TITAN_SKILLS_RANK_COLOR = TITAN_SKILLS_RANK_YELLOW;
				end
				if ( TITAN_SKILLS_PERCENT > 94 ) then
					TITAN_SKILLS_SKILL_COLOR = TITAN_SKILLS_GREEN;
					--TITAN_SKILLS_RANK_COLOR = TITAN_SKILLS_RANK_GREEN;
				end

				tooltipRichText = tooltipRichText..TITAN_SKILLS_SKILL_COLOR.." "..SkillList[i].name..":".."\t"..SkillList[i].rank;
