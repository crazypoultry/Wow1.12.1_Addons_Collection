...
-- TitanAtlas
-- Maintained by: Dan Gilbert
-- Original code: Adsertor
-- loglow@gmail.com
...
function TitanPanelAtlasButton_GetTooltipText()
	local retstr = TitanUtils_GetGreenText(ATLAS_HINT);
	return retstr;
end
...
function TitanPanelRightClickMenu_PrepareAtlasMenu()
	...
	info = {};
	info.text = ATLAS_OPTIONS_REPMAP;
	info.func = AtlasOptions_ReplaceWorldMapToggle;
	info.value = ATLAS_OPTIONS_REPMAP;
	info.checked = AtlasOptions.AtlasReplaceWorldMap;
	UIDropDownMenu_AddButton(info, UIDROPDOWNMENU_MENU_LEVEL);
	
	info = {};
	info.text = ATLAS_OPTIONS_RCLICK;
	info.func = AtlasOptions_RightClickToggle;
	info.value = ATLAS_OPTIONS_RCLICK;
	info.checked = AtlasOptions.AtlasRightClick;
	UIDropDownMenu_AddButton(info, UIDROPDOWNMENU_MENU_LEVEL);
