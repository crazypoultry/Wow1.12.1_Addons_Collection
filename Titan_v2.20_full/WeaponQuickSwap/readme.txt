...
Version History:
36 - Interface version 11200
35 - Removed early complete signal.  No longer ignores some ITEM_LOCK messages
34 - Added support for PLAYER_DEAD event to abort a swap.  No longer
     waits for last item lock to change to signal swap "Complete"
33 - No longer tries to stuff a weapon into a Soul Pouch, as if any Warlock 
     uses WeaponQuickSwap.
     Added timeout to reset mod if weapons are swapping for longer than 5s.
