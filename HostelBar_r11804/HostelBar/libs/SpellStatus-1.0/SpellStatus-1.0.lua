...
local MINOR_VERSION = "$Revision: 11060 $"
...
function SpellStatus:IsWanding()
	return (self.vars.AutoRepeating == true) and HasWandEquipped()
end
...
local function TriggerFailureEvent(self)
	--self:LevelDebug(2, "TriggerFailureEvent", self.vars.AttemptId, self.vars.AttemptName, self.vars.AttemptRank, self.vars.AttemptFullName)
	...
	elseif (self.vars.CMSFLP_SpellName == self.vars.NextMeleeName) then
		...
		self.vars.NextMeleeing = false
	elseif (self.vars.CMSFLP_SpellName == self.vars.ActiveName) then
		isActiveSpell = true
