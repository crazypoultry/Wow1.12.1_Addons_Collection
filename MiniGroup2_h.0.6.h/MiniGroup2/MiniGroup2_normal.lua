...
MiniGroup2 = AceAddon:new({
	...
	releaseDate   = "09-16-2005",
	aceCompatible = "102",
	author		  = "andreasg & UniRing",
	email		  = "andreas@nabolaget.org",
	...
	-- 	self:ApplySettings()
		...
		self:UnregisterEvent("PLAYER_ENTERING_WORLD")
			...
			self:RegisterEvent("SPELLCAST_START","UpdateSpellCastBar")
			self:RegisterEvent("SPELLCAST_STOP","UpdateSpellCastBar")
			self:RegisterEvent("SPELLCAST_FAILED","UpdateSpellCastBar")
			self:RegisterEvent("SPELLCAST_INTERRUPTED","UpdateSpellCastBar")
			self:RegisterEvent("SPELLCAST_DELAYED","UpdateSpellCastBar")
...
--sai /
	...
	StatusBarsOnValueChanged = function(self,value,pet)
		...
		if (MiniGroup2.TargetShowHostile == 1) and string.find(this:GetName(),"target") and not UnitIsFriend(string.gsub((this:GetParent()):GetName(),"MG",""), "player") then
			if ( MiniGroup2.SmoothHealthBars == 1 ) then
				self:SmoothTargetHealthBarOnValueChanged(value)
			else	
...
--		local aura, count = UnitBuff(unit, i, filter)
--		return aura, count, i, false, "HELPFUL"
	end,
	...
	AuraUpdate = function(self,unit,hat,bleh)	
		...
		if style == AuraStyle_Hide then
			...
			self:UpdateAuraStyle(unit, style, aura_Pos, filter)
		end
	end,
	...
	AuraOnEnter = function(self)
		...
		if ( this.buffFilter == "HELPFUL" or this.buffFilter == "HELPFUL_adjusted") then
			...
			if (CT_RA_BuffTextures ~= nil) then
...
--		DEFAULT_CHAT_FRAME:AddMessage("CREATEFRAME: "..unit)
		...
		Frame:CreateFontString(frameName.."_ClassText","OVERLAY")		
		text = Frame:CreateFontString(frameName.."_NameLabel","OVERLAY")
...
--		endcap:SetPoint("TOPLEFT",,"TOPRIGHT")
		...
		-- create statusbars
		local bar = CreateFrame("StatusBar",frameName.."_HealthBar")
		bar:SetParent(Frame)
