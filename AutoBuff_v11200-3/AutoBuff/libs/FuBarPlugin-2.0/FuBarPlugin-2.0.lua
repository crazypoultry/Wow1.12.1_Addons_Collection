...
local MINOR_VERSION = "$Revision: 7321 $"
...
function FuBarPlugin.GetAceOptionsDataTable(FuBarPlugin, self)
	return {
		icon = {
			...
			get = "IsIconShown",
			hidden = function()
				return not self.hasIcon or self.hasNoText
			end,
			order = -13.7,
			handler = self,
		},
		text = {
			...
			get = "IsTextShown",
			hidden = function()
				return self.cannotHideText or not self.hasIcon or self.hasNoText
			end,
			order = -13.6,
			handler = self,
		},
		colorText = {
			...
			get = "IsTextColored",
			hidden = function()
				return self.userDefinedFrame or self.hasNoText or self.hasNoColor
			end,
			order = -13.5,
			handler = self,
		},
		...
		minimapAttach = {
			...
			hidden = function()
				return self.cannotAttachToMinimap and not self:IsMinimapAttached()
			end,
			order = -13.1,
			handler = self,
		},
