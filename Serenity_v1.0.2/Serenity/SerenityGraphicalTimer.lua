...
-- }
function SerenityAfficheTimer(tableau, pointeur)
	-- One defines the place or will apparaitra the first frame   
	-- Declares that the first frame is the first mob
	
	if tableau ~= nil then
		local TimerTarget = 0;
		local yPosition = SerenityConfig.SensListe * 5;

		local PositionTitre = {};

		if SerenityConfig.SensListe > 0 then
			PositionTitre = {11, 13};
		else
			PositionTitre = {-13, -11};
		end
		...
		for index =  1, table.getn(tableau.texte), 1 do
			...
			if tableau.titre[index] then
				...
				-- Title displayed
				local frameName = "SerenityTarget"..TimerTarget.."Text";
				...
				frameItem:SetPoint(SerenityConfig.SpellTimerJust, "SerenitySpellTimerButton", "CENTER", SerenityConfig.SpellTimerPos * 23, yPosition);
				...
				if SerenityConfig.SpellTimerJust == "LEFT" then JustifInverse = "RIGHT"; end	
			
				local frameName1 = "SerenityTimer"..tableau.Gtimer[index].."Text";
				local frameItem1 = getglobal(frameName1);
				local frameName2 = "SerenityTimer"..tableau.Gtimer[index].."Bar";
				local frameItem2 = getglobal(frameName2);
				local frameName3 = "SerenityTimer"..tableau.Gtimer[index].."Texture";
				local frameItem3 = getglobal(frameName3);
				local frameName4 = "SerenityTimer"..tableau.Gtimer[index].."Spark";
				local frameItem4 = getglobal(frameName4);
				local frameName5 = "SerenityTimer"..tableau.Gtimer[index].."OutText";
				local frameItem5 = getglobal(frameName5);
			
				frameItem1:ClearAllPoints();
				frameItem1:SetPoint(SerenityConfig.SpellTimerJust, "SerenitySpellTimerButton", "CENTER", SerenityConfig.SpellTimerPos * 23, yPosition + 1);
				if SerenityConfig.Yellow then
				...
				frameItem2:SetPoint(SerenityConfig.SpellTimerJust, "SerenitySpellTimerButton", "CENTER", SerenityConfig.SpellTimerPos * 23, yPosition);
				...
				frameItem3:SetPoint(SerenityConfig.SpellTimerJust, "SerenitySpellTimerButton", "CENTER", SerenityConfig.SpellTimerPos * 23, yPosition);
				frameItem5:ClearAllPoints();
				frameItem5:SetTextColor(1, 1, 1);
				frameItem5:SetJustifyH(SerenityConfig.SpellTimerJust);
				frameItem5:SetPoint(SerenityConfig.SpellTimerJust, frameItem2, JustifInverse, SerenityConfig.SpellTimerPos * 5, 1);
				...
				yPosition = yPosition - SerenityConfig.SensListe * 11;
			end
		end
		if TimerTarget < 10 then
			for i = TimerTarget + 1, 10, 1 do
				local frameName = "SerenityTarget"..i.."Text";
				local frameItem = getglobal(frameName);
...
function Serenity_AddFrame(SpellTimer, TimerTable)
...
function Serenity_RemoveFrame(Gtime, TimerTable)
	-- The graphic timer is hidden 
	local elements = {"Text", "Bar", "Texture", "OutText"}
	for j = 1, 4, 1 do
		frameName = "SerenityTimer"..Gtime..elements[j];
		frameItem = getglobal(frameName);
		frameItem:Hide();
	end
