...
function Serenity_Msg(msg, type)
	if (msg) then		
		...
		-- If the message type is "GROUP", it is displayed in raid or party
		...
		-- If the message type is "USER" then the message is displayed
			...
			msg = Serenity_MsgAddColor(msg);
			local Intro = "|C2D47E7EESe|C2B7EECEEre|C2AA2EFEEni|C28C6F4EEty|CFFFFFFFF: ";
			if SerenityConfig.ChatType then
...
function Serenity_MsgAddColor(msg)
...
function SerenityTimerColor(percent)
...
function Serenity_MsgReplace(msg, target, Spell, mount)
