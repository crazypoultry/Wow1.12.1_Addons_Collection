...
local vmajor,vminor = "0.4", tonumber(string.sub("$Revision: 13187 $", 12, -3))
...
function Detox:SpellStatus_SpellCastInstant(sId, sName, sRank, sFullName, sCastTime)
	if cleaningSpells[sName] and cleanedType and cleanedName then
		self:MessageCombatFrame(cleanedName, DebuffTypeColor[cleanedType])
	end
	castTarget = nil
	cleanedType = nil
	cleanedName = nil
...
function Detox:SpellStatus_SpellCastFailure(sId, sName, sRank, sFullName, isActiveSpell, UIEM_Message, CMSFLP_SpellName, CMSFLP_Message)
	if CMSFLP_Message == SPELL_FAILED_LINE_OF_SIGHT then
				...
				if name == BS["Cleanse"] or name == BS["Dispel Magic"] or name == BS["Devour Magic"] then
					if rank and string.find(rank, L["Rank (%d+)"]) then
						...
						if tonumber(ranknum) > maxRankMagic then
							...
							self:Debug("found spell 'magic':",name)
						end
					else
						spells["magic"] = compost:Acquire(i, BookType, name)
						self:Debug("found spell 'magic':",name)
				...
				if name == BS["Dispel Magic"] or name == BS["Purge"] or name == BS["Devour Magic"] then
					if rank and string.find(rank, L["Rank (%d+)"]) then
						...
						if tonumber(ranknum) > maxRankEnemyMagic then
							...
							self:Debug("found spell 'enemymagic':",name)
						end
					else
						spells["enemymagic"] = compost:Acquire(i, BookType, name)
						self:Debug("found spell 'enemymagic':",name)
...
function Detox:Clean()
	...
	if not hasSpells then
		self:ParseSpellbook()
		if not hasSpells then
			self:Debug("no cleaning spells available, aborting")
			return false
		end
	end
	-- debug
	...
	if UnitExists("target") then
		if UnitIsFriend("target","player") then 
			...
			self:Debug("saving target",targetFriendlyName)
		else 
			targetIsEnemy = true
			self:Debug("saving enemy")
		end
	else
		self:Debug("no current target, not saving anything")
	end	
	...
	if not cleanedType then
		...
		self:Debug("checking",sp,"units in roster")
	end
	-- couldn't clean target, check for MCd units
	if not cleanedType and spells["enemymagic"] then
		self:Debug("searching for charmed units")
		for k,v in ipairs(priority) do
			if v.pri == 0 then break end
			local unit = v.unitid
			self:Debug("testing",v.unitid)
	...
	if not cleanedType then
		self:Debug("looping units in roster")
		for k,v in ipairs(priority) do
			if v.pri == 0 then break end
			local unit = v.unitid
			self:Debug("testing",v.unitid)
	...
	-- now try blacklist
	if not cleanedType then
		self:Debug("looping units in blacklist")
		for unit in pairs(blacklist) do
			self:Debug("testing",unit)
			cleanedType = self:CureUnit(unit)
			if cleanedType then
				cleanedName = UnitName(unit)
				blacklist[unit] = nil
	...
	-- now try enemies
	if not cleanedType and spells["enemymagic"] then
		self:Debug("looping enemies")
		for n, u in pairs(RL.roster) do
			if UnitExists(u.unitid.."target") and not UnitIsFriend("player",u.unitid.."target") then
				self:Debug("testing",u.unitid.."target")
	...
	elseif targetFriendlyName then
		if targetFriendlyName ~= UnitName("target") then 
			self:Debug("restoring target",targetFriendlyName)
			TargetByName(targetFriendlyName)
		else
			self:Debug("original friendly unit still targetted, not switching back")
		end
	elseif UnitExists("target") then 
		self:Debug("clearing target")
		ClearTarget() 
	else
		self:Debug("no original target we could restore")
	end
	-- restore self-cast
	if selfCast == 1 then
		self:Debug("restoring autoSelfCast")
	end
	SetCVar("autoSelfCast", selfCast)  -- maybe not do this here but on a delayed timer?
	if cleanedType then
		self:Debug("successfully cleaned",cleanedType,"from",cleanedName)
	end
...
function Detox:CureUnit(unit)
	self:Debug("checking if",UnitName(unit),"("..unit..")","needs a cure")
	if not self:UnitCurable(unit) then 
		self:Debug(UnitName(unit),"not curable (out of range/stealthed), skipping")
		return nil 
	end
	self:Debug(unit,"in range for curing, testing for debuffs")
	local cleaned = false
	local skip
	local type
	...
	if (spells["magic"] or spells["enemymagic"]) and aura:UnitHasDebuffType(unit, "Magic") then
		...
		if spells["enemymagic"] and UnitIsCharmed(unit) and UnitCanAttack("player", unit) then
				cleaned = self:CastCuringSpell(spells["enemymagic"], unit, true)
		elseif spells["magic"] and not UnitCanAttack("player", unit) then
				cleaned = self:CastCuringSpell(spells["magic"], unit, spells["enemymagic"])  -- will ClearCurrentTarget if we can also clean enemy magic, as we're then using Dispel Magic or Devour Magic
		end
	end
	...
	if not cleaned and spells["curse"] and not UnitIsCharmed(unit) and aura:UnitHasDebuffType(unit, "Curse") then
		type = "Curse"
		cleaned = self:CastCuringSpell(spells["curse"], unit, false)
	end
	...
	if not cleaned and ( spells["poison1"] or spells["poison2"] ) and not UnitIsCharmed(unit) and not skip and aura:UnitHasDebuffType(unit, "Poison") then
		...
		if spells["poison2"] and not aura:UnitHasBuff(unit, BS["Abolish Poison"]) then
			cleaned = self:CastCuringSpell(spells["poison2"], unit, false)
		else
			cleaned = self:CastCuringSpell(spells["poison1"], unit, false)
		end
	end
	...
	if not cleaned and unit == "player" and spells["poison3"] and aura:UnitHasDebuffType(unit, "Poison") then
		type = "Poison"
		cleaned = self:CastCuringSpell(spells["poison3"], unit, false)
	end
	...
	if not cleaned and ( spells["disease1"] or spells["disease2"] ) and not UnitIsCharmed(unit) and not skip and aura:UnitHasDebuffType(unit, "Disease") then
		...
		if spells["disease2"] and not aura:UnitHasBuff(unit, BS["Abolish Disease"]) then
			cleaned = self:CastCuringSpell(spells["disease2"], unit, false)
		else
			cleaned = self:CastCuringSpell(spells["disease1"], unit, false)
		end
	end
	if cleaned then return type else 
		self:Debug(UnitName(unit),"doesnt need any curing")
		return nil 
	end
end


function Detox:CastCuringSpell(spellID, unit, ClearCurrentTarget)
	self:Debug("attempting to cast spell",spellID[1],"on",unit)
	...
	if ClearCurrentTarget then
        if not UnitIsUnit("target", unit) then 
			self:Debug("clearing target because we're using a enemymagic spell")
			ClearTarget() 
		end
    elseif UnitIsFriend("player", "target") and not UnitIsUnit("target", unit) then
		self:Debug("clearing friendly target")
        ClearTarget()
	else
		self:Debug("spell doesnt need to clear target first")
    end
    if spellID[2] == BOOKTYPE_PET then -- Decursive is also doing this for Purge
		TargetUnit(unit) 
	end
    if SpellIsTargeting() then SpellStopTargeting() end
	-- cast spell
	castTarget = unit
	self:Debug("preparing spell",spellID[1])
	CastSpell(spellID[1], spellID[2])
	if spellID[2] == BOOKTYPE_PET then -- Decursive is also doing this for Purge
		self:Debug("pet spell, therefor targetting playertarget") --restoring target??
        TargetUnit("playertarget")
    elseif SpellIsTargeting() then -- check if spell is usable
		self:Debug("targetting unit",unit)
		SpellTargetUnit(unit)
	end
	-- if spell is still targetting, we had a problem with casting on the unit
	if SpellIsTargeting() then 
		self:Debug("error while casting")
		SpellStopTargeting()
		return false
	else
		self:Debug("casting was successful")
		return true
	end
end


function Detox:MessageCombatFrame(name, color)
	if not self.db.profile.feedback then return end
	local text = string.format(L["Cleaned %s"], name)
	...
	elseif CombatText_AddMessage then -- Blizzard Floating Combat Frame
