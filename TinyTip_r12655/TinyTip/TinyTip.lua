...
local function TooltipFormat(unit)
	...
	if _G.UnitIsPlayer(unit) and db["PvPRank"] ~= 1 and tmp2 > 0 then
			if db["PvPRank"] == 2 then
				fsSetText(gtLine1, "[" .. tmp .. "] " .. (name or "Unknown" ) .. ( (tmp3 and tmp3 ~= RealmName and ( (db["KeyServer"] and "(*)") or (" (" .. tmp3 .. ")") ) ) or "") )
			elseif db["PvPRank"] == 3 then
				fsSetText(gtLine1, (name or "Unknown") .. ( (tmp3 and tmp3 ~= RealmName and ( (db["KeyServer"] and "(*)") or (" (" .. tmp3 .. ")") ) ) or "") .. " [R" .. tmp2 .. "]" )
			else
				fsSetText(gtLine1, "[R" .. tmp2 .. "] " .. (name or "Unknown" ) .. ( (tmp3 and tmp3 ~= RealmName and ( (db["KeyServer"] and "(*)") or (" (" .. tmp3 .. ")") ) ) or "") )
			end
	else
			fsSetText(gtLine1, (name or "Unknown") .. ( (tmp3 and tmp3 ~= RealmName and ( (db["KeyServer"] and "(*)") or (" (" .. tmp3 .. ")") ) ) or "") )
	end
