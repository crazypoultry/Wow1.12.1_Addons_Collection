...
function Necrosis_InsertTimerParTable(IndexTable, Target, LevelTarget, SpellGroup, SpellTimer, TimerTable, ShadowVulnerabilityInUse)
	...
	table.insert(SpellTimer,
		{
			...
			Group = 0,
			Gtimer = nil,
            ColorVulnerable = ShadowVulnerabilityInUse,
		});
...
function NecrosisTimerX(nom, duree, truc, Target, LevelTarget, SpellGroup, SpellTimer, TimerTable, ShadowVulnerabilityInUse)
...
function Necrosis_DisplayTimer(display, index, SpellGroup, SpellTimer, GraphicalTimer, TimerTable)
    ...
    if (SpellTimer[index].ColorVulnerable) then
        color = "<lightPurple2>";
    end
	...
	if not SpellGroup.Visible[SpellTimer[index].Group]
        ...
        table.insert(GraphicalTimer.ColorVulnerable, SpellTimer[index].ColorVulnerable);
	...
	if NecrosisConfig.Circle == 2 then
		if SpellTimer[index].Name == NECROSIS_SPELL_TABLE[11].Name then
			if (minutes >= 16) then
				NecrosisButton:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\Turquoise\\Shard32");--..minutes - 15);
			elseif minutes >= 1 or seconds >= 33 then
				NecrosisButton:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\Orange\\Shard32");--..minutes + 1);
			else
				NecrosisButton:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\Rose\\Shard32");--..seconds);
			end
		end
	end
