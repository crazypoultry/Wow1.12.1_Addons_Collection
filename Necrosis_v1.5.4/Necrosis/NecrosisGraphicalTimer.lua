	...
	if tableau ~= nil then
		...
		for index =  1, table.getn(tableau.texte), 1 do
			...
			if tableau.titre[index] then
                ...
                -- color was calculated but override it in favor of white if the spell was cast with Shadow Vulnerability on target
                if (tableau.ColorVulnerable[index] == true) then
                    b = 0.784;
                    r = 0.451;
                    g = 0.105;
                end
