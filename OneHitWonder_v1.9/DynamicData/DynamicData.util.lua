...
-- public functions	
	...
	--
	notifyWhateverHandlers = function (list, p1, p2, p3, p4, p5, p6, p7, p8, p9) 
		local func = nil;
		for k, v in list do
	...
	-- Kinesia. It now _is_ copied and pasted from the new Sea function.
	...
	getItemTooltipInfo = function (bag, slot, tooltipName, strings)
		...
		local tooltip = getglobal(tooltipName);
		tooltip:SetOwner(DynamicDataScriptFrame,"ANCHOR_NONE");
	...
	-- 
	scheduleFunction = function(when, func, p1, p2, p3, p4, p5, p6, p7, p8, p9)
		if ( not when ) then
			return;
		end
		if ( not func ) then
		...
		scheduledFunction.args = { p1, p2, p3, p4, p5, p6, p7, p8, p9 };
	...
	-- 
	scheduleNamedFunction = function(name, when, func, p1, p2, p3, p4, p5, p6, p7, p8, p9)
		if ( not name ) then
			return DynamicData.util.scheduleFunction(when, func, p1, p2, p3, p4, p5, p6, p7, p8, p9);
		end
		if ( not func ) then
			return;
		end
	...
	--
	scheduleAfterInitFunction = function (func, p1, p2, p3, p4, p5, p6, p7, p8, p9) 
		local playerName = UnitName("player");
		if ( ( playerName ) and ( playerName ~= TEXT(UKNOWNBEING) ) and ( DynamicData.util.variablesHasBeenLoaded ) ) then
			func(p1, p2, p3, p4, p5, p6, p7, p8, p9);
		else
			DynamicData.util.scheduleFunction(1, DynamicData.util.scheduleAfterInitFunction, func, p1, p2, p3, p4, p5, p6, p7, p8, p9);
		end
	end;
...
-- private functions	
	...
	-- OnLoad()
	--
	--  sets up the util library. 
	--  NOTE: This no longer inserts an emulated Cosmos_Schedule if not present.
	--
	OnLoad = function()
