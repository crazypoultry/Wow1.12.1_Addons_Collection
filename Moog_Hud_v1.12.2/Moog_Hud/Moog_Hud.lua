...
function Moog_testing()
	local i;
	for i=1, 16 do
		button = getglobal("Moog_SelfHudBuff"..i);
		button:Show();
	end
	for i=1, 16 do
		button = getglobal("Moog_SelfHudDeBuff"..i);
		button:Show();
	end
end
function Moog_HudPlayerAuras()
	local i, icon, buff, debuff, debuffborder, debuffcount;
	for i=1, 16 do
		buff = UnitBuff("player", i);
		button = getglobal("Moog_SelfHudBuff"..i);
		if(buff and MoogHUDInfo.PlayerChevron) then
			icon = getglobal(button:GetName().."Icon");
			icon:SetTexture(buff);
			if (MoogHUDInfo.EnableChevronClickable) then
	...
	for i = 1, 16 do
		debuff, debuffApplications = UnitDebuff("player", i);
		button = getglobal("Moog_SelfHudDeBuff"..i);
		if (debuff and MoogHUDInfo.PlayerChevron) then
			icon = getglobal(button:GetName().."Icon");
			debuffborder = getglobal(button:GetName().."Border");
			debuffcount = getglobal(button:GetName().."Count");
			icon:SetTexture(debuff);
...
function Moog_HudOnEvent(event)
	if (event == "VARIABLES_LOADED" ) then
		...
		if MoogHUDInfo.PlayerChevron == nil then
			MoogHUDInfo.PlayerChevron = true; end
		if MoogHUDInfo.EnableChevronClickable == nil then
			MoogHUDInfo.EnableChevronClickable = false; end
		if MoogHUDInfo.SeperateNumbers == nil then
