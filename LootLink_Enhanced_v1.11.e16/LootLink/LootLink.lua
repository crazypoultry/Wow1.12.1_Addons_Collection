...
local lOriginal_DressUpItemLink;
...
function LootLink_SetHyperlink(tooltip, name, link, index)
...
function LootLink_Inspect(who)
	...
	if ( who == "mouseover" and CheckInteractDistance(who, 1) ) then
...
function LootLink_OnLoad()
	...
	-- Hook DressUpItem
	lOriginal_DressUpItemLink = DressUpItemLink;
	DressUpItemLink = LootLink_DressUpItemLink;
	
--	Removed loading messages (Phanx)
...
function LootLink_Minimap_Update()
	if ( LootLinkState.hideminimap ) then
		LootLinkMinimapButton:Hide();
	else
...
function LootLink_DressUpItemLink(item)
	lOriginal_DressUpItemLink(item);
	LootLink_ProcessLinks(item);
end
...
function LootLink_OnEvent()
	...
	elseif( event == "PLAYER_ENTERING_WORLD" ) then
		...
		if ( ConfabEditBox ) then
			ConfabEditBox:SetScript("OnChar", ChatEdit_OnChar);
		end
	...
	elseif( event == "UNIT_INVENTORY_CHANGED" ) then
		if( arg1 == "player" and not this.inspect ) then
			this.inspect = 0.5;
		end
...
function LootLinkItemButtonDropDown_OnClick()
	local id = this:GetID();
	local data = LootLinkFrame.buttondata;
	if ( not data ) then
		return;
	end
	if ( id == 2 ) then
	...
	elseif ( id == 1 ) then
		...
		local dialog = StaticPopup_Show("LOOTLINK_DELETEITEM_CONFIRM", data[2].c, data[1]);
		if ( dialog ) then
			dialog.data = data;
		end
	end
...
function LootLinkItemButtonDropDown_Initialize()
	local info = {};
	info.text = "Dress Up";
	info.notCheckable = 1;
	info.func = LootLinkItemButtonDropDown_OnClick;
	UIDropDownMenu_AddButton(info);
...
function LootLinkItemButtonDropDown_OnLoad()
	UIDropDownMenu_Initialize(this, LootLinkItemButtonDropDown_Initialize, "MENU");
end
...
function LootLinkItemButton_OnClick(button)
	...
	elseif( button == "RightButton" ) then
		if( IsShiftKeyDown() and IsControlKeyDown() ) then
			...
			LootLinkFrame.buttondata = data;
			ToggleDropDownMenu(1, nil, LootLinkItemButtonDropDown, this, 40, 0)
		end
	end
...
function LootLink_ClearDelayed()
	if ( LootLinkState.schedule ) then
		for k, v in LootLinkState.schedule do
	...
	LootLinkState.schedule = nil;
end

function LootLink_ScheduleItem(name,link)
	if ( not LootLinkState.schedule ) then
		LootLinkState.schedule = {};
	end
	for k, v in LootLinkState.schedule do
	...
	tinsert( LootLinkState.schedule, {name,link,ping/500} );
end
...
function LootLink_OnUpdate(elapsed)
	...
	if ( LootLinkFrame.inspect ) then
		LootLinkFrame.inspect = LootLinkFrame.inspect - elapsed;
		if ( LootLinkFrame.inspect < 0 ) then
			LootLinkFrame.inspect = nil;
			LootLink_ScanInventory();
			LootLink_Inspect("player");
		end
	end
		
	local list = LootLinkState.schedule;
	if ( list ) then
...
--		Adding possibility for arguments to slash commands (Phanx) ((borrowed from ItemSync)) "num, offset" necessary?
--		local command = string.lower(msg);
		local msg = string.lower(msg);
		local num, offset, command, args = string.find (msg, "^(%w+)%s*(.*)$");

--		Added chat line searching (Phanx)
		if ( command == LOOTLINK_CHATSEARCH or command == LOOTLINK_CHATSEARCH2 and args ~= nil or args ~= "" ) then
			LootLink_ChatSearch(args);

		elseif( command == LOOTLINK_HELP ) then
			local index = 0;
...
function LootLinkQuickSearch_Search(Next)
	...
	if ( text and text ~= "" ) then
		if ( not last or length < last ) then
			if ( not DoSearch(LootLinkFrame.qs or 0) and not DoSearch(1, LootLinkFrame.qs+1) ) then
...
--
-- Command line searching (Phanx)
-- Same as quick search except takes an argument and shows the window if it's hidden
--
function LootLink_ChatSearch(name)
	if ( name ) then
		if ( name == "" ) then
			name = nil;
		end
	...
	if ( not LootLinkFrame:IsVisible() ) then
		ShowUIPanel(LootLinkFrame);
	end
end
...
function LootLink_AddItem(name, link, color, trusted)
	...
	if ( name and link ) then
		...
		if ( value.i and value.i == link and LL_SearchData(value,"bu") == build ) then
			return;
		end
		...
		if ( LL_SearchData(value,"bu") == build ) then
			return;
		end
...
--------------------------------------------------------------------------------------------------
-- Type links - originally borrowed from Gazmik Fizzwidget's Linkerator
--------------------------------------------------------------------------------------------------
...
function LootLink_TypeLinks_LinkifyName(head, text, tail)
	if (head ~= "|h" and tail ~= "|h") then -- only linkify things text that isn't linked already
		...
		if ( not link ) then
			for k, v in pairs(ItemLinks) do
				if ( strlower(text) == strlower(k) ) then
					link = LootLink_GetLink(k);
					break;
				end
			end
		end
