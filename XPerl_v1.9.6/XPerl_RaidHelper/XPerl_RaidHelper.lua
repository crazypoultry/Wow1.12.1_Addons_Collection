...
local isWoW2 = strfind(GetBuildInfo(), "(%d+)") == 2		-- TBC remove
...
function XPerl_OnEnter()
	...
	if (XPerlConfigHelper.Tooltips == 1) then
		if (this.partyid and UnitName(this.partyid) and XPerlConfigHelper.TooltipsWhich == 2) then
			GameTooltip:SetOwner(this, a1)
			GameTooltip:SetUnit(this.partyid)

			if (this.unit and (this.type == "MTT" or this.type == "OT") and XPerlConfigHelper.TooltipsWhich == 2) then
				--local extra = 10
				--if (TinyTipExtrasExists) then
				--	extra = 20
				--end

				XPerl_BottomTip:SetOwner(GameTooltip, a2, 0, 10)		--extra)
				XPerl_BottomTip:SetUnit(this.unit)
				XPerl_BottomTip:SetBackdropColor(0.1, 0.4, 0.1, 0.75)
			end
		else
			if (XPerlConfigHelper.TooltipsWhich == 0) then
				GameTooltip:SetOwner(this, a1)
				GameTooltip:SetUnit(this.partyid)
			else
		...
		if (AllocateUnitNum < MAX_UNITS) then
			...
			if (unit) then
				...
				tinsert(list, unit)
                                ...
                                if (XPerl_StatusBarSetValue) then
                                	if (not XPerl_OldStatusBarSetValue) then
                                        	XPerl_OldStatusBarSetValue = unit.SetValue
                                        end
                                	unit.SetValue = XPerl_StatusBarSetValue
                                end
...
function XPerl_SetMainAssist(msg, ma, quiet)
	...
	if (ma == "broadcast") then
		if (IsRaidOfficer()) then
			...
			XPerl_Message(XPERL_HELPER_NEEDPROMOTE)
		end
	else
		if (not ma) then
			ma = 0
		...
		if (not quiet) then
			if (MainAssist) then
				XPerl_Message(string.format(XPERL_HELPER_MASET, XPerlColourTable[MainAssist.class]..MainAssist.name))
			else
				XPerl_Message(XPERL_HELPER_MACLEAR)
			end
		end
	end
...
function XPerl_MTRosterChanged()
	...
	if (MainTankCount == 0 or XPerlConfigHelper.UseCTRATargets == 0) then
		...
		if (XPerlConfigHelper.NoAutoList == 0) then
			for i = 1,GetNumRaidMembers() do
		...
		if (cmds[1] == "MA") then
			...
			if (fromID and maID) then
				if (not UnitIsUnit(fromID, "player")) then
					...
					XPerlConfigHelper.LastMainAssist = MainAssist.name
					XPerl_Message(string.format(XPERL_HELPER_MAREMOTESET, XPerlColourTable[fromClass]..from.."|r", XPerlColourTable[maClass]..MainAssist.name.."|r"))
				end
			end
		end
...
local function ScanForTargets()
	...
	local doStats = XPerl_Stats:IsShown() and XPerlConfigHelper.CollectOtherTargets == 1
	if (doStats) then
...
function XPerl_EnableDisable()
	if (XPerlConfigHelper and XPerlConfigHelper.RaidHelper == 1 and GetNumRaidMembers() > 0) then
		...
		if (KLHTM_Redraw and KLHTM_GetRaidData and XPerlConfigHelper.ThreatBar == 1) then
			if (not XPerl_Old_KLHTM_Redraw) then
				XPerl_Old_KLHTM_Redraw = KLHTM_Redraw
				KLHTM_Redraw = XPerl_KLHTM_Redraw
			end
		else
			XPerl_KLHTM_Redraw = nil
		...
		if (XPerl_Old_KLHTM_Redraw) then
			KLHTM_Redraw = XPerl_Old_KLHTM_Redraw
			XPerl_Old_KLHTM_Redraw = nil
		end
...
function XPerl_MakeTankList()
	...
	if (XPerl_NoFadeBars) then XPerl_NoFadeBars(true) end
	...
	if (MainAssist and not foundMA) then
		...
		XPerl_Message(string.format(XPERL_HELPER_MAREMOVED, XPerlColourTable[MainAssist.class]..MainAssist.name.."|r"))
		MainAssist = nil
	end
...
function XPerl_Toggle_Stats()
	if (XPerl_Stats:IsShown() or XPerlConfigHelper.CollectOtherTargets == 0) then
		XPerl_Stats:Hide()
		if (XPerl_Stats_List.shownStats ~= 0) then
...
-- XPerl_TargetFirstOther	-- TBC - Remove probably
...
-- XPerl_SmartAssist		-- TBC - Remove... /cry
	...
	local function XFindUnit()
		if (XFind) then
			...
			if (ValidTarget("target") and strlower(strsub(UnitName("target"),1,strlen(XFind))) == strlower(XFind)) then
				XPerl_Message(XPERL_HELPER_FINDFOUND)
				return true
			else
...
function XPerl_SetStatDetails(id, event)
	...
	if (XPerlConfigHelper.CollectOtherTargets == 0 or (id and XPerl_Stats_List.shownStats == id)) then
		XPerl_Stats_List.shownStats = 0
		XPerl_Stats_List:Hide()
		return
	end
...
function XPerl_Toggle_CollectOtherTargets(noChange)
	...
	if (XPerlConfigHelper.CollectOtherTargets == 0) then
		XPerl_Stats_List.shownStats = 0
		XPerl_Stats_List:Hide()
		XPerl_Stats:Hide()
	end
...
function XPerl_SetFind(msg)
	...
	if (XFind == "") then
		XFind = nil
		XPerl_Message(XPERL_HELPER_FINDCLEARED)
	else
		XPerl_Message(string.format(XPERL_HELPER_FINDSET, "|c0000FF00"..name.."|r"))
	end
...
function XPerl_UpdateThreat()
	...
	if (lastRedraw + 0.5 > GetTime()) then
		return
	end
	lastRedraw = GetTime()
