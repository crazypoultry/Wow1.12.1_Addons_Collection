...
local PartyEvents = {	"PARTY_MEMBERS_CHANGED", "PARTY_LEADER_CHANGED", "PARTY_LOOT_METHOD_CHANGED",
			...
			"PARTY_MEMBER_DISABLE", "UNIT_MODEL_CHANGED", "RAID_ROSTER_UPDATE"}
-- TBC			"UNIT_TARGET"
...
local function XPerl_Party_ShowPercentages(thisFrame)
	...
	if (XPerlConfig.ShowPartyPercent==1) then
		getglobal(thisid.."_StatsFrame"):SetWidth(141)
	end
...
local function XPerl_Party_UpdateHealth(thisFrame)
	...
	if (Partyhealth > Partyhealthmax) then
		-- New glitch with 1.12.1
		if (UnitIsDeadOrGhost(thisFrame.partyid)) then
			Partyhealth = 0
		else
			Partyhealth = Partyhealthmax
		end
	end

	if (thisFrame.feigning == 1 and not XPerl_IsFeignDeath(thisFrame.partyid)) then
	...
	if  (thisFrame.feigning == 1) then
		...
		XPerl_SetHealthBar(healthBar, Partyhealth, Partyhealthmax, XPerlConfig.HealerModeParty, XPerlConfig.HealerModePartyType)
	end
	...
	if (debuff1) then
		...
		if (XPerlConfig.PartyDebuffsBelow == 0) then
			...
			if (petFrame and petFrame:IsVisible()) then
        	                debuff1:SetPoint("TOPLEFT", "XPerl_partypet"..thisFrame:GetID().."_NameFrame", "TOPRIGHT", 0, -4)
			else
        	                debuff1:SetPoint("TOPLEFT", "XPerl_party"..thisFrame:GetID().."_StatsFrame", "TOPRIGHT", 0, -4)
			end
		else
			local buff1 = getglobal("XPerl_party"..thisFrame:GetID().."_BuffFrame_Buff1")
			if (not buff1) then
        	        	debuff1:SetPoint("TOPLEFT", ("XPerl_party"..thisFrame:GetID().."_BuffFrame"), "TOPLEFT", 0, -20)
			else
...
-- XPerl_Party_SetBuffLocation
local function XPerl_Party_SetBuffLocation(thisFrame)
...
local function XPerl_Party_Buff_UpdateAll(thisFrame)
	...
	XPerl_CheckDebuffs(partyid, thisFrame.FlashFrames)
end
...
function XPerl_Party_SetDeBuffTooltip()
	...
	XPerl_ToolTip_AddDebuffDuration(partyid, nil, this:GetID(), XPerlConfig.PartyCurableDebuffs)
end
...
local function XPerl_Party_UpdateManaType(thisFrame)
	...
	XPerl_SetManaBarType(thisFrame.partyid, getglobal(frameName))
end
		...
		if (frame:IsShown()) then
			...
			if (frame.Fading == 0) then
				if (update) then
					...
					XPerl_Party_UpdateTarget(frame)		-- TBC remove
				end
			else
...
-- Event Handler --
-------------------
function XPerl_Party_OnEvent()		-- TBC - function XPerl_Party_OnEvent(self, event)
...
XPerl_Party_Events.RAID_ROSTER_UPDATE			= XPerl_Party_Events.PLAYER_ENTERING_WORLD
...
function XPerl_Party_Set_Bits()
	for num=1,4 do
		...
		if (XPerlConfig.ShowPartyPortrait==0) then
			...
			statsFrame:SetPoint("TOPLEFT", levelFrame, "TOPRIGHT", -2, 0)
			...
			nameFrame:SetPoint("TOPLEFT", portraitFrame, "TOPRIGHT", -2, 0)
		...
		local width = (36 * XPerlConfig.ShowPartyPercent) + 106;	-- 136 enabled, 106 disabled
