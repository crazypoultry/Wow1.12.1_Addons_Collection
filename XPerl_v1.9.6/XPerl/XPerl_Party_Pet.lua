...
function XPerl_Party_Pet_CheckPet(argFrame)
	if (XPerlConfig.ShowPartyPets == 1) then
		if (UnitExists(argFrame.partyunit) and UnitExists(argFrame.petunit) and not UnitIsDead(argFrame.petunit)) then   -- and not UnitIsDeadOrGhost(argFrame.partyunit) UnitIsConnected(argFrame.petunit) and
...
local function XPerl_Party_Pet_UpdateHealth(argFrame)
	...
	if (UnitIsDead(argFrame.petunit)) then
		...
		if (XPerlConfig.HealerModeParty == 1) then
			if (XPerlConfig.HealerModePartyType == 1) then
...
local function XPerl_Party_Pet_UpdateManaType(argFrame)
	...
	XPerl_SetManaBarType(argFrame.petunit, getglobal(frameName))
end
...
function XPerl_Party_Pet_Buff_UpdateAll(argFrame)
	...
	XPerl_CheckDebuffs(argFrame.petunit, argFrame.FlashFrames)
end
...
function XPerl_Party_Pet_UpdateCombat(argFrame)
	if (UnitExists(argFrame.partyunit) and UnitExists(argFrame.petunit)) then
			...
			if (XPerlConfig.ShowPartyPetLevel == 1) then
...
function XPerl_Party_Pet_OnEvent()		-- TBC - function XPerl_Party_Pet_OnEvent(self, event)
...
function XPerl_Party_Pet_Set_Name()
	for i,frame in pairs(PartyPetFrames) do
		...
		f = getglobal(frame:GetName().."_StatsFrame")
		local manaBar = getglobal(frame:GetName().."_StatsFrame_ManaBar")
		if (XPerlConfig.ShowPartyPetMana == 1) then
			f:SetHeight(33)
			manaBar:Show()
		else
			f:SetHeight(23)
		...
		local levelText = getglobal(frame:GetName().."_NameFrameLevelText")
		if (XPerlConfig.ShowPartyPetLevel == 1) then
			levelText:Show()
		else
			levelText:Hide()
