...
-- TBC		this:RegisterEvent("UNIT_TARGET")		-- And redo event system.
-- 			In fact, just use the target code for all..
	end
end
...
-- TBC		this:UnregisterEvent("UNIT_TARGET")
	end
end
...
function XPerl_TargetTarget_UpdateDisplay(ttFrame,force)
	...
	if (UnitExists("target") and show == 1 and UnitIsConnected(ttFrame.unitid)) then		-- (UnitName("target")~=nil) and (show==1) and (UnitIsConnected(ttFrame.unitid))) then
		...
		if (ttFrame.targetname ~= nil) then
			...
			-- Set health
			XPerl_Target_SetHealth(ttFrame, XPerlConfig.ShowTargetTargetPercent)

			-- Set mana
			XPerl_Target_SetManaType(ttFrame)
			XPerl_Target_SetMana(ttFrame)
...
local function Process_HistoryFlash()
	...
	this:SubFrame("NameFrame"):SetBackdropBorderColor(r, g, b, XPerlConfig.BorderColour.a)
	this:SubFrame("StatsFrame"):SetBackdropBorderColor(r, g, b, XPerlConfig.BorderColour.a)
	this:SubFrame("LevelFrame"):SetBackdropBorderColor(r, g, b, XPerlConfig.BorderColour.a)

	if (this == XPerl_TargetTarget) then
		if (XPerl_TargetTarget_History1) then
			XPerl_TargetTarget_History1:SetBackdropBorderColor(r, g, b, XPerlConfig.BorderColour.a)
		end
	end
end
...
function XPerl_TargetTarget_OnEvent()
	...
	elseif (event == "RAID_TARGET_UPDATE") then
		...
		if (XPerl_TargetTargetTarget) then
			XPerl_TargetTarget_RaidIconUpdate(XPerl_TargetTargetTarget.unitid, XPerl_TargetTargetTarget_NameFrame_RaidIcon, XPerl_TargetTargetTarget_NameFrame)
		end
	...
	--		elseif (XPerl_TargetTargetTarget and UnitIsUnit(arg1, "targettargettarget")) then
