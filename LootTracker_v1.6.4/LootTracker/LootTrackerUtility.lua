...
function LT_GetPlayerUnitID(name)
    ...
    if (numPartyMembers == 0) then
        testIds = {"player", "target"};
    elseif (numPartyMembers <= 5) then
        testIds = {"player", "party1", "party2", "party3", "party4", "target"};
    else
...
-- Get a list of all the player names in the group.  This is used for money
-- drops.
------------------------------------------------------------------------------

function LT_GetAllPlayersInGroup()

    -- TODO: Include raid members as well.
    -- TODO: We can cache this information and hook party join/leave invents
    --       instead to save perf.
    local testIds = {"player", "party1", "party2", "party3", "party4"};
...
function LT_ParseTime(timeString)

    -- "04/19/06 08:36:53"
    local beginMatch, endMatch, month, day, year, hour, minute, second = string.find(timeString, "(%d+)/(%d+)/(%d+) (%d+):(%d+):(%d+)");

    if (beginMatch) then
        local timeData = {};
        timeData.year = 2000 + year;
        ...
        local time = time(timeData);
        
        return time;
    end
        
    LT_DebugMessage(3, "Unable to parse time: " .. timeString);
