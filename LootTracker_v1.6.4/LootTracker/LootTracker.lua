...
-- TODO: Fill in with values from this API
...
function LT_OnStartup()
    ...
    -- Check the version
    -- TODO: Why don't we know our DebugLevel at this point?
...
function LT_OnLootMessage(text)
    ...
    LT_DebugMessage(1, "Loot message did not match pattern: " .. text);
...
function LT_OnMoneyLoot(value, shared, source)
    ...
    if (item ~= nil) then
        ...
        local nCountToAdd = value;
    
        if (item.Value == nil) then
            -- New style (1.6 or later) money data
            
            -- The Value of the Money drop is 1.
            -- The drop count indicates the total real value.
            item.Value = 1;

        elseif (item.Value ~= 1) then
            -- Old style (1.5.1 or earlier) money data
            
            -- The Value of the Money item is an average drop size.
            LT_Message("Legacy (pre 1.6) money representation encountered.  Money values will become incorrect.");
            item.Value = 1;
        end
        
        local recipients = {UnitName("player")};
        if (shared) then
            recipients = LT_GetAllPlayersInGroup();
        end
    
        foreach(recipients, function(k,recipient)
        
            -- Add the receiving player to the item's Recipients list
            if (item.Recipients == nil) then
                item.Recipients = { };
            end
            LT_AddCountEntry(item.Recipients, recipient, value);
...
function LT_OnSlashCommand(rawArgument)
    ...
    elseif (command == LT_SLASHCOMMAND_UPDATE) then
        ...
        if (value ~= nil) then
            ...
            LT_UpdateDataToCurrentVersion();
            LT_Message(LT_SLASHCOMMAND_UPDATE_ERROR);
        end
        return;
        
    elseif (command == LT_SLASHCOMMAND_UPDATEVERSION) then
    
        foreach(LT_Data, function(realmnAndPlayer, userData)
            LT_Message("Checking " .. realmnAndPlayer);
            LT_MessageIndent();
            
            foreach(userData, function(sessionName, session)
                LT_Message("Checking " .. sessionName);
                LT_MessageIndent();
                
                LT_ValidateSession(session, sessionName);
