...
function oSkin:CharacterFrame_ShowSubFrame(frameName)
...
function oSkin:CF_PetTab_Update()
...
function oSkin:CF_ResizeTabs(CFTxMult)
...
--		self:Debug("SBFTB: [%s, %s, %s, %s, %s]", i, left, right, top, bottom)
        ...
        if self.db.profile.TexturedTab then oSkin:applySkin(tabName, nil, 0)
		else oSkin:applySkin(tabName) end
		if i == 1 then
...
--		self:Debug("TalentFrame_Update")
    ...
    for i=1,MAX_TALENT_TABS do
		...
		tabName:SetWidth(tabName:GetWidth() * FTyMult)
        if self.db.profile.TexturedTab then oSkin:applySkin(tabName, nil, 0)
        else oSkin:applySkin(_G["TalentFrameTab"..i]) end
		if i == 1 then
