OUTFITDISPLAYFRAME_VERSION = GetAddOnMetadata("OutfitDisplayFrame", "Version");
...
function OutfitDisplayFrame_OnEvent(...)
   ...
   elseif ( arg[1] == "BANKFRAME_OPENED" or arg[1] == "BANKFRAME_CLOSED" ) then
   ...
   elseif ( arg[1] == "PLAYER_ENTERING_WORLD" ) then
      this:RegisterEvent("ITEM_LOCK_CHANGED");
   elseif ( arg[1] == "PLAYER_LEAVING_WORLD" ) then
...
local function IsItemOneHanded(item)
   if ( item ) then
      local _,_,_,_,_,_,_,bodyslot = _GetItemInfo("item:"..item);
...
function OutfitDisplayFrame_UpdateMessage(frame, outfit)
   if ( frame ) then
      local msg;
      if ( outfit ) then
	 msg = CheckSwitchWillFail(frame, outfit);
      end
...
function OutfitDisplayFrame_GetWearing(check)
   ...
   if ( ShowingCloak() ) then
      wearing["Options"].cloak = 1;
   else
      wearing["Options"].cloak = 0;
   end
   if ( ShowingHelm() ) then
      wearing["Options"].helm = 1;
   else
      wearing["Options"].helm = 0;
   end
   return wearing;
end
...
local function ShowCheck(dohelm, flag)
   if ( flag ) then
      if ( dohelm ) then
	 ShowHelm(flag);
      else
...
function OutfitDisplayFrame_ExecuteSwapIteration()
   ...
   local beslow = PerformSlowerSwap;

   if ( outfitswap.helm ~= nil or outfitswap.cloak ~= nil ) then

      ShowCheck ( true, outfitswap.helm);
      ShowCheck ( false, outfitswap.cloak);
   ...
   if ( outfitswap and not beslow ) then
      return OutfitDisplayFrame_ExecuteSwapIteration();
   end
end

function OutfitDisplayFrame_SwitchOne(outfit, index, skipcount)
...
function OutfitDisplayFrame_SwitchOutfit(outfit)
   ...
   if ( outfit["Options"] ) then
      -- do cloak and helm setting last
      outfitswap = swaplist_push(outfitswap, nil, nil, nil, nil,
				 outfit["Options"].helm,
				 outfit["Options"].cloak);
   end

   -- do everything except weapon slots
   local skipcount = 0;
   for i=1,16,1 do
      skipcount = OutfitDisplayFrame_SwitchOne(outfit, i, skipcount);
   end
