...
function ElkBuffBar:Init()
	...
	self.frame.SetPoint_org = self.frame.SetPoint
	self.frame.SetPoint = function(self, point, frame, relativePoint , x, y)
							DEFAULT_CHAT_FRAME:AddMessage("|cffffff00[ElkBuffBar]|r DEBUG - SetPoint("..(point or "nil")..", "..(frame and frame:GetName() or "nil")..", "..(relativePoint or "nil")..", "..(x or "nil")..", "..(y or "nil")..") called.")
							self:SetPoint_org(point, frame, relativePoint , x, y)
						end
...
function ElkBuffBar:InitSettings()
		...
		if ElkBuffBarOptions.dbcolor ~= true and ElkBuffBarOptions.dbcolor ~= false then ElkBuffBarOptions.dbcolor = true end
		...
		if carg2 then
			...
			if carg2 ~= "true" and carg2 ~= "false" then
				...
				if carg2 == "true" then
					...
					ElkBuffBarOptions.dbcolor = false
				end
				self:UpdateLayout()
