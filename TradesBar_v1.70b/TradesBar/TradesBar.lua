...
--START  
  ...
  --other stuff
  BINDING_HEADER_TRADESBAR	= "TradesBar";
...
--START
  ...
  TradesBarData = { };
    ...
    TRADES_PICK_LOCK = "Schlossknacken";
...
--START
  ...
  TradesBarData = { };
    ...
    TRADES_GNOME_ENGINEERING = "Ing\195\169nierie des gnomes";
    TRADES_GOBLIN_ENGINEERING = "Ing\195\169nierie des gobelins";
...
function TradesBar_SlashHandler(msg)
	...
	elseif (string.find(msg, "help")) then
		...
		DEFAULT_CHAT_FRAME:AddMessage("Type '/tradesbar help' for help");
	end
...
function TradesBar_OnEvent()
	if (event == "PLAYER_ENTERING_WORLD") then
		...
		DEFAULT_CHAT_FRAME:AddMessage("TradesBar is now loaded. (Created by Iling)");
		DEFAULT_CHAT_FRAME:AddMessage("Type '/tradesbar help' for more options");
		StartTradesBar();
	end
...
function TradesBar_O()
	if (TradesBarData[playerName]) then
		...
		if (TradesBarData[playerName]["horizontal"] == "false" and TradesBarData[playerName]["order"] == "az") then
			for i = 2, 23 do
		...
		elseif (TradesBarData[playerName]["horizontal"] == "true" and TradesBarData[playerName]["order"] == "az") then
			for i = 2, 23 do
				local button = getglobal("TradesBarButton"..i);
				button:ClearAllPoints();
