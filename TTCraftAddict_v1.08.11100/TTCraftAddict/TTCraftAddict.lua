...
function TTCA_BuildNextItem(constructionList)
	...
	for i=1, cItems, 1 do
		...
		if ((tradeSkillDetails.NumAvailable >= currentItem.NumRequired) and (i==1)) then

			TTCA_DoSkill(currentItem.Id, currentItem.NumRequired);

			-- This means we built our intended target.  So, reset the quantity.
			TTCA_InputBox_Quantity:SetNumber(1);
			...
			-- We should figure out exactly how many the player already has so we don't make too many.
			local count = TTCA_GetCountInInventory(tradeSkillDetails.Name);

			if ((tradeSkillDetails.NumAvailable >= (currentItem.NumRequired - count)) and (currentItem.NumAvailable < (currentItem.NumRequired - count))) then

				TTCA_DoSkill(currentItem.Id, currentItem.NumRequired - count);
				return;
...
function TTCA_IsItemBuildable(buildTree, index, iNumRequired)
	...
	if (currentItem.Type == "reagent") then
		...
		if (currentItem.NumAvailable >= iNumRequired * currentItem.NumRequired) then
		...
		for i=index+1, cBuildItems, 1 do
			...
			if (i <= cBuildItems) then
				...
				if (currentItem.Level + 1 == buildTree[i].Level) then
					...
					if (not TTCA_IsItemBuildable(buildTree, i, (currentItem.NumRequired - currentItem.NumAvailable) / currentItem.NumRequired)) then
...
-- GetCountInInventory
--
-- Given an item name, finds out how many the player has in their inventory.
-- ============================================================================
function TTCA_GetCountInInventory(itemName)

	local count = 0;

	for bagId=0, 4, 1 do

		local cBagSlots = GetContainerNumSlots(bagId);

		for slotId=1, cBagSlots, 1 do

			local itemLink = GetContainerItemLink(bagId, slotId);

			if (itemLink ~= nil and string.find(itemLink, itemName)) then
