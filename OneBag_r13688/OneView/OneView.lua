--$Id: OneView.lua 15193 2006-10-26 02:38:54Z caldar $
OneView = OneCore:NewModule("OneView", "AceEvent-2.0", "AceHook-2.1", "AceDebug-2.0", "AceConsole-2.0", "AceDB-2.0")
local L = AceLibrary("AceLocale-2.2"):new("OneView")
...
function OneView:OnInitialize()
    ...
    baseArgs.args.showbutton = {
        name = L["Show Button"], type = 'toggle',
        desc = L["Toggles if the OneView quick button is shown."],
        get = function() return self.db.profile.showbutton end,
        set = function(v) 
            self.db.profile.showbutton = v 
            if v then
                OneViewFu:Show()
            else
                OneViewFu:Hide()
...
function OneView:BuildCharSelectOptions()
	...
	for serverId, v in list do
		...
		for k, v2 in v do
			...
			args.args[tmpStr].args[string.gsub(charName, " ", "")] = {
